# SNA Introduction 20_CONFIANÇA C) Confia nas ações realizadas por este serviço. (var18)
Leonardo Martins  
17 de julho de 2016  
...
load("~/SNArRDJF/Robject/0_interactive_var18.RData")
```
...
```
#Descriptive Analysis for 20_CONFIANÇA C) Confia nas ações realizadas por este serviço. (var18)

##Vertices and Edges Number

```r
#1. Vertices and Edges Number
var18_vcount<-vcount(var18)
var18_ecount<-ecount(var18)
```
##Vertice Number
...
```r
edge_density_var18<-edge_density(var18) #The proportion of present edges from all possible edges in the network.
edge_density_var18
```
...
```r
edge_connectivity(var18, source =NULL, target =NULL, checks = T) #source and target can be replaced - their are here just as default
```
...
V(var18)$name[1]  # just as an example
```
...
```r
V(var18)$name[2]  # just as an example
```
...
```r
V(var18)$name[3]  # just as an example
```
...
```r
# list by others id's
V(var18)$name[6]  # just as an example
```
...
```r
V(var18)$name[150]  # just as an example
```
...
```r
V(var18)$name[185]  # just as an example
```
...
```r
edge_connectivity(var18, source =150, target =3, checks = T) 
```
...
```r
vertex_connectivity(var18, source = NULL, target = NULL, checks = TRUE)
```
...
```r
vertex_connectivity(var18, source =6, target =185, checks = T) 
```
...
```r
save.image("~/SNArRDJF/Robject/1_intro_var18.RData") 
```
