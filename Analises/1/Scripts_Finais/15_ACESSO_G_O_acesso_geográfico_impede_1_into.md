# SNA Introduction 15_ACESSO G) O acesso geográfico a este serviço impede a realização de ativades em conjunto. (var13) 
Leonardo Martins  
17 de julho de 2016  
...
load("~/SNArRDJF/Robject/0_interactive_var13.RData")
```
...
```
#Descriptive Analysis for 15_ACESSO G) O acesso geográfico a este serviço impede a realização de ativades em conjunto. (var13) 

##Vertices and Edges Number

```r
#1. Vertices and Edges Number
var13_vcount<-vcount(var13)
var13_ecount<-ecount(var13)
```
##Vertice Number
...
```r
edge_density_var13<-edge_density(var13) #The proportion of present edges from all possible edges in the network.
edge_density_var13
```
...
```r
edge_connectivity(var13, source =NULL, target =NULL, checks = T) #source and target can be replaced - their are here just as default
```
...
V(var13)$name[1]  # just as an example
```
...
```r
V(var13)$name[2]  # just as an example
```
...
```r
V(var13)$name[3]  # just as an example
```
...
```r
# list by others id's
V(var13)$name[6]  # just as an example
```
...
```r
V(var13)$name[150]  # just as an example
```
...
```r
V(var13)$name[185]  # just as an example
```
...
```r
edge_connectivity(var13, source =150, target =3, checks = T) 
```
...
```r
vertex_connectivity(var13, source = NULL, target = NULL, checks = TRUE)
```
...
```r
vertex_connectivity(var13, source =6, target =185, checks = T) 
```
...
```r
save.image("~/SNArRDJF/Robject/1_intro_var13.RData") 
```
