...
load("~/SNArRDJF/Robject/0_interactive_atividade.RData")
```
...
```
#Descriptive Analysis for 27_ATIVIDADE

##Vertices and Edges Number

```r
#1. Vertices and Edges Number
atividade_vcount<-vcount(atividade)
atividade_ecount<-ecount(atividade)
```
##Vertice Number
...
```r
edge_density_atividade<-edge_density(atividade) #The proportion of present edges from all possible edges in the network.
edge_density_atividade
```
...
```r
edge_connectivity(atividade, source =NULL, target =NULL, checks = T) #source and target can be replaced - their are here just as default
```
...
V(atividade)$name[1]  # just as an example
```
...
```r
V(atividade)$name[2]  # just as an example
```
...
```r
V(atividade)$name[3]  # just as an example
```
...
```r
# list by others id's
V(atividade)$name[6]  # just as an example
```
...
```r
V(atividade)$name[150]  # just as an example
```
...
```r
V(atividade)$name[185]  # just as an example
```
...
```r
edge_connectivity(atividade, source =150, target =3, checks = T) 
```
...
```r
vertex_connectivity(atividade, source = NULL, target = NULL, checks = TRUE)
```
...
```r
vertex_connectivity(atividade, source =6, target =185, checks = T) 
```
...
```r
save.image("~/SNArRDJF/Robject/1_intro_atividade.RData") 
```
