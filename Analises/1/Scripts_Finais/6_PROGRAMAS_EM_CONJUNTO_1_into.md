# SNA Introduction 6_PROGRAMAS EM CONJUNTO (var4)
Leonardo Martins  
17 de julho de 2016  
...
load("~/SNArRDJF/Robject/0_interactive_var4.RData")
```
...
```
#Descriptive Analysis for 6_PROGRAMAS EM CONJUNTO (var4)

##Vertices and Edges Number

```r
#1. Vertices and Edges Number
var4_vcount<-vcount(var4)
var4_ecount<-ecount(var4)
```
##Vertice Number
...
```r
edge_density_var4<-edge_density(var4) #The proportion of present edges from all possible edges in the network.
edge_density_var4
```
...
```r
edge_connectivity(var4, source =NULL, target =NULL, checks = T) #source and target can be replaced - their are here just as default
```
...
V(var4)$name[1]  # just as an example
```
...
```r
V(var4)$name[2]  # just as an example
```
...
```r
V(var4)$name[3]  # just as an example
```
...
```r
# list by others id's
V(var4)$name[6]  # just as an example
```
...
```r
V(var4)$name[150]  # just as an example
```
...
```r
V(var4)$name[185]  # just as an example
```
...
```r
edge_connectivity(var4, source =150, target =3, checks = T) 
```
...
```r
vertex_connectivity(var4, source = NULL, target = NULL, checks = TRUE)
```
...
```r
vertex_connectivity(var4, source =6, target =185, checks = T) 
```
...
```r
save.image("~/SNArRDJF/Robject/1_intro_var4.RData") 
```
