...
load("~/SNArRDJF/Robject/0_interactive_confianca.RData")
```
...
```
#Descriptive Analysis for 31_CONFIANÇA
...
V(confianca)$name[1]  # just as an example
```
...
```r
V(confianca)$name[2]  # just as an example
```
...
```r
V(confianca)$name[3]  # just as an example
```
...
```r
# list by others id's
V(confianca)$name[6]  # just as an example
```
...
```r
V(confianca)$name[150]  # just as an example
```
...
```r
V(confianca)$name[185]  # just as an example
```
...
```r
save.image("~/SNArRDJF/Robject/1_intro_confianca.RData") 
```
