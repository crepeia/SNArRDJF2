# SNA Introduction 10_ACESSO B) O horário de funcionamento deste serviço possibilita a realização de ações em conjunto. (var8)
Leonardo Martins  
17 de julho de 2016  
...
load("~/SNArRDJF/Robject/0_interactive_var8.RData")
```
...
```
#Descriptive Analysis for 10_ACESSO B) O horário de funcionamento deste serviço possibilita a realização de ações em conjunto. (var8)
...
```r
edge_connectivity(var8, source =NULL, target =NULL, checks = T) #source and target can be replaced - their are here just as default
```
...
V(var8)$name[1]  # just as an example
```
...
```r
V(var8)$name[2]  # just as an example
```
...
```r
V(var8)$name[3]  # just as an example
```
...
```r
# list by others id's
V(var8)$name[6]  # just as an example
```
...
```r
V(var8)$name[150]  # just as an example
```
...
```r
V(var8)$name[185]  # just as an example
```
...
```r
edge_connectivity(var8, source =150, target =3, checks = T) 
```
...
```r
vertex_connectivity(var8, source = NULL, target = NULL, checks = TRUE)
```
...
```r
vertex_connectivity(var8, source =6, target =185, checks = T) 
```
...
```r
save.image("~/SNArRDJF/Robject/1_intro_var8.RData") 
```
