# SNA Introduction 25_CONFIANÇA H) Espera que o relacionamento com este serviço continue nos próximos anos (var23).
Leonardo Martins  
17 de julho de 2016  
...
load("~/SNArRDJF/Robject/0_interactive_var23.RData")
```
...
```
#Descriptive Analysis for 25_CONFIANÇA H) Espera que o relacionamento com este serviço continue nos próximos anos (var23).
...
```r
edge_connectivity(var23, source =NULL, target =NULL, checks = T) #source and target can be replaced - their are here just as default
```
...
V(var23)$name[1]  # just as an example
```
...
```r
V(var23)$name[2]  # just as an example
```
...
```r
V(var23)$name[3]  # just as an example
```
...
```r
# list by others id's
V(var23)$name[6]  # just as an example
```
...
```r
V(var23)$name[150]  # just as an example
```
...
```r
V(var23)$name[185]  # just as an example
```
...
```r
edge_connectivity(var23, source =150, target =3, checks = T) 
```
...
```r
vertex_connectivity(var23, source = NULL, target = NULL, checks = TRUE)
```
...
```r
vertex_connectivity(var23, source =6, target =185, checks = T) 
```
...
```r
save.image("~/SNArRDJF/Robject/1_intro_var23.RData") 
```
