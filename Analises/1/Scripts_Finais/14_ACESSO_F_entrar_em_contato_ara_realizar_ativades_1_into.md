# SNA Introduction 14_ACESSO F) Seu serviço tem facilade de entrar em contato com este serviço para realizar ativades em conjunto (var12).
Leonardo Martins  
17 de julho de 2016  
...
load("~/SNArRDJF/Robject/0_interactive_var12.RData")
```
...
```
#Descriptive Analysis for 14_ACESSO F) Seu serviço tem facilade de entrar em contato com este serviço para realizar ativades em conjunto (var12).
...
V(var12)$name[1]  # just as an example
```
...
```r
V(var12)$name[2]  # just as an example
```
...
```r
V(var12)$name[3]  # just as an example
```
...
```r
# list by others id's
V(var12)$name[6]  # just as an example
```
...
```r
V(var12)$name[150]  # just as an example
```
...
```r
V(var12)$name[185]  # just as an example
```
...
```r
save.image("~/SNArRDJF/Robject/1_intro_var12.RData") 
```
