# SNA Introduction 2_RELACIONAMENTO FORMAL (var0)
Leonardo Martins  
17 de julho de 2016  
...
load("~/SNArRDJF/Robject/0_interactive_var0.RData")
```
...
```
#Descriptive Analysis for 2_RELACIONAMENTO FORMAL (var0)

##Vertices and Edges Number

```r
#1. Vertices and Edges Number
var0_vcount<-vcount(var0)
var0_ecount<-ecount(var0)
```
##Vertice Number
...
```r
edge_density_var0<-edge_density(var0) #The proportion of present edges from all possible edges in the network.
edge_density_var0
```
...
```r
edge_connectivity(var0, source =NULL, target =NULL, checks = T) #source and target can be replaced - their are here just as default
```
...
V(var0)$name[1]  # just as an example
```
...
```r
V(var0)$name[2]  # just as an example
```
...
```r
V(var0)$name[3]  # just as an example
```
...
```r
# list by others id's
V(var0)$name[6]  # just as an example
```
...
```r
V(var0)$name[150]  # just as an example
```
...
```r
V(var0)$name[185]  # just as an example
```
...
```r
edge_connectivity(var0, source =150, target =3, checks = T) 
```
...
```r
vertex_connectivity(var0, source = NULL, target = NULL, checks = TRUE)
```
...
```r
vertex_connectivity(var0, source =6, target =185, checks = T) 
```
...
```r
save.image("~/SNArRDJF/Robject/1_intro_var0.RData") 
```
