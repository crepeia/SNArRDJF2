# SNA Introduction 11_ACESSO C) Este serviço disponibiliza informações sobre suas ativades para o seu serviço. (var9)
Leonardo Martins  
17 de julho de 2016  
...
load("~/SNArRDJF/Robject/0_interactive_var9.RData")
```
...
```
#Descriptive Analysis for 11_ACESSO C) Este serviço disponibiliza informações sobre suas ativades para o seu serviço. (var9)

##Vertices and Edges Number

```r
#1. Vertices and Edges Number
var9_vcount<-vcount(var9)
var9_ecount<-ecount(var9)
```
##Vertice Number
...
```r
edge_density_var9<-edge_density(var9) #The proportion of present edges from all possible edges in the network.
edge_density_var9
```
...
```r
edge_connectivity(var9, source =NULL, target =NULL, checks = T) #source and target can be replaced - their are here just as default
```
...
V(var9)$name[1]  # just as an example
```
...
```r
V(var9)$name[2]  # just as an example
```
...
```r
V(var9)$name[3]  # just as an example
```
...
```r
# list by others id's
V(var9)$name[6]  # just as an example
```
...
```r
V(var9)$name[150]  # just as an example
```
...
```r
V(var9)$name[185]  # just as an example
```
...
```r
edge_connectivity(var9, source =150, target =3, checks = T) 
```
...
```r
vertex_connectivity(var9, source = NULL, target = NULL, checks = TRUE)
```
...
```r
vertex_connectivity(var9, source =6, target =185, checks = T) 
```
...
```r
save.image("~/SNArRDJF/Robject/1_intro_var9.RData") 
```
