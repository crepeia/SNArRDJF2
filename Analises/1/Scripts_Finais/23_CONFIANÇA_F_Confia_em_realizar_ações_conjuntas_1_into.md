# SNA Introduction 23_CONFIANÇA F) Confia em realizar ações conjuntas com este serviço (var21).
Leonardo Martins  
17 de julho de 2016  
...
load("~/SNArRDJF/Robject/0_interactive_var21.RData")
```
...
```
#Descriptive Analysis for 23_CONFIANÇA F) Confia em realizar ações conjuntas com este serviço (var21).

##Vertices and Edges Number

```r
#1. Vertices and Edges Number
var21_vcount<-vcount(var21)
var21_ecount<-ecount(var21)
```
##Vertice Number
...
```r
edge_density_var21<-edge_density(var21) #The proportion of present edges from all possible edges in the network.
edge_density_var21
```
...
```r
edge_connectivity(var21, source =NULL, target =NULL, checks = T) #source and target can be replaced - their are here just as default
```
...
V(var21)$name[1]  # just as an example
```
...
```r
V(var21)$name[2]  # just as an example
```
...
```r
V(var21)$name[3]  # just as an example
```
...
```r
# list by others id's
V(var21)$name[6]  # just as an example
```
...
```r
V(var21)$name[150]  # just as an example
```
...
```r
V(var21)$name[185]  # just as an example
```
...
```r
edge_connectivity(var21, source =150, target =3, checks = T) 
```
...
```r
vertex_connectivity(var21, source = NULL, target = NULL, checks = TRUE)
```
...
```r
vertex_connectivity(var21, source =6, target =185, checks = T) 
```
...
```r
save.image("~/SNArRDJF/Robject/1_intro_var21.RData") 
```
