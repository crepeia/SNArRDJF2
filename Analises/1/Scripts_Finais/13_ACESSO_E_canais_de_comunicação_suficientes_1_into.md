# SNA Introduction 13_ACESSO E) Os canais de comunicação com este serviço são suficientes para realização de ações em conjunto. (var11)
Leonardo Martins  
17 de julho de 2016  
...
load("~/SNArRDJF/Robject/0_interactive_var11.RData")
```
...
```
#Descriptive Analysis for 13_ACESSO E) Os canais de comunicação com este serviço são suficientes para realização de ações em conjunto. (var11)
...
```r
edge_connectivity(var11, source =NULL, target =NULL, checks = T) #source and target can be replaced - their are here just as default
```
...
V(var11)$name[1]  # just as an example
```
...
```r
V(var11)$name[2]  # just as an example
```
...
```r
V(var11)$name[3]  # just as an example
```
...
```r
# list by others id's
V(var11)$name[6]  # just as an example
```
...
```r
V(var11)$name[150]  # just as an example
```
...
```r
V(var11)$name[185]  # just as an example
```
...
```r
edge_connectivity(var11, source =150, target =3, checks = T) 
```
...
```r
vertex_connectivity(var11, source = NULL, target = NULL, checks = TRUE)
```
...
```r
vertex_connectivity(var11, source =6, target =185, checks = T) 
```
...
```r
save.image("~/SNArRDJF/Robject/1_intro_var11.RData") 
```
