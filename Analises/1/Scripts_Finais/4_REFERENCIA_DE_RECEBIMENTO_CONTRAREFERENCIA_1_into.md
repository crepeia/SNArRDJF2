# SNA Introduction 4_REFERENCIA DE RECEBIMENTO CONTRAREFERENCIA (var2)
Leonardo Martins  
17 de julho de 2016  
...
load("~/SNArRDJF/Robject/0_interactive_var2.RData")
```
...
```
#Descriptive Analysis for 4_REFERENCIA DE RECEBIMENTO CONTRAREFERENCIA (var2)

##Vertices and Edges Number

```r
#1. Vertices and Edges Number
var2_vcount<-vcount(var2)
var2_ecount<-ecount(var2)
```
##Vertice Number
...
```r
edge_density_var2<-edge_density(var2) #The proportion of present edges from all possible edges in the network.
edge_density_var2
```
...
```r
edge_connectivity(var2, source =NULL, target =NULL, checks = T) #source and target can be replaced - their are here just as default
```
...
V(var2)$name[1]  # just as an example
```
...
```r
V(var2)$name[2]  # just as an example
```
...
```r
V(var2)$name[3]  # just as an example
```
...
```r
# list by others id's
V(var2)$name[6]  # just as an example
```
...
```r
V(var2)$name[150]  # just as an example
```
...
```r
V(var2)$name[185]  # just as an example
```
...
```r
edge_connectivity(var2, source =150, target =3, checks = T) 
```
...
```r
vertex_connectivity(var2, source = NULL, target = NULL, checks = TRUE)
```
...
```r
vertex_connectivity(var2, source =6, target =185, checks = T) 
```
...
```r
save.image("~/SNArRDJF/Robject/1_intro_var2.RData") 
```
