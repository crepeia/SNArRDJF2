---
title: "SNA Introduction 1.2 - ALGUM RELACIONAMENTO (x2)"
...
load("~/SNArRDJF/Robject/0_interactive_x2.RData")
```
...
```
#Descriptive Analysis for 1.2 - ALGUM RELACIONAMENTO (x2)
...
V(x2)$name[1]  # just as an example
V(x2)$name[2]  # just as an example
V(x2)$name[3]  # just as an example

# list by others id's
V(x2)$name[6]  # just as an example
V(x2)$name[150]  # just as an example
V(x2)$name[185]  # just as an example
...
```{r, echo=T, fig.height=12, fig.width=12, message=FALSE, warning=FALSE}
save.image("~/SNArRDJF/Robject/1_intro_x2.RData") 
```
