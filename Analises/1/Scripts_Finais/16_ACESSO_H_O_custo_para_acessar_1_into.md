# SNA Introduction 16_ACESSO H) O custo para acessar este serviço impede a realização de ativades em conjunto. (var14)
Leonardo Martins  
17 de julho de 2016  
...
load("~/SNArRDJF/Robject/0_interactive_var14.RData")
```
...
```
#Descriptive Analysis for 16_ACESSO H) O custo para acessar este serviço impede a realização de ativades em conjunto. (var14)
...
```r
edge_connectivity(var14, source =NULL, target =NULL, checks = T) #source and target can be replaced - their are here just as default
```
...
V(var14)$name[1]  # just as an example
```
...
```r
V(var14)$name[2]  # just as an example
```
...
```r
V(var14)$name[3]  # just as an example
```
...
```r
# list by others id's
V(var14)$name[6]  # just as an example
```
...
```r
V(var14)$name[150]  # just as an example
```
...
```r
V(var14)$name[185]  # just as an example
```
...
```r
edge_connectivity(var14, source =150, target =3, checks = T) 
```
...
```r
vertex_connectivity(var14, source = NULL, target = NULL, checks = TRUE)
```
...
```r
vertex_connectivity(var14, source =6, target =185, checks = T) 
```
...
```r
save.image("~/SNArRDJF/Robject/1_intro_var14.RData") 
```
