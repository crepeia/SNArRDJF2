...
#1. Descriptive Analysis for var0* - Complete Graph - All networks were generated using from this one
...
hist(degree(var0, mode = "in", normalized = F), ylab="Frequency", xlab="Degree",  breaks=vcount(var0)/10, main="Histogram of Indegree Nodes - var0*")
hist(degree(var0, mode = "out", normalized = F), ylab="Frequency", xlab="Degree",  breaks=vcount(var0)/10, main="Histogram of Outdegree Nodes - var0*")
hist(degree(var0, mode = "all", normalized = F), ylab="Frequency", xlab="Degree",  breaks=vcount(var0)/10, main="Histogram of All Degree Nodes - var0*")
```  
...
```{r}
V(var0)$strength_var0<- strength(var0, weights=E(var0)$var0)
strength_var0<- strength(var0, weights=E(var0)$var0)

##Stats
summary(strength_var0)
sd(strength_var0)
```
...
#Histogram var0 degree
hist(degree(var0, mode = "in", normalized = F), ylab="Frequency", xlab="Normalized Degree",  breaks=vcount(var0)/10, main="Histogram of Normalized Indegree Nodes - var0*")
hist(degree(var0, mode = "out", normalized = F), ylab="Frequency", xlab="Normalized Degree",  breaks=vcount(var0)/10, main="Histogram of Normalized Indegree Nodes - var0*")
hist(degree(var0, mode = "all", normalized = F), ylab="Frequency", xlab="Normalized Degree",  breaks=vcount(var0)/10, main="Histogram of Normalized All Degree Nodes - var0*")
```

#2.2.1 Centralization Degree
```{r}
V(var0)$centr_degree_var0 <- centralization.degree(var0)$res
centr_degree_var0 <- centralization.degree(var0)

#Centralization
centr_degree_var0$centralization
#Theoretical Max
centr_degree_var0$theoretical_max
```
...
hist(degree.distribution(var0), breaks=vcount(var0)/10, ylab="Frequency", xlab="Degree Distribuition", main="Histogram of Degree Distribuition - var0*")

#Degree Probability Distribution
dd <- degree.distribution(var0, cumulative=T, mode="all")
plot(dd, pch=19, cex=1, col="orange", xlab="Degree", ylab="Cumulative Frequency", main= "Cumulative Frequency of var0* ")
```
...
plot(d[ind], 
     ...
     main="Log-Log Degree Distribution For var0*"
     )
```

#2.5.1 Average Neighbor Degree versus Vertex Degree (log-log scale for var0*)
```{r}
#Average Neighbor Degree versus vertex degree (log-log scale for var0)
a.nn.deg.var0 <- graph.knn(var0,V(var0))$knn

#Saving to igraph object
V(var0)$a.nn.deg.var0 <- a.nn.deg.var0 <- graph.knn(var0,V(var0))$knn
...
plot(degree(var0), 
     ...
     main="Average Neighbor Degree vs Vertex Degree - Log-Log Scale for var0*"
     )
```

#2.5.2 Average Weighted Neighbor Degree versus vertex degree (log-log scale for weighted var0*)
```{r}
#Average Weighted Neighbor Degree versus vertex degree (log-log scale for var0)
a.nn.deg.var0_w <- graph.knn(var0,V(var0), weights=E(var0)$var0)$knn

#Saving to igraph object
V(var0)$a.nn.deg.var0_w <-a.nn.deg.var0 <- graph.knn(var0,V(var0), weights=E(var0)$var0)$knn

#Average Weighted Neighbor Descriptive
summary(a.nn.deg.var0_w)
sd(a.nn.deg.var0_w)
...
plot(degree(var0), 
     ...
     xlab=c("Log Vertex Degree"),
     ylab=c("Log Average Neighbor Degree"),
     main="Average Weighted Neighbor Degree vs Vertex Degree - Log-Log Scale For Weighted var0*"
     )
```
...
#Adding Title
  title("Network Vertex Degree Sized - var0*", sub = "Source: from authors ")
...
```{r}
#Get Variable
V(var0)$var0_color_degree<-V(var0)$totaldegree
...
#Adding Title
  title("Network Vertex Degree Sized and Colored - var0*", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
   sprintf("Median In Degree: %.2f\nMedian Out Degree: %.2f",
     median(degree(var0, mode="in")), 
     median(degree(var0, mode="out"))
...
```{r}
#Get Variable
V(var0)$var0_color_degree<-V(var0)$centr_degree_var0
...
plot(small_var0, 
     ...
     vertex.frame.color="#ffffff",
     edge.arrow.size=0.1,
     vertex.label.cex=log(degree(small_var0))/10,
...
#Adding Title
  title("Network Higher Than Median Degree - var0*", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
   sprintf("Median In Degree: %.2f\nMedian Out Degree: %.2f",
...
plot(small_var0, 
     ...
     vertex.label.cex=log(degree(small_var0)+2)/2,
...
#Adding Title
  title("Network Lower than Median Degree - var0*", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
   sprintf("Median In Degree: %.2f\nMedian Out Degree: %.2f",
     median(degree(small_var0, mode="in")), 
     median(degree(small_var0, mode="out"))
...
```{r}
#Plotting based only on degree measures  #a.nn.deg.var0_w 
plot(var0, 
     layout=layout.kamada.kawai(var0), 
     vertex.label=get.vertex.attribute(var0,"LABEL_COR"),
     ...
     vertex.frame.color="#ffffff",
     edge.arrow.size=0.1,
     vertex.label.cex=sqrt(graph.knn(var0,V(var0))$knn)/20,
     edge.width=(edge.betweenness(var0, weights =E(var0)$var0))/500,
     edge.curved = TRUE
     )
...
#Adding Title
  title("Network Average Neighbor Degree Sized - var0*", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
...
plot(var0, 
     layout=layout.kamada.kawai(var0), 
     vertex.label=get.vertex.attribute(var0,"LABEL_COR"),
     vertex.size=sqrt(graph.knn(var0,V(var0), weights= E(var0)$var0)$knn),
     vertex.frame.color="#ffffff",
     edge.arrow.size=0.1,
     vertex.label.cex=log(graph.knn(var0,V(var0), weights= E(var0)$var0)$knn)/10,
     edge.width=edge.betweenness(var0, weights =E(var0)$var0)/500,
     edge.curved = TRUE
     )
...
#Adding Title
  title("Network Average Weighted Neighbor Degree Sized - var0*", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
   sprintf("Median Average Weighte Neighbor Degree: %.2f",
     median(graph.knn(var0,V(var0), weights= E(var0)$var0)$knn)
   ))
```
...
#Saving to Igraph object
V(var0)$incloseness_w <- closeness(var0, mode = "in", normalized = T, weights=E(var0)$var0) #%>% round(6)
V(var0)$outcloseness_w <- closeness(var0, mode = "out", normalized = T,weights=E(var0)$var0) #%>% round(6)
V(var0)$totalcloseness_w <- closeness(var0, mode = "total", normalized = T, weights=E(var0)$var0) #%>% round(6)

#Saving to Environment
var0_incloseness_w<- closeness(var0, mode = "in", normalized = T, weights=E(var0)$var0) #%>% round(6)
var0_outcloseness_w<- closeness(var0, mode = "out", normalized = T, weights=E(var0)$var0) #%>% round(6)
var0_totalcloseness_w<- closeness(var0, mode = "total", normalized = T, weights=E(var0)$var0) #%>% round(6)
...
```{r}
V(var0)$closeness_stats_var0_w <- closeness(var0, vids = V(var0), mode = "all", weights =V(var0)$var0, normalized = FALSE)

closeness_stats_var0_w <- closeness(var0, vids = V(var0), mode = "all", weights =V(var0)$var0, normalized = FALSE)
```

#2.7.5 Centralization Closseness
```{r}
V(var0)$centr_closeness_var0 <- centralization.closeness(var0)$res
centr_closeness_var0 <- centralization.closeness(var0)

#Centralization
centr_closeness_var0$centralization
#Theoretical Max
centr_closeness_var0$theoretical_max
```
...
#Adding Title
  title("Network Closeness Degree Sized and Colored - var0*", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
...
```{r}
#Get Variable
V(var0)$var0_color_degree<-round(V(var0)$centr_closeness_var0,3)
...
#Adding Title
  title("Network Centralization Closeness - var0*", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
   sprintf("Median In Closennes: %.4f\nMedian Out Closennes: %.4f",
     median(closeness(var0, mode="in")), 
     median(closeness(var0, mode="out"))
...
```{r}
V(var0)$betweenness_var0_w <- betweenness(var0, weights=E(var0)$var0) %>% round(1)

betweenness_var0_w <- betweenness(var0, weights=E(var0)$var0) %>% round(1)

##all
summary(betweenness_var0_w)
sd(betweenness_var0_w)
```

#2.8.3 Centralization Betweenness
```{r}
V(var0)$centr_betweenness_var0 <- centralization.betweenness(var0)$res
centr_betweenness_var0 <- centralization.betweenness(var0)

#Centralization
centr_betweenness_var0$centralization
#Theoretical Max
centr_betweenness_var0$theoretical_max
```
...
plot(var0, 
     ...
     edge.width=log(edge.betweenness(var0, weights =E(var0)$var0))/50,
     edge.curved = TRUE
     )
...
#Adding Title
  title("Network Vertex Betweenness Sized - var0*", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
...
```{r}
#Get Variable
V(var0)$var0_color_degree<-round(V(var0)$centr_betweenness_var0,3)
...
#Adding Title
  title("Network Centralization Betweenness - var0*", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
...
```{r}
#Eigen_Centrality 
V(var0)$eigenvector <- eigen_centrality(var0, directed = TRUE, weights = V(var0)$var0) %$% vector %>% round(3)

var0_eigenvector <- eigen_centrality(var0, directed = TRUE, weights = V(var0)$var0) %$% vector %>% round(3)

#Eigenvalue Calculated Eigenvector
var0_eigenvector_value <- eigen_centrality(var0, directed = TRUE, weights = V(var0)$var0) %$% value %>% round(3) # The eigenvalue corresponding to the calculated eigenvector, i.e. the centrality scores.
```
...
```{r}
V(var0)$centr_eigen_var0 <- centr_eigen(var0, directed = T)$vector
centr_eigen_var0 <-centr_eigen(var0, directed = T)$vector
#Centralization
...
plot(var0, 
     layout=layout.kamada.kawai(var0), 
     vertex.label=get.vertex.attribute(var0,"LABEL_COR"),
     vertex.size=(eigen_centrality(var0)$vector)*20,
     vertex.frame.color="#ffffff",
     edge.arrow.size=0.1,
     vertex.label.cex=sqrt(eigen_centrality(var0)$vector)/2,
     edge.width=edge.betweenness(var0, weights =E(var0)$var0)/500,
     edge.curved = TRUE
     )
...
#Adding Title
  title("Eigenvector Centrality Measure - var0*", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
...
plot(var0, 
     ...
     vertex.frame.color="#ffffff",
     edge.arrow.size=0.1,
     vertex.label.cex=(centr_eigen(var0, directed = T)$vector + 1)/10,
     edge.width=(edge.betweenness(var0, weights =E(var0)$var0))/500,
     edge.curved = TRUE
     )
...
power_centrality_stats_var0 <- power_centrality(var0, nodes = V(var0), loops = FALSE, exponent = 1, rescale = FALSE, tol = 1e-07, sparse = TRUE)

#Saving power as an igraph attributte
V(var0)$power_centrality_stats_var0 <-power_centrality(var0, nodes = V(var0), loops = FALSE, exponent = 1, rescale = FALSE, tol = 1e-07, sparse = TRUE) 

#Or are you more powerful if you're connected to weak, dependent people? (negative attenuation factor)
i=sort(power_centrality_stats_var0) %>% round(3)
table_i<-cbind(V(var0)$V5_LABEL_LABEL,i)
...
d=sort(power_centrality_stats_var0, decreasing = T) %>% round(3)
table_d<-cbind(V(var0)$V5_LABEL_LABEL,d)
...
V(var0)$power_centrality_stats_var0_r<-round(V(var0)$power_centrality_stats_var0,0)

# Creating brewer pallette
vertex_col_spower <- colorRampPalette(brewer.pal(length(unique(V(var0)$power_centrality_stats_var0_r)), "Blues"))(length(unique(V(var0)$power_centrality_stats_var0_r))) 

#Saving as Vertex properties                                   
V(var0)$color_spower <- vertex_col_spower[factor(V(var0)$power_centrality_stats_var0_r)]  
```
...
#Solving Problems with legend rendering 
a<-V(var0)$power_centrality_stats_var0_r
...
#Adding Title
  title("Bonacich Power Centrality Scores of Network Positions - var0*", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
   sprintf("Median Power Degree: %.2f\n SD Power Degree:%.2f",
     median(power_centrality_stats_var0),sd(power_centrality_stats_var0)
     ))
```
...
```{r, results="asis"}
#Saving power as an object
alpha_centrality_stats_var0 <- alpha_centrality(var0, loops = FALSE, alpha = 1, exo = 1, weights = V(var0)$var0, tol = 1e-07, sparse = TRUE)

#Saving power as an igraph attributte
V(var0)$alpha_centrality_stats_var0<-alpha_centrality(var0, loops = FALSE, alpha = 1, exo = 1, weights = V(var0)$var0, tol = 1e-07, sparse = TRUE)

#Descriptive 
summary(alpha_centrality_stats_var0)
sd(alpha_centrality_stats_var0)

#Or are you more powerful if you're connected to weak, dependent people? (negative attenuation factor)
i=sort(alpha_centrality_stats_var0) %>% round(3)
table_i<-cbind(V(var0)$V5_LABEL_LABEL,i)
...
#Are you more powerful if you're connected to powerful people? (positive attenuation factor)
d=sort(alpha_centrality_stats_var0, decreasing = T) %>% round(3)
table_d<-cbind(V(var0)$V5_LABEL_LABEL,d)
colnames(table_d) <- c("ID", "Power")
table_d10<-table_d[1:10,]
...
```{r}
# Set colors to plot distances:
V(var0)$alpha_centrality_stats_var0_r<-round(V(var0)$alpha_centrality_stats_var0,0)

# Creating brewer pallette
vertex_col_s <- colorRampPalette(brewer.pal(length(unique(V(var0)$alpha_centrality_stats_var0_r)), "YlOrRd"))(length(unique(V(var0)$alpha_centrality_stats_var0_r))) 

#Saving as Vertex properties                                                                  
V(var0)$color_s <- vertex_col_s[factor(V(var0)$alpha_centrality_stats_var0_r)]  
```
...
plot(var0, 
     ...
     vertex.label.color="black",
     vertex.frame.color="#ffffff",
     vertex.size=sqrt((alpha_centrality(var0, weights = V(var0)$var0)^2)),
     edge.arrow.size=0.1,
...
#Solving Problems with legend rendering 
a<-V(var0)$alpha_centrality_stats_var0_r
b<-V(var0)$color_s
c<-table(a,b)
d<-as.data.frame(c)
e<-subset(d, d$Freq>0)
...
#Adding Title
  title("Bonacich's Alpha Centrality Scores of Network Positions - var0*", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
   sprintf("Median Alpha Degree: %.2f\n SD Alpha Degree:%.2f",
     median(alpha_centrality_stats_var0),sd(alpha_centrality_stats_var0)
     ))
```
...
var0_hub<- hub.score(var0)$vector
...
plot(var0, 
     layout=layout.kamada.kawai(var0), 
     vertex.label=get.vertex.attribute(var0,"LABEL_COR"),
     vertex.size=10*sqrt(hub.score(var0)$vector),
     edge.arrow.size=0.1,
     vertex.label.cex=log(degree(var0))/10,
     edge.width=log(edge.betweenness(var0, weights =E(var0)$var0))/5,
     edge.curved = TRUE)
...
#Adding Title
  title("Network Hub Sized - var0*", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
...
var0_auth <- authority.score(var0)$vector
#stats
summary(var0_auth)
sd(var0_auth)
...
plot(var0, 
     layout=layout.kamada.kawai(var0), 
     vertex.label=get.vertex.attribute(var0,"LABEL_COR"),
     vertex.size=10*sqrt(authority.score(var0)$vector),
     edge.arrow.size=0.1,
     vertex.label.cex=log(degree(var0))/10,
     edge.width=log(edge.betweenness(var0, weights =E(var0)$var0))/5,
     edge.curved = TRUE)
...
#Adding Title
  title("Network Authorities Scores  Sized - var0*", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
...
plot(var0, 
     layout=layout.kamada.kawai(var0), 
     vertex.label=get.vertex.attribute(var0,"LABEL_COR"),
     vertex.size=30*sqrt(page.rank(var0)$vecto),
     edge.arrow.size=0.1,
     vertex.label.cex=log(degree(var0))/10,
     edge.width=log(edge.betweenness(var0, weights =E(var0)$var0))/100,
     edge.curved = TRUE)
...
#Adding Title
  title("Network Google Page Rank Sized - var0*", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
...
```{r}
#For in-degrees:
indegnamesPB=which(indegPB>sort(indegPB)[vcount(var0)-5]) #For out- degrees:
outdegnamesPB=which(outdegPB>sort(outdegPB)[vcount(var0)-5]) #For PageRank:
prnamesPB=which(prPB>sort(prPB)[vcount(var0)-5]) #For Hubs:
hnamesPB=which(hPB>sort(hPB)[vcount(var0)-5]) #For Authorities:
authnamesPB=which(authPB>sort(authPB)[vcount(var0)-5])
...
plot(make_ego_graph_all_var0, 
     ...
     edge.arrow.size=0.1,
     vertex.label.cex=log(degree(make_ego_graph_all_var0))/10,
...
#Adding Title
  title("Ego Network Page Rank Sized - All connections from CAPSAD - var0*", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
   sprintf("Mean Page Rank Scores: %.2f\n SD Page Rank Scores: %.2f",
...
plot(make_ego_graph_out_var0, 
     ...
     vertex.size=30*sqrt(page.rank(make_ego_graph_out_var0)$vector),
     edge.arrow.size=0.1,
     vertex.label.cex=log(degree(make_ego_graph_out_var0))/10,
...
#Adding Title
  title("Ego Network Page Rank Sized - Out Connections from CAPSAD - var0*", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
   sprintf("Mean Page Rank Scores: %.2f\n SD Page Rank Scores: %.2f",
...
plot(make_ego_graph_in_var0, 
     ...
     vertex.size=30*sqrt(page.rank(make_ego_graph_in_var0)$vector),
     edge.arrow.size=0.1,
...
#Adding Title
  title("Ego Network CAPS AD Page Rank Sized - In connections from CAPS AD - var0*", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
   sprintf("Mean Page Rank Scores: %.2f\n SD Page Rank Scores: %.2f",
...
plot(make_ego_graph_all_var0_without_CAPSAD, 
     ...
     vertex.size=30*sqrt(page.rank(make_ego_graph_all_var0_without_CAPSAD)$vector),
     edge.arrow.size=0.1,
     vertex.label.cex=log(degree(make_ego_graph_all_var0_without_CAPSAD))/10,
     edge.width=log(edge.betweenness(make_ego_graph_all_var0_without_CAPSAD))/10,
     edge.curved = TRUE)
...
#Adding Title
  title("Ego Network CAPS AD Page Rank Sized - All Connections Removing CAPS AD - var0*", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
   sprintf("Mean Page Rank Scores: %.2f\n SD Page Rank Scores: %.2f",
...
plot(make_ego_graph_all_var0_without_complex, 
     ...
     edge.arrow.size=0.1,
     vertex.label.cex=log(degree(make_ego_graph_all_var0_without_complex))/10,
     edge.width=log(edge.betweenness(make_ego_graph_all_var0_without_complex))/10,
     edge.curved = TRUE)
...
#Adding Title
  title("Ego Network CAPS AD Page Rank Sized - All Connections Removing High Complex Service - var0*", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
   sprintf("Mean Page Rank Scores: %.2f\n SD Page Rank Scores: %.2f",
...
#Adding Title
  title("Minimum Spanning Tree Non-normalized - from var0*", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
   sprintf("Median In Degree: %.2f\nMedian Out Degree: %.2f",
...
msp_var0_w<-minimum.spanning.tree(var0, weights=1/edge.betweenness(var0, weights =E(var0)$var0), algorithm='prim')
...
#Plotting based only on degree measures 
plot(msp_var0_w, 
     layout=layout.kamada.kawai(msp_var0_w), 
     ...
     edge.width=1/edge.betweenness(msp_var0_w, weights =E(msp_var0_w)$msp_var0_w),
     edge.curved = TRUE
     )
...
#Adding Title
  title("Minimum Weighted Spanning Tree (Edge Betweenness) - var0*", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
   sprintf("Median In Degree: %.2f\nMedian Out Degree: %.2f",
...
is.connected(var0) #trick information - let's check this out further
```

#3.3.1 Strong Connected Components
```{r}
#Strong Connected
components_strong_var0<-components(var0, mode="strong")
V(var0)$components_strong_var0_membership<-components_strong_var0$membership # adding to igraph object

#Number of clusters
count_components(var0)

#Display components structure
components_strong_var0
```
...
plot(var0, 
     ...
     mark.groups = by(seq_along(components_strong_var0$membership),
                      components_strong_var0$membership, 
...
#Adding Title
  title("Network Vertex Degree Sized - Classfied by Components Structure - var0*", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
   sprintf("Median In Degree: %.2f\nMedian Out Degree: %.2f",
     median(degree(var0, mode="in")), 
     median(degree(var0, mode="out"))
...
```{r}
#Shortest Paths
var0_sp_in <- shortest.paths(var0, mode='in', weights=V(var0)$var0) # sames as bellow
var0_sp_out <- shortest.paths(var0, mode='out', weights=V(var0)$var0) #same as above
var0_sp_all <- shortest.paths(var0, mode='all', weights=V(var0)$var0) # all
...
```{r}
#All shortest paths 
distances_sp_all_var0<-distances(var0, mode="all", weights=E(var0)$var0)
#distances_sp_all_var0

#In shortest paths 
distances_sp_in_var0<-distances(var0, mode="in",weights=E(var0)$var0)
#distances_sp_in_var0

#Out shortest paths 
distances_sp_out_var0<-distances(var0, mode="out", weights=E(var0)$var0)
#distances_sp_out_var0
```

#3.7.4 Histogram from shortest path length between each pair of vertices. 
```{r}
distance_table(var0, directed = TRUE)
```
...
var0_dist.from.CAPSAD <- distances(var0, v=V(var0)[V2_LABEL_ID=="q170_CAPS...CAPS.AD"], to=V(var0), weights=V(var0)$var0)
...
#Adding Title
  title("Distance from CAPS AD - var0*", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
...
cliques_var0_u_size<-sapply(cliques(var0_u), length) # clique sizes
largest_cliques(var0_u) # cliques with max number of nodes
```
...
V(as.undirected_var0_u)$vcollabel <- recode(V(as.undirected_var0_u)$vcol, "'gold' = 'Largest Clique'; else= 'Others'")
...
plot(as.undirected(var0_u), 
     ...
     edge.arrow.size=0.1,
     vertex.label.cex=log(degree(as.undirected(var0_u)))/10,
     edge.width=log(edge.betweenness(as.undirected(var0_u), weights =E(as.undirected(var0_u))$var0))/5,
     edge.curved = TRUE
     )
...
```{r}
#Edge Betweenness
E(var0)$edge.btweenness<-edge.betweenness(var0, weights =E(var0)$var0)
...
E(var0)[order(var0_edge.betweenness, decreasing=T)[1:10]]
```

#3.10.1 Community detection (using edge betweenness)
```{r}
## Community structure detection based on edge betweenness
communityEdgeBetwn_var0 <- edge.betweenness.community(var0)
...
plot(comps_var01, 
     ...
     edge.arrow.size=0.1,
     vertex.label.cex=log(degree(comps_var01))/10,
...
#Non-weighted 
transitivity(var0, weights=E(var0)$var0, type="global")

#Weighted
transitivity(var0, weights=E(var0)$var0,type="weighted")
transitivity(var0, weights=E(var0)$var0,type="global")
...
transitivity_local_w<-transitivity(var0, weights=E(var0)$var0, type="barrat")
...
#Diameter Weigthed 
diameter(var0, weights=E(var0)$var0)
var0_dim<-get.diameter(var0, weights=E(var0)$var0)
var0_dim
```
...
#Adding Title
  title("Diameter of var0*", sub = "Source: from authors ", xlab="created by igraph 0")
#Adding Stats
...
node_list <- get.data.frame(var0, what = "vertices")

#Writing Node CSV 
write.csv2(node_list, "~/SNArRDJF/Banco Redes R/Networks/node_var0.csv", fileEncoding = "macintosh")

# Edge List
edge_list <- get.data.frame(var0, what = "edges") %>%
...
write.csv2(edge_list, "~/SNArRDJF/Banco Redes R/Networks/edge_var0.csv", fileEncoding = "macintosh")
