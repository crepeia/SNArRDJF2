...
#1. Descriptive Analysis for var12* - Complete Graph - All networks were generated using from this one
...
#Histogram var12 degree
hist(degree(var12, mode = "in", normalized = F), ylab="Frequency", xlab="Degree",  breaks=vcount(var12)/10, main="Histogram of Indegree Nodes - var12*")
hist(degree(var12, mode = "out", normalized = F), ylab="Frequency", xlab="Degree",  breaks=vcount(var12)/10, main="Histogram of Outdegree Nodes - var12*")
hist(degree(var12, mode = "all", normalized = F), ylab="Frequency", xlab="Degree",  breaks=vcount(var12)/10, main="Histogram of All Degree Nodes - var12*")
```  
...
```{r}
V(var12)$strength_var12<- strength(var12, weights=E(var12)$var12)
strength_var12<- strength(var12, weights=E(var12)$var12)

##Stats
summary(strength_var12)
sd(strength_var12)
```
...
#Histogram var12 degree
hist(degree(var12, mode = "in", normalized = F), ylab="Frequency", xlab="Normalized Degree",  breaks=vcount(var12)/10, main="Histogram of Normalized Indegree Nodes - var12*")
hist(degree(var12, mode = "out", normalized = F), ylab="Frequency", xlab="Normalized Degree",  breaks=vcount(var12)/10, main="Histogram of Normalized Indegree Nodes - var12*")
hist(degree(var12, mode = "all", normalized = F), ylab="Frequency", xlab="Normalized Degree",  breaks=vcount(var12)/10, main="Histogram of Normalized All Degree Nodes - var12*")
```

#2.2.1 Centralization Degree
```{r}
V(var12)$centr_degree_var12 <- centralization.degree(var12)$res
centr_degree_var12 <- centralization.degree(var12)

#Centralization
centr_degree_var12$centralization
#Theoretical Max
centr_degree_var12$theoretical_max
```
...
hist(degree.distribution(var12), breaks=vcount(var12)/10, ylab="Frequency", xlab="Degree Distribuition", main="Histogram of Degree Distribuition - var12*")

#Degree Probability Distribution
dd <- degree.distribution(var12, cumulative=T, mode="all")
plot(dd, pch=19, cex=1, col="orange", xlab="Degree", ylab="Cumulative Frequency", main= "Cumulative Frequency of var12* ")
```
...
plot(d[ind], 
     ...
     main="Log-Log Degree Distribution For var12*"
     )
```

#2.5.1 Average Neighbor Degree versus Vertex Degree (log-log scale for var12*)
```{r}
#Average Neighbor Degree versus vertex degree (log-log scale for var12)
a.nn.deg.var12 <- graph.knn(var12,V(var12))$knn

#Saving to igraph object
V(var12)$a.nn.deg.var12 <- a.nn.deg.var12 <- graph.knn(var12,V(var12))$knn
...
plot(degree(var12), 
     ...
     main="Average Neighbor Degree vs Vertex Degree - Log-Log Scale for var12*"
     )
```

#2.5.2 Average Weighted Neighbor Degree versus vertex degree (log-log scale for weighted var12*)
```{r}
#Average Weighted Neighbor Degree versus vertex degree (log-log scale for var12)
a.nn.deg.var12_w <- graph.knn(var12,V(var12), weights=E(var12)$var12)$knn

#Saving to igraph object
V(var12)$a.nn.deg.var12_w <-a.nn.deg.var12 <- graph.knn(var12,V(var12), weights=E(var12)$var12)$knn

#Average Weighted Neighbor Descriptive
summary(a.nn.deg.var12_w)
sd(a.nn.deg.var12_w)
...
plot(degree(var12), 
     ...
     xlab=c("Log Vertex Degree"),
     ylab=c("Log Average Neighbor Degree"),
     main="Average Weighted Neighbor Degree vs Vertex Degree - Log-Log Scale For Weighted var12*"
     )
```
...
#Adding Title
  title("Network Vertex Degree Sized - var12*", sub = "Source: from authors ")
...
```{r}
#Get Variable
V(var12)$var12_color_degree<-V(var12)$totaldegree
...
#Adding Title
  title("Network Vertex Degree Sized and Colored - var12*", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
   sprintf("Median In Degree: %.2f\nMedian Out Degree: %.2f",
     median(degree(var12, mode="in")), 
     median(degree(var12, mode="out"))
...
```{r}
#Get Variable
V(var12)$var12_color_degree<-V(var12)$centr_degree_var12
...
plot(small_var12, 
     ...
     vertex.frame.color="#ffffff",
     edge.arrow.size=0.1,
     vertex.label.cex=log(degree(small_var12))/10,
...
#Adding Title
  title("Network Higher Than Median Degree - var12*", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
   sprintf("Median In Degree: %.2f\nMedian Out Degree: %.2f",
...
plot(small_var12, 
     ...
     vertex.label.cex=log(degree(small_var12)+2)/2,
...
#Adding Title
  title("Network Lower than Median Degree - var12*", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
   sprintf("Median In Degree: %.2f\nMedian Out Degree: %.2f",
     median(degree(small_var12, mode="in")), 
     median(degree(small_var12, mode="out"))
...
```{r}
#Plotting based only on degree measures  #a.nn.deg.var12_w 
plot(var12, 
     layout=layout.kamada.kawai(var12), 
     vertex.label=get.vertex.attribute(var12,"LABEL_COR"),
     ...
     vertex.frame.color="#ffffff",
     edge.arrow.size=0.1,
     vertex.label.cex=sqrt(graph.knn(var12,V(var12))$knn)/20,
     edge.width=(edge.betweenness(var12, weights =E(var12)$var12))/500,
     edge.curved = TRUE
     )
...
#Adding Title
  title("Network Average Neighbor Degree Sized - var12*", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
...
plot(var12, 
     layout=layout.kamada.kawai(var12), 
     vertex.label=get.vertex.attribute(var12,"LABEL_COR"),
     vertex.size=sqrt(graph.knn(var12,V(var12), weights= E(var12)$var12)$knn),
     vertex.frame.color="#ffffff",
     edge.arrow.size=0.1,
     vertex.label.cex=log(graph.knn(var12,V(var12), weights= E(var12)$var12)$knn)/10,
     edge.width=edge.betweenness(var12, weights =E(var12)$var12)/500,
     edge.curved = TRUE
     )
...
#Adding Title
  title("Network Average Weighted Neighbor Degree Sized - var12*", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
   sprintf("Median Average Weighte Neighbor Degree: %.2f",
     median(graph.knn(var12,V(var12), weights= E(var12)$var12)$knn)
   ))
```
...
#Saving to Igraph object
V(var12)$incloseness_w <- closeness(var12, mode = "in", normalized = T, weights=E(var12)$var12) #%>% round(6)
V(var12)$outcloseness_w <- closeness(var12, mode = "out", normalized = T,weights=E(var12)$var12) #%>% round(6)
V(var12)$totalcloseness_w <- closeness(var12, mode = "total", normalized = T, weights=E(var12)$var12) #%>% round(6)

#Saving to Environment
var12_incloseness_w<- closeness(var12, mode = "in", normalized = T, weights=E(var12)$var12) #%>% round(6)
var12_outcloseness_w<- closeness(var12, mode = "out", normalized = T, weights=E(var12)$var12) #%>% round(6)
var12_totalcloseness_w<- closeness(var12, mode = "total", normalized = T, weights=E(var12)$var12) #%>% round(6)
...
```{r}
V(var12)$closeness_stats_var12_w <- closeness(var12, vids = V(var12), mode = "all", weights =V(var12)$var12, normalized = FALSE)

closeness_stats_var12_w <- closeness(var12, vids = V(var12), mode = "all", weights =V(var12)$var12, normalized = FALSE)
```

#2.7.5 Centralization Closseness
```{r}
V(var12)$centr_closeness_var12 <- centralization.closeness(var12)$res
centr_closeness_var12 <- centralization.closeness(var12)

#Centralization
centr_closeness_var12$centralization
#Theoretical Max
centr_closeness_var12$theoretical_max
```
...
#Adding Title
  title("Network Closeness Degree Sized and Colored - var12*", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
...
```{r}
#Get Variable
V(var12)$var12_color_degree<-round(V(var12)$centr_closeness_var12,3)
...
#Adding Title
  title("Network Centralization Closeness - var12*", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
   sprintf("Median In Closennes: %.4f\nMedian Out Closennes: %.4f",
     median(closeness(var12, mode="in")), 
     median(closeness(var12, mode="out"))
...
```{r}
V(var12)$betweenness_var12_w <- betweenness(var12, weights=E(var12)$var12) %>% round(1)

betweenness_var12_w <- betweenness(var12, weights=E(var12)$var12) %>% round(1)

##all
summary(betweenness_var12_w)
sd(betweenness_var12_w)
```

#2.8.3 Centralization Betweenness
```{r}
V(var12)$centr_betweenness_var12 <- centralization.betweenness(var12)$res
centr_betweenness_var12 <- centralization.betweenness(var12)

#Centralization
centr_betweenness_var12$centralization
#Theoretical Max
centr_betweenness_var12$theoretical_max
```
...
plot(var12, 
     ...
     edge.width=log(edge.betweenness(var12, weights =E(var12)$var12))/50,
     edge.curved = TRUE
     )
...
#Adding Title
  title("Network Vertex Betweenness Sized - var12*", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
...
```{r}
#Get Variable
V(var12)$var12_color_degree<-round(V(var12)$centr_betweenness_var12,3)
...
#Adding Title
  title("Network Centralization Betweenness - var12*", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
...
```{r}
#Eigen_Centrality 
V(var12)$eigenvector <- eigen_centrality(var12, directed = TRUE, weights = V(var12)$var12) %$% vector %>% round(3)

var12_eigenvector <- eigen_centrality(var12, directed = TRUE, weights = V(var12)$var12) %$% vector %>% round(3)

#Eigenvalue Calculated Eigenvector
var12_eigenvector_value <- eigen_centrality(var12, directed = TRUE, weights = V(var12)$var12) %$% value %>% round(3) # The eigenvalue corresponding to the calculated eigenvector, i.e. the centrality scores.
```
...
```{r}
V(var12)$centr_eigen_var12 <- centr_eigen(var12, directed = T)$vector
centr_eigen_var12 <-centr_eigen(var12, directed = T)$vector
#Centralization
...
plot(var12, 
     layout=layout.kamada.kawai(var12), 
     vertex.label=get.vertex.attribute(var12,"LABEL_COR"),
     vertex.size=(eigen_centrality(var12)$vector)*20,
     vertex.frame.color="#ffffff",
     edge.arrow.size=0.1,
     vertex.label.cex=sqrt(eigen_centrality(var12)$vector)/2,
     edge.width=edge.betweenness(var12, weights =E(var12)$var12)/500,
     edge.curved = TRUE
     )
...
#Adding Title
  title("Eigenvector Centrality Measure - var12*", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
...
plot(var12, 
     ...
     vertex.frame.color="#ffffff",
     edge.arrow.size=0.1,
     vertex.label.cex=(centr_eigen(var12, directed = T)$vector + 1)/10,
     edge.width=(edge.betweenness(var12, weights =E(var12)$var12))/500,
     edge.curved = TRUE
     )
...
power_centrality_stats_var12 <- power_centrality(var12, nodes = V(var12), loops = FALSE, exponent = 1, rescale = FALSE, tol = 1e-07, sparse = TRUE)

#Saving power as an igraph attributte
V(var12)$power_centrality_stats_var12 <-power_centrality(var12, nodes = V(var12), loops = FALSE, exponent = 1, rescale = FALSE, tol = 1e-07, sparse = TRUE) 

#Or are you more powerful if you're connected to weak, dependent people? (negative attenuation factor)
i=sort(power_centrality_stats_var12) %>% round(3)
table_i<-cbind(V(var12)$V5_LABEL_LABEL,i)
...
d=sort(power_centrality_stats_var12, decreasing = T) %>% round(3)
table_d<-cbind(V(var12)$V5_LABEL_LABEL,d)
...
V(var12)$power_centrality_stats_var12_r<-round(V(var12)$power_centrality_stats_var12,0)

# Creating brewer pallette
vertex_col_spower <- colorRampPalette(brewer.pal(length(unique(V(var12)$power_centrality_stats_var12_r)), "Blues"))(length(unique(V(var12)$power_centrality_stats_var12_r))) 

#Saving as Vertex properties                                   
V(var12)$color_spower <- vertex_col_spower[factor(V(var12)$power_centrality_stats_var12_r)]  
```
...
#Solving Problems with legend rendering 
a<-V(var12)$power_centrality_stats_var12_r
...
#Adding Title
  title("Bonacich Power Centrality Scores of Network Positions - var12*", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
   sprintf("Median Power Degree: %.2f\n SD Power Degree:%.2f",
     median(power_centrality_stats_var12),sd(power_centrality_stats_var12)
     ))
```
...
```{r, results="asis"}
#Saving power as an object
alpha_centrality_stats_var12 <- alpha_centrality(var12, loops = FALSE, alpha = 1, exo = 1, weights = V(var12)$var12, tol = 1e-07, sparse = TRUE)

#Saving power as an igraph attributte
V(var12)$alpha_centrality_stats_var12<-alpha_centrality(var12, loops = FALSE, alpha = 1, exo = 1, weights = V(var12)$var12, tol = 1e-07, sparse = TRUE)

#Descriptive 
summary(alpha_centrality_stats_var12)
sd(alpha_centrality_stats_var12)

#Or are you more powerful if you're connected to weak, dependent people? (negative attenuation factor)
i=sort(alpha_centrality_stats_var12) %>% round(3)
table_i<-cbind(V(var12)$V5_LABEL_LABEL,i)
...
#Are you more powerful if you're connected to powerful people? (positive attenuation factor)
d=sort(alpha_centrality_stats_var12, decreasing = T) %>% round(3)
table_d<-cbind(V(var12)$V5_LABEL_LABEL,d)
colnames(table_d) <- c("ID", "Power")
table_d10<-table_d[1:10,]
...
```{r}
# Set colors to plot distances:
V(var12)$alpha_centrality_stats_var12_r<-round(V(var12)$alpha_centrality_stats_var12,0)

# Creating brewer pallette
vertex_col_s <- colorRampPalette(brewer.pal(length(unique(V(var12)$alpha_centrality_stats_var12_r)), "YlOrRd"))(length(unique(V(var12)$alpha_centrality_stats_var12_r))) 

#Saving as Vertex properties                                                                  
V(var12)$color_s <- vertex_col_s[factor(V(var12)$alpha_centrality_stats_var12_r)]  
```
...
plot(var12, 
     ...
     vertex.label.color="black",
     vertex.frame.color="#ffffff",
     vertex.size=sqrt((alpha_centrality(var12, weights = V(var12)$var12)^2)),
     edge.arrow.size=0.1,
...
#Solving Problems with legend rendering 
a<-V(var12)$alpha_centrality_stats_var12_r
b<-V(var12)$color_s
c<-table(a,b)
d<-as.data.frame(c)
e<-subset(d, d$Freq>0)
...
#Adding Title
  title("Bonacich's Alpha Centrality Scores of Network Positions - var12*", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
   sprintf("Median Alpha Degree: %.2f\n SD Alpha Degree:%.2f",
     median(alpha_centrality_stats_var12),sd(alpha_centrality_stats_var12)
     ))
```
...
var12_hub<- hub.score(var12)$vector
...
plot(var12, 
     layout=layout.kamada.kawai(var12), 
     vertex.label=get.vertex.attribute(var12,"LABEL_COR"),
     vertex.size=10*sqrt(hub.score(var12)$vector),
     edge.arrow.size=0.1,
     vertex.label.cex=log(degree(var12))/10,
     edge.width=log(edge.betweenness(var12, weights =E(var12)$var12))/5,
     edge.curved = TRUE)
...
#Adding Title
  title("Network Hub Sized - var12*", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
...
var12_auth <- authority.score(var12)$vector
#stats
summary(var12_auth)
sd(var12_auth)
...
plot(var12, 
     layout=layout.kamada.kawai(var12), 
     vertex.label=get.vertex.attribute(var12,"LABEL_COR"),
     vertex.size=10*sqrt(authority.score(var12)$vector),
     edge.arrow.size=0.1,
     vertex.label.cex=log(degree(var12))/10,
     edge.width=log(edge.betweenness(var12, weights =E(var12)$var12))/5,
     edge.curved = TRUE)
...
#Adding Title
  title("Network Authorities Scores  Sized - var12*", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
...
var12_pr<-page.rank(var12)$vector
#stats
summary(var12_pr)
sd(var12_pr)
...
plot(var12, 
     layout=layout.kamada.kawai(var12), 
     vertex.label=get.vertex.attribute(var12,"LABEL_COR"),
     vertex.size=30*sqrt(page.rank(var12)$vecto),
     edge.arrow.size=0.1,
     vertex.label.cex=log(degree(var12))/10,
     edge.width=log(edge.betweenness(var12, weights =E(var12)$var12))/100,
     edge.curved = TRUE)
...
#Adding Title
  title("Network Google Page Rank Sized - var12*", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
...
```{r}
#For in-degrees:
indegnamesPB=which(indegPB>sort(indegPB)[vcount(var12)-5]) #For out- degrees:
outdegnamesPB=which(outdegPB>sort(outdegPB)[vcount(var12)-5]) #For PageRank:
prnamesPB=which(prPB>sort(prPB)[vcount(var12)-5]) #For Hubs:
hnamesPB=which(hPB>sort(hPB)[vcount(var12)-5]) #For Authorities:
authnamesPB=which(authPB>sort(authPB)[vcount(var12)-5])
...
plot(make_ego_graph_all_var12, 
     ...
     edge.arrow.size=0.1,
     vertex.label.cex=log(degree(make_ego_graph_all_var12))/10,
...
#Adding Title
  title("Ego Network Page Rank Sized - All connections from CAPSAD - var12*", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
   sprintf("Mean Page Rank Scores: %.2f\n SD Page Rank Scores: %.2f",
...
plot(make_ego_graph_out_var12, 
     ...
     vertex.size=30*sqrt(page.rank(make_ego_graph_out_var12)$vector),
     edge.arrow.size=0.1,
     vertex.label.cex=log(degree(make_ego_graph_out_var12))/10,
...
#Adding Title
  title("Ego Network Page Rank Sized - Out Connections from CAPSAD - var12*", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
   sprintf("Mean Page Rank Scores: %.2f\n SD Page Rank Scores: %.2f",
...
plot(make_ego_graph_in_var12, 
     ...
     vertex.size=30*sqrt(page.rank(make_ego_graph_in_var12)$vector),
     edge.arrow.size=0.1,
...
#Adding Title
  title("Ego Network CAPS AD Page Rank Sized - In connections from CAPS AD - var12*", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
   sprintf("Mean Page Rank Scores: %.2f\n SD Page Rank Scores: %.2f",
...
plot(make_ego_graph_all_var12_without_CAPSAD, 
     ...
     vertex.size=30*sqrt(page.rank(make_ego_graph_all_var12_without_CAPSAD)$vector),
     edge.arrow.size=0.1,
     vertex.label.cex=log(degree(make_ego_graph_all_var12_without_CAPSAD))/10,
     edge.width=log(edge.betweenness(make_ego_graph_all_var12_without_CAPSAD))/10,
     edge.curved = TRUE)
...
#Adding Title
  title("Ego Network CAPS AD Page Rank Sized - All Connections Removing CAPS AD - var12*", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
   sprintf("Mean Page Rank Scores: %.2f\n SD Page Rank Scores: %.2f",
...
plot(make_ego_graph_all_var12_without_complex, 
     ...
     edge.arrow.size=0.1,
     vertex.label.cex=log(degree(make_ego_graph_all_var12_without_complex))/10,
     edge.width=log(edge.betweenness(make_ego_graph_all_var12_without_complex))/10,
     edge.curved = TRUE)
...
#Adding Title
  title("Ego Network CAPS AD Page Rank Sized - All Connections Removing High Complex Service - var12*", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
   sprintf("Mean Page Rank Scores: %.2f\n SD Page Rank Scores: %.2f",
...
#Adding Title
  title("Minimum Spanning Tree Non-normalized - from var12*", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
   sprintf("Median In Degree: %.2f\nMedian Out Degree: %.2f",
...
msp_var12_w<-minimum.spanning.tree(var12, weights=1/edge.betweenness(var12, weights =E(var12)$var12), algorithm='prim')
...
#Plotting based only on degree measures 
plot(msp_var12_w, 
     layout=layout.kamada.kawai(msp_var12_w), 
     ...
     edge.width=1/edge.betweenness(msp_var12_w, weights =E(msp_var12_w)$msp_var12_w),
     edge.curved = TRUE
     )
...
#Adding Title
  title("Minimum Weighted Spanning Tree (Edge Betweenness) - var12*", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
   sprintf("Median In Degree: %.2f\nMedian Out Degree: %.2f",
...
is.connected(var12) #trick information - let's check this out further
```

#3.3.1 Strong Connected Components
```{r}
#Strong Connected
components_strong_var12<-components(var12, mode="strong")
V(var12)$components_strong_var12_membership<-components_strong_var12$membership # adding to igraph object

#Number of clusters
count_components(var12)

#Display components structure
components_strong_var12
```
...
plot(var12, 
     ...
     mark.groups = by(seq_along(components_strong_var12$membership),
                      components_strong_var12$membership, 
...
#Adding Title
  title("Network Vertex Degree Sized - Classfied by Components Structure - var12*", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
   sprintf("Median In Degree: %.2f\nMedian Out Degree: %.2f",
     median(degree(var12, mode="in")), 
     median(degree(var12, mode="out"))
...
```{r}
#Shortest Paths
var12_sp_in <- shortest.paths(var12, mode='in', weights=V(var12)$var12) # sames as bellow
var12_sp_out <- shortest.paths(var12, mode='out', weights=V(var12)$var12) #same as above
var12_sp_all <- shortest.paths(var12, mode='all', weights=V(var12)$var12) # all
...
```{r}
#All shortest paths 
distances_sp_all_var12<-distances(var12, mode="all", weights=E(var12)$var12)
#distances_sp_all_var12

#In shortest paths 
distances_sp_in_var12<-distances(var12, mode="in",weights=E(var12)$var12)
#distances_sp_in_var12

#Out shortest paths 
distances_sp_out_var12<-distances(var12, mode="out", weights=E(var12)$var12)
#distances_sp_out_var12
```

#3.7.4 Histogram from shortest path length between each pair of vertices. 
```{r}
distance_table(var12, directed = TRUE)
```
...
var12_dist.from.CAPSAD <- distances(var12, v=V(var12)[V2_LABEL_ID=="q170_CAPS...CAPS.AD"], to=V(var12), weights=V(var12)$var12)
...
#Adding Title
  title("Distance from CAPS AD - var12*", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
...
cliques_var12_u_size<-sapply(cliques(var12_u), length) # clique sizes
largest_cliques(var12_u) # cliques with max number of nodes
```
...
V(as.undirected_var12_u)$vcollabel <- recode(V(as.undirected_var12_u)$vcol, "'gold' = 'Largest Clique'; else= 'Others'")
...
plot(as.undirected(var12_u), 
     ...
     edge.arrow.size=0.1,
     vertex.label.cex=log(degree(as.undirected(var12_u)))/10,
     edge.width=log(edge.betweenness(as.undirected(var12_u), weights =E(as.undirected(var12_u))$var12))/5,
     edge.curved = TRUE
     )
...
```{r}
#Edge Betweenness
E(var12)$edge.btweenness<-edge.betweenness(var12, weights =E(var12)$var12)
var12_edge.betweenness <-edge.betweenness(var12, weights =E(var12)$var12)
...
E(var12)[order(var12_edge.betweenness, decreasing=T)[1:10]]
```

#3.10.1 Community detection (using edge betweenness)
```{r}
## Community structure detection based on edge betweenness
communityEdgeBetwn_var12 <- edge.betweenness.community(var12)
var12_edge.betweenness.community <- edge.betweenness.community(var12)

#Creating communities
V(var12)$edge.betweenness.community <- membership(var12_edge.betweenness.community)
var12_edge.betweenness.community <- membership(var12_edge.betweenness.community)
```
...
plot(comps_var121, 
     ...
     edge.arrow.size=0.1,
     vertex.label.cex=log(degree(comps_var121))/10,
...
#Non-weighted 
transitivity(var12, weights=E(var12)$var12, type="global")

#Weighted
transitivity(var12, weights=E(var12)$var12,type="weighted")
transitivity(var12, weights=E(var12)$var12,type="global")
...
transitivity_local_w<-transitivity(var12, weights=E(var12)$var12, type="barrat")
...
#Adding Title
  title("Diameter of var12*", sub = "Source: from authors ", xlab="created by igraph 0")
#Adding Stats
...
node_list <- get.data.frame(var12, what = "vertices")

#Writing Node CSV 
write.csv2(node_list, "~/SNArRDJF/Banco Redes R/Networks/node_var12.csv", fileEncoding = "macintosh")

# Edge List
edge_list <- get.data.frame(var12, what = "edges") %>%
...
write.csv2(edge_list, "~/SNArRDJF/Banco Redes R/Networks/edge_var12.csv", fileEncoding = "macintosh")
