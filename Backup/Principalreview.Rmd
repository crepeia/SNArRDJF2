...
#Importing Squared Matrix - adding complete sentence for each question
```{r, message=FALSE, warning=FALSE, include=FALSE}
...
for (i in names(full_data_frame)){
  
  full_data_frame_id[[i]][(full_data_frame_idfrom]]==full_data_frame_id[[to]])!=0]<-0
}

full_data_frame_id$var0[(full_data_frame_id$from==full_data_frame_id$to)!=0]
...
for (i in names(attributes.csv))
V(full_no_zero)$i=as.character(attributes.csv$i[match(V(full_no_zero)$name, attributes.csv$V3_LABEL_ID2)])
...
#clique size
sapply(mc, length)
col <- rep("blue", length(V(acesso)))
#plot the largest (last) click
col[mc[[length(mc)]]] <- "red"
plot(acesso, layout=layout, vertex.color=col)
