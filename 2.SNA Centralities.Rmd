---
title: "Untitled"
author: "Leonardo Martins"
date: "17 de julho de 2016"
output: html_document
---


####IMPORTANTE Checar q167 toxicomanos  q111 deville  q125 rendição IMPORTANTE######

#1.24. Loading objects generated with 1.Principal.Rmd Script - Please run this script only after run 1.Principal.Rmd
```{r}
load("~/SNArRDJF/1.Principal.RData")
```

##1.25 Reload packages
```{r packages, message=TRUE, warning=TRUE}
require(igraph) #load package for social network analysis
library(dplyr)
library(RColorBrewer)
library(car)
library(xtable)
#library(ggplot2)
library(xtable)
set.seed(123)
```


#1. Descriptive Analysis for full_no_zero_fancy - Complete Graph - All networks were generated using from this one

###1.1 Vertices and Edges Number
```{r}
#1. Vertices and Edges Number
full_no_zero_vcount<-vcount(full_no_zero)
full_no_zero_ecount<-ecount(full_no_zero)
```
###1.2 Vertice Number
```{r}
vcount(full_no_zero)
```
###1.3 Edge Number
```{r}
ecount(full_no_zero)
```

#2. Centrality Measures

• Undirected graphs:
– Actor centrality - involvement (connections) with other actors

• Directed graphs:
– Actor centrality - source of the ties (outgoing edges)
– Actor prestige - recipient of many ties (incoming edges)

#2. Centrality Degree (number of ties/nearest neighbors).
High centrality degree - direct contact with many other actors
```{r}
#Saving in igrpah object
V(full_no_zero)$indegree<-degree(full_no_zero, mode = "in") # Actor prestige - recipient of many ties (incoming edges)
V(full_no_zero)$outdegree <- degree(full_no_zero, mode = "out") # Actor centrality - source of the ties (outgoing edges)
V(full_no_zero)$totaldegree <- degree(full_no_zero, mode = "total")

#Saving in Global Environment as an object
full_no_zero_indegree<-degree(full_no_zero, mode = "in")
full_no_zero_outdegree<-degree(full_no_zero, mode = "out")
full_no_zero_totaldegree<-degree(full_no_zero, mode = "total")
```

#2.1 Centrality Degree Descriptive Statistics - non-normalized
```{r}
# Centrality Degree Descriptive Statistics
##in
summary(full_no_zero_indegree)
sd(full_no_zero_indegree)
##out
summary(full_no_zero_outdegree)
sd(full_no_zero_outdegree)
##all
summary(full_no_zero_totaldegree)
sd(full_no_zero_totaldegree)

#Histogram full_no_zero degree
hist(degree(full_no_zero, mode = "in", normalized = F), ylab="Frequency", xlab="Degree",  breaks=vcount(full_no_zero)/10, main="Histogram of Indegree Nodes - full_no_zero_fancy")
hist(degree(full_no_zero, mode = "out", normalized = F), ylab="Frequency", xlab="Degree",  breaks=vcount(full_no_zero)/10, main="Histogram of Outdegree Nodes - full_no_zero_fancy")
hist(degree(full_no_zero, mode = "all", normalized = F), ylab="Frequency", xlab="Degree",  breaks=vcount(full_no_zero)/10, main="Histogram of All Degree Nodes - full_no_zero_fancy")
```  

A slightly more nuanced metric is “strength centrality”, which is defined as the sum of the weights of all the connections for a given node. This is also sometimes called “weighted degree centrality”.

#2.1.1 Compute strength - weighted
```{r}
V(full_no_zero)$strength_full_no_zero<- strength(full_no_zero, weights=E(full_no_zero)$full_no_zero)
strength_full_no_zero<- strength(full_no_zero, weights=E(full_no_zero)$full_no_zero)

##Stats
summary(strength_full_no_zero)
sd(strength_full_no_zero)
```

#2.2 Centrality Degree Normalized
```{r}
#Centrality Degree Normalized saving igraph object
V(full_no_zero)$indegree_n<-degree(full_no_zero, mode = "in", normalized = T)
V(full_no_zero)$outdegree_n<- degree(full_no_zero, mode = "out", normalized = T)
V(full_no_zero)$totaldegree_n<- degree(full_no_zero, mode = "total", normalized = T)

#Saving in Global Environment as an object
full_no_zero_indegree_n<-degree(full_no_zero, mode = "in", normalized = T)
full_no_zero_outdegree_n<-degree(full_no_zero, mode = "out", normalized = T)
full_no_zero_totaldegree_n<-degree(full_no_zero, mode = "total", normalized = T)

# Centrality Degree Normalized Descriptive Statistics

##in
summary(full_no_zero_indegree_n)
sd(full_no_zero_indegree_n)
##out
summary(full_no_zero_outdegree_n)
sd(full_no_zero_outdegree_n)
##all
summary(full_no_zero_totaldegree_n)
sd(full_no_zero_totaldegree_n)

#Histogram full_no_zero degree
hist(degree(full_no_zero, mode = "in", normalized = F), ylab="Frequency", xlab="Normalized Degree",  breaks=vcount(full_no_zero)/10, main="Histogram of Normalized Indegree Nodes - full_no_zero_fancy")
hist(degree(full_no_zero, mode = "out", normalized = F), ylab="Frequency", xlab="Normalized Degree",  breaks=vcount(full_no_zero)/10, main="Histogram of Normalized Indegree Nodes - full_no_zero_fancy")
hist(degree(full_no_zero, mode = "all", normalized = F), ylab="Frequency", xlab="Normalized Degree",  breaks=vcount(full_no_zero)/10, main="Histogram of Normalized All Degree Nodes - full_no_zero_fancy")
```

#2.2.1 Centralization Degree
```{r}
V(full_no_zero)$centr_degree_full_no_zero <- centralization.degree(full_no_zero)$res
centr_degree_full_no_zero <- centralization.degree(full_no_zero)

#Centralization
centr_degree_full_no_zero$centralization
#Theoretical Max
centr_degree_full_no_zero$theoretical_max
```

##2.3 Degree distribution considering total equal one
```{r}
## Degree distribution when the totality is considered 1
full_no_zero_degree.distribution<-degree.distribution(full_no_zero)

## Degree distribution Descriptive Stats
summary(full_no_zero_degree.distribution)
sd(full_no_zero_degree.distribution)

#Histogram full_no_zero distribution degree
hist(degree.distribution(full_no_zero), breaks=vcount(full_no_zero)/10, ylab="Frequency", xlab="Degree Distribuition", main="Histogram of Degree Distribuition - full_no_zero_fancy")

#Degree Probability Distribution
dd <- degree.distribution(full_no_zero, cumulative=T, mode="all")
plot(dd, pch=19, cex=1, col="orange", xlab="Degree", ylab="Cumulative Frequency", main= "Cumulative Frequency of full_no_zero_fancy ")
```

#2.4 Log-Log Degree Distribution - Scale Free Network - Does it fit to power law
```{r}
#Log-Log Degree Distribution - Scale Free Network - Does it fit to power law? 
dd.full_no_zero <- degree.distribution(full_no_zero)
d <- 1:max(degree(full_no_zero))-1
ind <- (dd.full_no_zero != 0)

plot(d[ind], 
     dd.full_no_zero[ind], 
     log="xy", 
     col="blue",
     xlab=c("Log-Degree"), 
     ylab=c("Log-Intensity"),
     main="Log-Log Degree Distribution For full_no_zero_fancy"
     )
```

#2.5.1 Average Neighbor Degree versus Vertex Degree (log-log scale for full_no_zero_fancy)
```{r}
#Average Neighbor Degree versus vertex degree (log-log scale for full_no_zero)
a.nn.deg.full_no_zero <- graph.knn(full_no_zero,V(full_no_zero))$knn

#Saving to igraph object
V(full_no_zero)$a.nn.deg.full_no_zero <- a.nn.deg.full_no_zero <- graph.knn(full_no_zero,V(full_no_zero))$knn

#Plotting Average Neighbor Degree versus vertex degree
plot(degree(full_no_zero), 
     a.nn.deg.full_no_zero, 
     log="xy", 
     col="goldenrod", 
     xlab=c("Log Vertex Degree"),
     ylab=c("Log Average Neighbor Degree"),
     main="Average Neighbor Degree vs Vertex Degree - Log-Log Scale for full_no_zero_fancy"
     )
```

#2.5.2 Average Weighted Neighbor Degree versus vertex degree (log-log scale for weighted full_no_zero_fancy)
```{r}
#Average Weighted Neighbor Degree versus vertex degree (log-log scale for full_no_zero)
a.nn.deg.full_no_zero_w <- graph.knn(full_no_zero,V(full_no_zero), weights=E(full_no_zero)$full_no_zero)$knn

#Saving to igraph object
V(full_no_zero)$a.nn.deg.full_no_zero_w <-a.nn.deg.full_no_zero <- graph.knn(full_no_zero,V(full_no_zero), weights=E(full_no_zero)$full_no_zero)$knn

#Average Weighted Neighbor Descriptive
summary(a.nn.deg.full_no_zero_w)
sd(a.nn.deg.full_no_zero_w)

#Plotting Average Neighbor Degree versus vertex degree
plot(degree(full_no_zero), 
     a.nn.deg.full_no_zero, 
     log="xy", 
     col="goldenrod", 
     xlab=c("Log Vertex Degree"),
     ylab=c("Log Average Neighbor Degree"),
     main="Average Weighted Neighbor Degree vs Vertex Degree - Log-Log Scale For Weighted full_no_zero_fancy"
     )
```

#2.6.1 Network plotting based only on degree measures 
```{r}
#Plotting based only on degree measures 
plot(full_no_zero, 
     layout=layout.kamada.kawai(full_no_zero), 
     vertex.label=get.vertex.attribute(full_no_zero,"LABEL_COR"),
     vertex.size=degree(full_no_zero)/10,
     vertex.frame.color="#ffffff",
     edge.arrow.size=degree(full_no_zero)/10000,
     vertex.label.cex=degree(full_no_zero)/500,
     edge.width=(edge.betweenness(full_no_zero, weights =E(full_no_zero)$full_no_zero))/500,
     edge.curved = TRUE
     )

#Solving Problems with legend rendering 
a<-V(full_no_zero)$LABEL_COR
b<-V(full_no_zero)$color
c<-table(a,b)
d<-as.data.frame(c)
e<-subset(d, d$Freq>0)
f<-t(e$a)
g<-t(e$b)

#Adding Legend
legend(x=1, y=1.25,
       legend=as.character(f),
       pch=21,
       col = "#777777", 
       pt.bg=as.character(g),
       pt.cex=4,
       bty="n", 
       ncol=1,
       lty=1,
       cex = .8)

#Adding Title
  title("Network Vertex Degree Sized - full_no_zero_fancy", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
   sprintf("Median In Degree: %.2f\nMedian Out Degree: %.2f",
     median(degree(full_no_zero, mode="in")), 
     median(degree(full_no_zero, mode="out"))
   ))
```

#2.6.2 Network plotting based only on degree measures 
```{r}
#Get Variable
V(full_no_zero)$full_no_zero_color_degree<-V(full_no_zero)$totaldegree

#Creating brewer pallette
vertex_full_no_zero_color_degree<-
  colorRampPalette(brewer.pal(length(unique(
          V(full_no_zero)$full_no_zero_color_degree)), "RdBu"))(
            length(unique(V(full_no_zero)$full_no_zero_color_degree)))

#Saving as Vertex properties 
V(full_no_zero)$vertex_full_no_zero_color_degree<- vertex_full_no_zero_color_degree[as.numeric(cut(degree(full_no_zero),breaks =length(unique(V(full_no_zero)$full_no_zero_color_degree))))]
   
#Plotting
plot(full_no_zero, 
     layout=layout.kamada.kawai(full_no_zero),
     vertex.color=V(full_no_zero)$vertex_full_no_zero_color_degree,
     vertex.label=get.vertex.attribute(full_no_zero,"LABEL_COR"),
     vertex.size=degree(full_no_zero)/10,
     vertex.frame.color="#ffffff",
     edge.arrow.size=0.1,
     vertex.label.cex=(degree(full_no_zero))/800,
     edge.width=(edge.betweenness(full_no_zero, weights =E(full_no_zero)$full_no_zero))/500,
     edge.curved = TRUE
     )

#Solving Problems with legend rendering 
a<-V(full_no_zero)$full_no_zero_color_degree
b<-V(full_no_zero)$vertex_full_no_zero_color_degree
c<-table(a,b)
d<-as.data.frame(c)
e<-subset(d, d$Freq>0)
e<-e[order(e$a,decreasing=T),] 
f<-t(e$a)
g<-t(e$b)

#Adding Legend
legend(x=1, y=1.25,
       legend=as.character(f),
       pch=21,
       col = "#777777", 
       pt.bg=as.character(g),
       pt.cex=4,
       bty="n", 
       ncol=1,
       lty=1,
       cex = .3)

#Adding Title
  title("Network Vertex Degree Sized and Colored - full_no_zero_fancy", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
   sprintf("Median In Degree: %.2f\nMedian Out Degree: %.2f",
     median(degree(full_no_zero, mode="in")), 
     median(degree(full_no_zero, mode="out"))
   ))
```

#2.6.3 Network Plotting Centralization - Degree Measures 
```{r}
#Get Variable
V(full_no_zero)$full_no_zero_color_degree<-V(full_no_zero)$centr_degree_full_no_zero

#Creating brewer pallette
vertex_full_no_zero_color_degree<-
  colorRampPalette(brewer.pal(length(unique(
          V(full_no_zero)$full_no_zero_color_degree)), "Spectral"))(
            length(unique(V(full_no_zero)$full_no_zero_color_degree)))

#Saving as Vertex properties 
V(full_no_zero)$vertex_full_no_zero_color_degree<- vertex_full_no_zero_color_degree[as.numeric(cut(V(full_no_zero)$full_no_zero_color_degree,breaks =length(unique(V(full_no_zero)$full_no_zero_color_degree))))]
   
#Plotting
plot(full_no_zero, 
     #layout=layout.kamada.kawai(full_no_zero),
     vertex.color=V(full_no_zero)$vertex_full_no_zero_color_degree,
     vertex.label=get.vertex.attribute(full_no_zero,"LABEL_COR"),
     vertex.size=degree(full_no_zero)/10,
     vertex.frame.color="#ffffff",
     edge.arrow.size=0.1,
     vertex.label.cex=(degree(full_no_zero))/800,
     edge.width=(edge.betweenness(full_no_zero, weights =E(full_no_zero)$full_no_zero))/500,
     edge.curved = TRUE
     )

#Solving Problems with legend rendering 
a<-V(full_no_zero)$full_no_zero_color_degree
b<-V(full_no_zero)$vertex_full_no_zero_color_degree
c<-table(a,b)
d<-as.data.frame(c)
e<-subset(d, d$Freq>0)
e<-e[order(e$a,decreasing=T),] 
f<-t(e$a)
g<-t(e$b)

#Adding Legend
legend(x=1, y=1.25,
       legend=as.character(f),
       pch=21,
       col = "#777777", 
       pt.bg=as.character(g),
       pt.cex=4,
       bty="n", 
       ncol=1,
       lty=1,
       cex = .3)

#Adding Title
  title("Network Vertex Degree Sized and Colored - full_no_zero_fancy", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
   sprintf("Median In Degree: %.2f\nMedian Out Degree: %.2f",
     median(degree(full_no_zero, mode="in")), 
     median(degree(full_no_zero, mode="out"))
   ))
```

#2.6.4 Alternative vizualization using degree in order to genarete sub-graphs - Higher than median degree network
```{r}
# Network elements with lower than meadian degree
lowerthanmedian.network_full_no_zero<-V(full_no_zero)[degree(full_no_zero)<=median(degree(full_no_zero))] 

#Deleting vertices based in intersection betewenn full_no_zero 
small_full_no_zero<-delete.vertices(full_no_zero, lowerthanmedian.network_full_no_zero)

#Plotting 
plot(small_full_no_zero, 
     layout=layout.kamada.kawai(small_full_no_zero), 
     vertex.label=get.vertex.attribute(small_full_no_zero,"LABEL_COR"),
     vertex.size=degree(small_full_no_zero)/10,
     vertex.frame.color="#ffffff",
     edge.arrow.size=0.1,
     vertex.label.cex=log(degree(small_full_no_zero)+1)/10,
     edge.width=(edge.betweenness(small_full_no_zero))/500,
     edge.curved = TRUE
     )

#Solving Problems with legend rendering 
a<-V(small_full_no_zero)$LABEL_COR
b<-V(small_full_no_zero)$color
c<-table(a,b)
d<-as.data.frame(c)
e<-subset(d, d$Freq>0)
f<-t(e$a)
g<-t(e$b)

#Adding Legend
legend(x=1, y=1.25,
       legend=as.character(f),
       pch=21,
       col = "#777777", 
       pt.bg=as.character(g),
       pt.cex=4,
       bty="n", 
       ncol=1,
       lty=1,
       cex = .8)

#Adding Title
  title("Network Higher Than Median Degree - full_no_zero_fancy", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
   sprintf("Median In Degree: %.2f\nMedian Out Degree: %.2f",
     median(degree(small_full_no_zero, mode="in")), 
     median(degree(small_full_no_zero, mode="out"))
   )
  )

```

#2.6.5 Alternative vizualization using degree in order to genarete sub-graphs - Lower than median degree network 
```{r}
# Network elements with lower than meadian degree
lowerthanmedian.network_full_no_zero<-V(full_no_zero)[degree(full_no_zero)>=median(degree(full_no_zero))] 

#Deleting vertices based in intersection betewenn full_no_zero 
small_full_no_zero<-delete.vertices(full_no_zero, lowerthanmedian.network_full_no_zero)

#Plotting 
plot(small_full_no_zero, 
     layout=layout.kamada.kawai(small_full_no_zero), 
     vertex.label=get.vertex.attribute(small_full_no_zero,"LABEL_COR"),
     vertex.size=degree(small_full_no_zero)/2,
     edge.arrow.size=0.1,
     vertex.frame.color="#ffffff",
     vertex.label.degree=(-pi)/2,	
     vertex.label.cex=log(degree(small_full_no_zero)+2)/5,
     edge.width=(edge.betweenness(small_full_no_zero)),
     edge.curved = TRUE
     )

#Solving Problems with legend rendering 
a<-V(small_full_no_zero)$LABEL_COR
b<-V(small_full_no_zero)$color
c<-table(a,b)
d<-as.data.frame(c)
e<-subset(d, d$Freq>0)
f<-t(e$a)
g<-t(e$b)

#Adding Legend
legend(x=1, y=1.25,
       legend=as.character(f),
       pch=21,
       col = "#777777", 
       pt.bg=as.character(g),
       pt.cex=4,
       bty="n", 
       ncol=1,
       lty=1,
       cex = .8)

#Adding Title
  title("Network Lower than Median Degree - full_no_zero_fancy", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
   sprintf("Median In Degree: %.2f\nMedian Out Degree: %.2f",
     median(degree(small_full_no_zero, mode="in")), 
     median(degree(small_full_no_zero, mode="out"))
   )
  )
```

#2.6.6 Plotting using Average Neighbor Degree
```{r}
#Plotting based only on degree measures  #a.nn.deg.full_no_zero_w 
plot(full_no_zero, 
     layout=layout.kamada.kawai(full_no_zero), 
     vertex.label=get.vertex.attribute(full_no_zero,"LABEL_COR"),
     vertex.size=(graph.knn(full_no_zero,V(full_no_zero))$knn)/10,
     vertex.frame.color="#ffffff",
     edge.arrow.size=0.1,
     vertex.label.cex=sqrt(graph.knn(full_no_zero,V(full_no_zero))$knn)/30,
     edge.width=(edge.betweenness(full_no_zero, weights =E(full_no_zero)$full_no_zero))/500,
     edge.curved = TRUE
     )

#Solving Problems with legend rendering 
a<-V(full_no_zero)$LABEL_COR
b<-V(full_no_zero)$color
c<-table(a,b)
d<-as.data.frame(c)
e<-subset(d, d$Freq>0)
f<-t(e$a)
g<-t(e$b)

#Adding Legend
legend(x=1, y=1.25,
       legend=as.character(f),
       pch=21,
       col = "#777777", 
       pt.bg=as.character(g),
       pt.cex=4,
       bty="n", 
       ncol=1,
       lty=1,
       cex = .8)

#Adding Title
  title("Network Average Neighbor Degree Sized - full_no_zero_fancy", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
   sprintf("Median Average Neighbor Degree: %.2f",
     median(graph.knn(full_no_zero,V(full_no_zero))$knn)
   ))
```

#2.6.7 Plotting using Average Weighted Neighbor Degree
```{r}
#Plotting based only on degree measures  a.nn.deg.full_no_zero_w 
plot(full_no_zero, 
     layout=layout.kamada.kawai(full_no_zero), 
     vertex.label=get.vertex.attribute(full_no_zero,"LABEL_COR"),
     vertex.size=sqrt(graph.knn(full_no_zero,V(full_no_zero), weights= E(full_no_zero)$full_no_zero)$knn),
     vertex.frame.color="#ffffff",
     vertex.label.color="black",
     edge.arrow.size=0.1,
     vertex.label.cex=log(graph.knn(full_no_zero,V(full_no_zero), weights= E(full_no_zero)$full_no_zero)$knn)/10,
     edge.width=edge.betweenness(full_no_zero, weights =E(full_no_zero)$full_no_zero)/500,
     edge.curved = TRUE
     )

#Solving Problems with legend rendering 
a<-V(full_no_zero)$LABEL_COR
b<-V(full_no_zero)$color
c<-table(a,b)
d<-as.data.frame(c)
e<-subset(d, d$Freq>0)
f<-t(e$a)
g<-t(e$b)

#Adding Legend
legend(x=1, y=1.25,
       legend=as.character(f),
       pch=21,
       col = "#777777", 
       pt.bg=as.character(g),
       pt.cex=4,
       bty="n", 
       ncol=1,
       lty=1,
       cex = .8)

#Adding Title
  title("Network Average Weighted Neighbor Degree Sized - full_no_zero_fancy", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
   sprintf("Median Average Weighte Neighbor Degree: %.2f",
     median(graph.knn(full_no_zero,V(full_no_zero), weights= E(full_no_zero)$full_no_zero)$knn)
   ))
```

#2.6.8 Circle Degree ***Too intensive computation*** #code
```{r}
#Circle Degree ***Too intense computation***
#A_full_no_zero <- get.adjacency(full_no_zero, sparse=FALSE)
#library(network)
#g <- network::as.network.matrix(A_full_no_zero)
#library(sna)
#sna::gplot.target(g, degree(g), main="Degree",
#circ.lab = FALSE, circ.col="skyblue",
#usearrows = FALSE,
#vertex.col=c("blue", rep("red", 32), "yellow"),
#edge.col="darkgray")
```

#2.7 Closeness - centrality based on distance to others in the graph - How close an actor to all the other actors in network.

High closeness centrality - short communication path to others, minimal number of steps to reach others.

Answers the “Kevin Bacon” question:
“How many steps are required to access every other vertex from a given vertex?”

One practical implication of this metric: it helps you gauge how information might spread within your network, and who might be the best people to leverage if you need to make sure information gets around. (http://www.tc.umn.edu/~alink/R-social-network-analysis.html)

Closeness centrality can be defined as a measure of how far other nodes are from the node in question. Nodes with high closeness centrality are likely to be relatively efficient in receiving or transmitting information to/from distant parts of the social network.

Scores may be interpreted as arising from a reciprocal process in which the centrality of each actor is proportional to the sum of the centralities of those actors to whom he or she is connected. In general, vertices with high eigenvector centralities are those which are connected to many other vertices which are, in turn, connected to many others (and so on). (The perceptive may realize that this implies that the largest values will be obtained by individuals in large cliques (or high-density substructures)

#2.7.1 Closeness Non-normalized
```{r}
#Closeness Non-normalized

#Saving to Igraph object
V(full_no_zero)$incloseness <- closeness(full_no_zero, mode = "in") #%>% round(6)
V(full_no_zero)$outcloseness <- closeness(full_no_zero, mode = "out") #%>% round(6)
V(full_no_zero)$totalcloseness <- closeness(full_no_zero, mode = "total") #%>% round(4)

#Saving to Environment
full_no_zero_incloseness<- closeness(full_no_zero, mode = "in") #%>% round(6)
full_no_zero_outcloseness<- closeness(full_no_zero, mode = "out") #%>% round(6)
full_no_zero_totalcloseness<- closeness(full_no_zero, mode = "total") #%>% round(6)

##in
summary(full_no_zero_incloseness)
sd(full_no_zero_incloseness)

##out
summary(full_no_zero_outcloseness)
sd(full_no_zero_outcloseness)

##all
summary(full_no_zero_totalcloseness)
sd(full_no_zero_totalcloseness)
```

#2.7.2 Closeness Non-normalized 
```{r}
#Closeness Normalized

#Saving to Igraph object
V(full_no_zero)$incloseness_n <- closeness(full_no_zero, mode = "in", normalized = T) #%>% round(6)
V(full_no_zero)$outcloseness_n <- closeness(full_no_zero, mode = "out", normalized = T) #%>% round(6)
V(full_no_zero)$totalcloseness_n <- closeness(full_no_zero, mode = "total", normalized = T) #%>% round(6)

#Saving to Environment
full_no_zero_incloseness_n<- closeness(full_no_zero, mode = "in", normalized = T) #%>% round(6)
full_no_zero_outcloseness_n<- closeness(full_no_zero, mode = "out", normalized = T) #%>% round(6)
full_no_zero_totalcloseness_n<- closeness(full_no_zero, mode = "total", normalized = T) #%>% round(6)

##in
summary(full_no_zero_incloseness_n)
sd(full_no_zero_incloseness_n)

##out
summary(full_no_zero_outcloseness_n)
sd(full_no_zero_outcloseness_n)

##all
summary(full_no_zero_totalcloseness_n)
sd(full_no_zero_totalcloseness_n)
```


#2.7.3 Closeness normalized weighted 
```{r}
#Closeness Normalized weighted - it only makes sense with var>7 or acesso and confianca - even though its here to make as a first script draft

#Saving to Igraph object
V(full_no_zero)$incloseness_w <- closeness(full_no_zero, mode = "in", normalized = T, weights=E(full_no_zero)$full_no_zero) #%>% round(6)
V(full_no_zero)$outcloseness_w <- closeness(full_no_zero, mode = "out", normalized = T,weights=E(full_no_zero)$full_no_zero) #%>% round(6)
V(full_no_zero)$totalcloseness_w <- closeness(full_no_zero, mode = "total", normalized = T, weights=E(full_no_zero)$full_no_zero) #%>% round(6)

#Saving to Environment
full_no_zero_incloseness_w<- closeness(full_no_zero, mode = "in", normalized = T, weights=E(full_no_zero)$full_no_zero) #%>% round(6)
full_no_zero_outcloseness_w<- closeness(full_no_zero, mode = "out", normalized = T, weights=E(full_no_zero)$full_no_zero) #%>% round(6)
full_no_zero_totalcloseness_w<- closeness(full_no_zero, mode = "total", normalized = T, weights=E(full_no_zero)$full_no_zero) #%>% round(6)

##in
summary(full_no_zero_incloseness_w)
sd(full_no_zero_incloseness_w)

##out
summary(full_no_zero_outcloseness_w)
sd(full_no_zero_outcloseness_w)

##all
summary(full_no_zero_totalcloseness_w)
sd(full_no_zero_totalcloseness_w)
```

#2.7.4 Closeness Weighted for all vertex 
```{r}
V(full_no_zero)$closeness_stats_full_no_zero_w <- closeness(full_no_zero, vids = V(full_no_zero), mode = "all", weights =V(full_no_zero)$full_no_zero, normalized = FALSE)

closeness_stats_full_no_zero_w <- closeness(full_no_zero, vids = V(full_no_zero), mode = "all", weights =V(full_no_zero)$full_no_zero, normalized = FALSE)
```

#2.7.5 Centralization Closseness
```{r}
V(full_no_zero)$centr_closeness_full_no_zero <- centralization.closeness(full_no_zero)$res
centr_closeness_full_no_zero <- centralization.closeness(full_no_zero)

#Centralization
centr_closeness_full_no_zero$centralization
#Theoretical Max
centr_closeness_full_no_zero$theoretical_max
```

#2.7.6 Network Plotting Based On Closeness 
```{r}
#Get Variable
V(full_no_zero)$full_no_zero_color_degree<-round(V(full_no_zero)$totalcloseness,4)

#Creating brewer pallette
vertex_full_no_zero_color_degree<-
  colorRampPalette(brewer.pal(length(unique(
          V(full_no_zero)$full_no_zero_color_degree)), "RdBu"))(
            length(unique(V(full_no_zero)$full_no_zero_color_degree)))

#Saving as Vertex properties 
V(full_no_zero)$vertex_full_no_zero_color_degree<-
  vertex_full_no_zero_color_degree[as.numeric(
  cut(V(full_no_zero)$full_no_zero_color_degree,
      breaks=length(unique(V(full_no_zero)$full_no_zero_color_degree))))]

#Plotting
plot(full_no_zero, 
     layout=layout.kamada.kawai(full_no_zero),
     vertex.color=V(full_no_zero)$vertex_full_no_zero_color_degree,
     vertex.label=get.vertex.attribute(full_no_zero,"LABEL_COR"),
     vertex.size=closeness(full_no_zero)*4000,
     vertex.frame.color="#ffffff",
     edge.arrow.size=0.1,
     vertex.label.cex=(closeness(full_no_zero))/80,
     edge.width=(edge.betweenness(full_no_zero, weights =E(full_no_zero)$full_no_zero))/500,
     edge.curved = TRUE
     )

#Solving Problems with legend rendering 
a<-V(full_no_zero)$full_no_zero_color_degree
b<-V(full_no_zero)$vertex_full_no_zero_color_degree
c<-table(a,b)
d<-as.data.frame(c)
e<-subset(d, d$Freq>0)
e<-e[order(e$a,decreasing=T),] 
f<-t(e$a)
g<-t(e$b)

#Adding Legend
legend(x=1, y=1.25,
       legend=as.character(f),
       pch=21,
       col = "#777777", 
       pt.bg=as.character(g),
       pt.cex=4,
       bty="n", 
       ncol=1,
       lty=1,
       cex = .3)

#Adding Title
  title("Network Closeness Degree Sized and Colored - full_no_zero_fancy", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
   sprintf("Median In Closennes: %.4f\nMedian Out Closennes: %.4f",
     median(closeness(full_no_zero, mode="in")), 
     median(closeness(full_no_zero, mode="out"))
   ))
```

#2.7.6 Network Plotting Based On Centralization Closeness 
```{r}
#Get Variable
V(full_no_zero)$full_no_zero_color_degree<-round(V(full_no_zero)$centr_closeness_full_no_zero,3)

#Creating brewer pallette
vertex_full_no_zero_color_degree<-
  colorRampPalette(brewer.pal(length(unique(
          V(full_no_zero)$full_no_zero_color_degree)), "Spectral"))(
            length(unique(V(full_no_zero)$full_no_zero_color_degree)))

#Saving as Vertex properties 
V(full_no_zero)$vertex_full_no_zero_color_degree<-
  vertex_full_no_zero_color_degree[as.numeric(
  cut(V(full_no_zero)$full_no_zero_color_degree,
      breaks=length(unique(V(full_no_zero)$full_no_zero_color_degree))))]

#Plotting
plot(full_no_zero, 
     layout=layout.kamada.kawai(full_no_zero),
     vertex.color=V(full_no_zero)$vertex_full_no_zero_color_degree,
     vertex.label=get.vertex.attribute(full_no_zero,"LABEL_COR"),
     vertex.size=centralization.closeness(full_no_zero)$res*20,
     vertex.frame.color="#ffffff",
     edge.arrow.size=0.1,
     vertex.label.cex=centralization.closeness(full_no_zero)$res,
     edge.width=(edge.betweenness(full_no_zero, weights =E(full_no_zero)$full_no_zero))/500,
     edge.curved = TRUE
     )

#Solving Problems with legend rendering 
a<-V(full_no_zero)$full_no_zero_color_degree
b<-V(full_no_zero)$vertex_full_no_zero_color_degree
c<-table(a,b)
d<-as.data.frame(c)
e<-subset(d, d$Freq>0)
e<-e[order(e$a,decreasing=T),] 
f<-t(e$a)
g<-t(e$b)

#Adding Legend
legend(x=1.25, y=1.25,
       legend=as.character(f),
       pch=21,
       col = "#777777", 
       pt.bg=as.character(g),
       pt.cex=4,
       bty="n", 
       ncol=1,
       lty=1,
       cex = .3)

#Adding Title
  title("Network Centralization Closeness - full_no_zero_fancy", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
   sprintf("Median In Closennes: %.4f\nMedian Out Closennes: %.4f",
     median(closeness(full_no_zero, mode="in")), 
     median(closeness(full_no_zero, mode="out"))
   ))
```

#2.8 Betweenness - Number of shortest paths going through the actor σst(i)

High betweenness centrality - vertex lies on many shortest paths. Probability that a communication from s
to t will go through i.

Betweenness measures the number of shortest paths between nodes in the network that go through the node in question. Nodes with relatively high betweenness are likely to be key conduits of information flow across a network, and their removal may have a large impact on spreading phenomena.

#2.8.1 Betweenness Centrality - (Vertex)
```{r}
#Betweenness centrality based on a broker position connecting others or Number of geodesics that pass through the node or the edge.

#Betweenness - number of geodesics (shortest paths) going through a vertex - A higher number means an more important node.
V(full_no_zero)$betweenness <- betweenness(full_no_zero) %>% round(1)
full_no_zero_betweenness <- betweenness(full_no_zero) %>% round(1)

##all
summary(full_no_zero_betweenness)
sd(full_no_zero_betweenness)
```

#2.8.2 Betweenness Centrality Weighted - (Vertex)
```{r}
V(full_no_zero)$betweenness_full_no_zero_w <- betweenness(full_no_zero, weights=E(full_no_zero)$full_no_zero) %>% round(1)

betweenness_full_no_zero_w <- betweenness(full_no_zero, weights=E(full_no_zero)$full_no_zero) %>% round(1)

##all
summary(betweenness_full_no_zero_w)
sd(betweenness_full_no_zero_w)
```

#2.8.3 Centralization Betweenness
```{r}
V(full_no_zero)$centr_betweenness_full_no_zero <- centralization.betweenness(full_no_zero)$res
centr_betweenness_full_no_zero <- centralization.betweenness(full_no_zero)

#Centralization
centr_betweenness_full_no_zero$centralization
#Theoretical Max
centr_betweenness_full_no_zero$theoretical_max
```

#2.8.1 Plotting Betweenness Centrality - (Vertex)
```{r}
#Plotting based only on betweenness measures 
plot(full_no_zero, 
     layout=layout.kamada.kawai(full_no_zero), 
     vertex.label=get.vertex.attribute(full_no_zero,"LABEL_COR"),
     vertex.size=betweenness(full_no_zero)/500,
     edge.arrow.size=0.1,
     vertex.label.cex=(betweenness(full_no_zero)+1)/10000
     edge.width=log(edge.betweenness(full_no_zero, weights =E(full_no_zero)$full_no_zero))/50,
     edge.curved = TRUE
     )

#Solving Problems with legend rendering 
a<-V(full_no_zero)$LABEL_COR
b<-V(full_no_zero)$color
c<-table(a,b)
d<-as.data.frame(c)
e<-subset(d, d$Freq>0)
f<-t(e$a)
g<-t(e$b)

#Adding Legend
legend(x=1, y=1.25,
       legend=as.character(f),
       pch=21,
       col = "#777777", 
       pt.bg=as.character(g),
       pt.cex=4,
       bty="n", 
       ncol=1,
       lty=1,
       cex = .8)

#Adding Title
  title("Network Vertex Betweenness Sized - full_no_zero_fancy", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
   sprintf("Median Betweenness: %.2f\nSD Betweenness: %.2f",
     median(betweenness(full_no_zero)), 
     sd(betweenness(full_no_zero))
   ))
```

#2.8.2 Network Plotting Based On Centralization Betweenness
```{r}
#Get Variable
V(full_no_zero)$centr_betweenness_full_no_zero<-(centralization.betweenness(full_no_zero)$res)/10000
V(full_no_zero)$full_no_zero_color_degree<-round(V(full_no_zero)$centr_betweenness_full_no_zero,1)

#Creating brewer pallette
vertex_full_no_zero_color_degree<-
  colorRampPalette(brewer.pal(length(unique(
          V(full_no_zero)$full_no_zero_color_degree)), "Spectral"))(
            length(unique(V(full_no_zero)$full_no_zero_color_degree)))

#Saving as Vertex properties 
V(full_no_zero)$vertex_full_no_zero_color_degree<-
  vertex_full_no_zero_color_degree[as.numeric(
  cut(V(full_no_zero)$full_no_zero_color_degree,
      breaks=length(unique(V(full_no_zero)$full_no_zero_color_degree))))]

#Plotting
plot(full_no_zero, 
     layout=layout.kamada.kawai(full_no_zero),
     vertex.color=V(full_no_zero)$vertex_full_no_zero_color_degree,
     vertex.label=get.vertex.attribute(full_no_zero,"LABEL_COR"),
     vertex.size=(centralization.betweenness(full_no_zero)$res)/500,
     vertex.frame.color="#ffffff",
     edge.arrow.size=0.1,
     vertex.label.cex=(centralization.betweenness(full_no_zero)$res + 1)/10000,
     edge.width=(edge.betweenness(full_no_zero, weights =E(full_no_zero)$full_no_zero))/500,
     edge.curved = TRUE
     )

#Solving Problems with legend rendering 
a<-V(full_no_zero)$full_no_zero_color_degree
b<-V(full_no_zero)$vertex_full_no_zero_color_degree
c<-table(a,b)
d<-as.data.frame(c)
e<-subset(d, d$Freq>0)
e<-e[order(e$a,decreasing=T),] 
f<-t(e$a)
g<-t(e$b)

#Adding Legend
legend(x=1.25, y=1.25,
       legend=as.character(f),
       pch=21,
       col = "#777777", 
       pt.bg=as.character(g),
       pt.cex=4,
       bty="n", 
       ncol=1,
       lty=1,
       cex = .3)

#Adding Title
  title("Network Centralization Betweenness - full_no_zero_fancy", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
   sprintf("Median Betweenness: %.0f\nSD Betweenness: %.0f",
     median(betweenness(full_no_zero)), 
     sd(betweenness(full_no_zero))
   ))
```

#2.9 Eigenvector - centrality proportional to the sum of connection centralities - Values of the first eigenvector of the graph matrix.

#2.9.1 Eigen Centrality - Saving objects
```{r}
#Eigen_Centrality 
V(full_no_zero)$eigenvector <- eigen_centrality(full_no_zero, directed = TRUE, weights = V(full_no_zero)$full_no_zero) %$% vector %>% round(3)

full_no_zero_eigenvector <- eigen_centrality(full_no_zero, directed = TRUE, weights = V(full_no_zero)$full_no_zero) %$% vector %>% round(3)

#Eigenvalue Calculated Eigenvector
full_no_zero_eigenvector_value <- eigen_centrality(full_no_zero, directed = TRUE, weights = V(full_no_zero)$full_no_zero) %$% value %>% round(3) # The eigenvalue corresponding to the calculated eigenvector, i.e. the centrality scores.
```

#2.9.2 Eigen Centrality Descriptive
```{r}
#all
summary(full_no_zero_eigenvector)
sd(full_no_zero_eigenvector)

#Eigenvalue Calculated Eigenvector
full_no_zero_eigenvector_value
```

#2.9.3 Centralization EigenVector - how central the most central node in the network in relation to all other nodes.
```{r}
V(full_no_zero)$centr_eigen_full_no_zero <- centr_eigen(full_no_zero, directed = T)$vector
centr_eigen_full_no_zero <-centr_eigen(full_no_zero, directed = T)$vector
#Centralization
centr_eigen(full_no_zero, directed = T)$centralization
#Max
centr_eigen(full_no_zero, directed = T)$theoretical_max
```

##2.9.4 Plotting based only on Eigenvector Centrality measures 
```{r}
#Plotting based only on Eigen Centrality measures 
plot(full_no_zero, 
     layout=layout.kamada.kawai(full_no_zero), 
     vertex.label=get.vertex.attribute(full_no_zero,"LABEL_COR"),
     vertex.size=(eigen_centrality(full_no_zero)$vector)*20,
     vertex.frame.color="#ffffff",
     edge.arrow.size=0.1,
     vertex.label.cex=sqrt((eigen_centrality(full_no_zero)$vector)+1)/10,
     edge.width=edge.betweenness(full_no_zero, weights =E(full_no_zero)$full_no_zero)/500,
     edge.curved = TRUE
     )

#Solving Problems with legend rendering 
a<-V(full_no_zero)$LABEL_COR
b<-V(full_no_zero)$color
c<-table(a,b)
d<-as.data.frame(c)
e<-subset(d, d$Freq>0)
f<-t(e$a)
g<-t(e$b)

#Adding Legend
legend(x=1, y=1.25,
       legend=as.character(f),
       pch=21,
       col = "#777777", 
       pt.bg=as.character(g),
       pt.cex=4,
       bty="n", 
       ncol=1,
       lty=1,
       cex = .8)

#Adding Title
  title("Eigenvector Centrality Measure - full_no_zero_fancy", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
   sprintf("Median Eigenvector: %.2f",
     median(eigen_centrality(full_no_zero)$vector)
   ))
```

#2.9.4 Network Plotting Based On Centralization Eigenvector
```{r}
#Get Variable
V(full_no_zero)$full_no_zero_color_degree<-round(centr_eigen(full_no_zero, directed = T)$vector,2)

#Creating brewer pallette
vertex_full_no_zero_color_degree<-
  colorRampPalette(brewer.pal(length(unique(
          V(full_no_zero)$full_no_zero_color_degree)), "Spectral"))(
            length(unique(V(full_no_zero)$full_no_zero_color_degree)))

#Saving as Vertex properties 
V(full_no_zero)$vertex_full_no_zero_color_degree<-
  vertex_full_no_zero_color_degree[as.numeric(
  cut(V(full_no_zero)$full_no_zero_color_degree,
      breaks=length(unique(V(full_no_zero)$full_no_zero_color_degree))))]

#Plotting
plot(full_no_zero, 
     layout=layout.kamada.kawai(full_no_zero),
     vertex.color=V(full_no_zero)$vertex_full_no_zero_color_degree,
     vertex.label=get.vertex.attribute(full_no_zero,"LABEL_COR"),
     vertex.size=centr_eigen(full_no_zero, directed = T)$vector*25,
     vertex.frame.color="#ffffff",
     edge.arrow.size=0.1,
     vertex.label.cex=(centr_eigen(full_no_zero, directed = T)$vector + 1)/5,
     edge.width=(edge.betweenness(full_no_zero, weights =E(full_no_zero)$full_no_zero))/500,
     edge.curved = TRUE
     )

#Solving Problems with legend rendering 
a<-V(full_no_zero)$full_no_zero_color_degree
b<-V(full_no_zero)$vertex_full_no_zero_color_degree
c<-table(a,b)
d<-as.data.frame(c)
e<-subset(d, d$Freq>0)
e<-e[order(e$a,decreasing=T),] 
f<-t(e$a)
g<-t(e$b)

#Adding Legend
legend(x=1.25, y=1.25,
       legend=as.character(f),
       pch=21,
       col = "#777777", 
       pt.bg=as.character(g),
       pt.cex=4,
       bty="n", 
       ncol=1,
       lty=1,
       cex = .3)

#Adding Title
  title("Network Centralization Betweenness - full_no_zero_fancy", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
   sprintf("Median Betweenness: %.0f\nSD Betweenness: %.0f",
     median(betweenness(full_no_zero)), 
     sd(betweenness(full_no_zero))
   ))
```

#2.8.1 Bonacich's Power Centrality Scores of Network Positions

Each node's centrality score depends both on how many connections it has and how many connections its connections have.
“It doesn't only matter how many friends you have. It matters how many friends your friends have.” <http://www.leonidzhukov.net/hse/2014/socialnetworks/papers/Bonacich-Centrality.pdf>

#Power (beta) Centrality Stats
```{r, results="asis"}
#Saving power as an object
power_centrality_stats_full_no_zero <- power_centrality(full_no_zero, nodes = V(full_no_zero), loops = FALSE, exponent = 1, rescale = FALSE, tol = 1e-07, sparse = TRUE)

#Saving power as an igraph attributte
V(full_no_zero)$power_centrality_stats_full_no_zero <-power_centrality(full_no_zero, nodes = V(full_no_zero), loops = FALSE, exponent = 1, rescale = FALSE, tol = 1e-07, sparse = TRUE) 

#Or are you more powerful if you're connected to weak, dependent people? (negative attenuation factor)
i=sort(power_centrality_stats_full_no_zero) %>% round(3)
table_i<-cbind(V(full_no_zero)$V5_LABEL_LABEL,i)
colnames(table_i) <- c("ID", "Power")
table_i10<-table_i[1:10,]
toptable_i10 <- xtable(table_i10)
print(toptable_i10 , floating=FALSE, type="html")

#Are you more powerful if you're connected to powerful people? (positive attenuation factor)
d=sort(power_centrality_stats_full_no_zero, decreasing = T) %>% round(3)
table_d<-cbind(V(full_no_zero)$V5_LABEL_LABEL,d)
colnames(table_d) <- c("ID", "Power")
table_d10<-table_d[1:10,]
toptable_d10 <- xtable(table_d10)
print(toptable_d10, floating=FALSE, type="html")
```

#2.8.3 #Setting color for each vertex based on Power
```{r}
# Set colors to plot distances:
V(full_no_zero)$power_centrality_stats_full_no_zero_r<-round(V(full_no_zero)$power_centrality_stats_full_no_zero,0)

# Creating brewer pallette
vertex_col_spower <- colorRampPalette(brewer.pal(length(unique(V(full_no_zero)$power_centrality_stats_full_no_zero_r)), "Blues"))(length(unique(V(full_no_zero)$power_centrality_stats_full_no_zero_r))) 

#Saving as Vertex properties                                   
V(full_no_zero)$color_spower <- vertex_col_spower[factor(V(full_no_zero)$power_centrality_stats_full_no_zero_r)]  
```

#2.8.3 Plotting Power Centrality Degree
```{r}
plot(full_no_zero, 
     layout=layout.kamada.kawai(full_no_zero), 
     vertex.color=V(full_no_zero)$color_spower, 
     vertex.label=V(full_no_zero)$V5_LABEL_LABEL,
     vertex.frame.color="#ffffff",
     vertex.label.color="black",
     vertex.size=sqrt((power_centrality(full_no_zero)^2))*5,
     edge.arrow.size=0.1,
     vertex.label.cex=sqrt((power_centrality(full_no_zero)+1)^2)/10,
     edge.width=edge.betweenness(full_no_zero, weights =E(full_no_zero)$full_no_zero)/500,
     edge.curved = TRUE
     )

#Solving Problems with legend rendering 
a<-V(full_no_zero)$power_centrality_stats_full_no_zero_r
b<-V(full_no_zero)$color_spower
c<-table(a,b)
d<-as.data.frame(c)
e<-subset(d, d$Freq>0)
f<-t(e$a)
g<-t(e$b)

#Adding Legend
legend(x=1, y=1.25,
       legend=as.character(f),
       pch=21,
       col = "#777777", 
       pt.bg=as.character(g),
       pt.cex=4,
       bty="n", 
       ncol=1,
       lty=1,
       cex = .5)

#Adding Title
  title("Bonacich Power Centrality Scores of Network Positions - full_no_zero_fancy", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
   sprintf("Median Power Degree: %.2f\n SD Power Degree:%.2f",
     median(power_centrality_stats_full_no_zero),sd(power_centrality_stats_full_no_zero)
     ))
```

#2.8.2 Alpha Bonacich's centrality scores of network positions
```{r, results="asis"}
#Saving power as an object
alpha_centrality_stats_full_no_zero <- alpha_centrality(full_no_zero, loops = FALSE, alpha = 1, exo = 1, weights = V(full_no_zero)$full_no_zero, tol = 1e-07, sparse = TRUE)

#Saving power as an igraph attributte
V(full_no_zero)$alpha_centrality_stats_full_no_zero<-alpha_centrality(full_no_zero, loops = FALSE, alpha = 1, exo = 1, weights = V(full_no_zero)$full_no_zero, tol = 1e-07, sparse = TRUE)

#Descriptive 
summary(alpha_centrality_stats_full_no_zero)
sd(alpha_centrality_stats_full_no_zero)

#Or are you more powerful if you're connected to weak, dependent people? (negative attenuation factor)
i=sort(alpha_centrality_stats_full_no_zero) %>% round(3)
table_i<-cbind(V(full_no_zero)$V5_LABEL_LABEL,i)
colnames(table_i) <- c("ID", "Alpha")
table_i10<-table_d[1:10,]
toptable_i10 <- xtable(table_i10)
print(toptable_i10 , floating=FALSE, type="html")

#Are you more powerful if you're connected to powerful people? (positive attenuation factor)
d=sort(alpha_centrality_stats_full_no_zero, decreasing = T) %>% round(3)
table_d<-cbind(V(full_no_zero)$V5_LABEL_LABEL,d)
colnames(table_d) <- c("ID", "Power")
table_d10<-table_d[1:10,]
toptable_d <- xtable(table_d10)
print(toptable_d , floating=FALSE, type="html")
```

#2.8.3 #Setting color for each vertex based on Alpha
```{r}
# Set colors to plot distances:
V(full_no_zero)$alpha_centrality_stats_full_no_zero_r<-round(V(full_no_zero)$alpha_centrality_stats_full_no_zero,0)

# Creating brewer pallette
vertex_col_s <- colorRampPalette(brewer.pal(length(unique(V(full_no_zero)$alpha_centrality_stats_full_no_zero_r)), "YlOrRd"))(length(unique(V(full_no_zero)$alpha_centrality_stats_full_no_zero_r))) 

#Saving as Vertex properties                                                                  
V(full_no_zero)$color_s <- vertex_col_s[factor(V(full_no_zero)$alpha_centrality_stats_full_no_zero_r)]  
```

#2.8.3 Plotting Alpha Centrality Degree
```{r}
plot(full_no_zero, 
     layout=layout.kamada.kawai(full_no_zero), 
     vertex.color=V(full_no_zero)$color_s, 
     vertex.label=V(full_no_zero)$V5_LABEL_LABEL,
     vertex.label.color="black",
     vertex.frame.color="#ffffff",
     vertex.size=sqrt((alpha_centrality(full_no_zero, weights = V(full_no_zero)$full_no_zero)^2)), #Squared in order to avoid sinal
     edge.arrow.size=0.1,
     vertex.label.cex=sqrt((alpha_centrality(full_no_zero))^2)/50,
     edge.width=edge.betweenness(full_no_zero, weights =E(full_no_zero)$full_no_zero)/500,
     edge.curved = TRUE
     )

#Solving Problems with legend rendering 
a<-V(full_no_zero)$alpha_centrality_stats_full_no_zero_r
b<-V(full_no_zero)$color_s
c<-table(a,b)
d<-as.data.frame(c)
e<-subset(d, d$Freq>0)
f<-t(e$a)
g<-t(e$b)

#Adding Legend
legend(x=1, y=1.25,
       legend=as.character(f),
       pch=21,
       col = "#777777", 
       pt.bg=as.character(g),
       pt.cex=4,
       bty="n", 
       ncol=1,
       lty=1,
       cex = .5)

#Adding Title
  title("Bonacich's Alpha Centrality Scores of Network Positions - full_no_zero_fancy", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
   sprintf("Median Alpha Degree: %.2f\n SD Alpha Degree:%.2f",
     median(alpha_centrality_stats_full_no_zero),sd(alpha_centrality_stats_full_no_zero)
     ))
```

#2.10 Hubs and Authorities (HITS)

Citation networks. Reviews vs original research (authoritative) papers
• authorities, contain useful information
• hubs, contains links to authorities
• Good authorities referred by good hubs
• Good hubs point to good authorities

#2.10.1 Hubs - are expected to contain catalogs with a large number of outgoing links; 
```{r}
#Kleinberg’s hub scores
V(full_no_zero)$hub <- hub.score(full_no_zero)$vector
full_no_zero_hub<- hub.score(full_no_zero)$vector

#stats
summary(full_no_zero_hub)
sd(full_no_zero_hub)

#Plotting based only on degree measures 
plot(full_no_zero, 
     layout=layout.kamada.kawai(full_no_zero), 
     vertex.label=get.vertex.attribute(full_no_zero,"LABEL_COR"),
     vertex.size=10*hub.score(full_no_zero)$vector,
     vertex.frame.color="#ffffff",
     vertex.label.color="black",
     edge.arrow.size=0.1,
     vertex.label.cex=log(degree(full_no_zero)+1)/10,
     edge.width=edge.betweenness(full_no_zero, weights =E(full_no_zero)$full_no_zero)/500,
     edge.curved = TRUE)

#Solving Problems with legend rendering 
a<-V(full_no_zero)$LABEL_COR
b<-V(full_no_zero)$color
c<-table(a,b)
d<-as.data.frame(c)
e<-subset(d, d$Freq>0)
f<-t(e$a)
g<-t(e$b)

#Adding Legend
legend(x=1, y=1.25,
       legend=as.character(f),
       pch=21,
       col = "#777777", 
       pt.bg=as.character(g),
       pt.cex=4,
       bty="n", 
       ncol=1,
       lty=1,
       cex = .8)

#Adding Title
  title("Network Hub Sized - full_no_zero_fancy", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
   sprintf("Mean Hub Score: %.2f\n SD Hub Score: %.2f",
     mean(full_no_zero_hub),
     sd(full_no_zero_hub)))
```

#2.10.2 Authorities would get many incoming links from hubs, presumably because of their high-quality relevant information.
```{r}
#Kleinberg’s authority scores
V(full_no_zero)$authority.score<- authority.score(full_no_zero)$vector
full_no_zero_auth <- authority.score(full_no_zero)$vector
#stats
summary(full_no_zero_auth)
sd(full_no_zero_auth)

#Plotting based only on Authorities Scores 
plot(full_no_zero, 
     layout=layout.kamada.kawai(full_no_zero), 
     vertex.label=get.vertex.attribute(full_no_zero,"LABEL_COR"),
     vertex.size=10*authority.score(full_no_zero)$vector,
     vertex.frame.color="#ffffff",
     vertex.label.color="black",
     edge.arrow.size=0.1,
     vertex.label.cex=log(degree(full_no_zero)+1)/20,
     edge.width=log(edge.betweenness(full_no_zero, weights =E(full_no_zero)$full_no_zero))/50,
     edge.curved = TRUE)

#Solving Problems with legend rendering 
a<-V(full_no_zero)$LABEL_COR
b<-V(full_no_zero)$color
c<-table(a,b)
d<-as.data.frame(c)
e<-subset(d, d$Freq>0)
f<-t(e$a)
g<-t(e$b)

#Adding Legend
legend(x=1, y=1.25,
       legend=as.character(f),
       pch=21,
       col = "#777777", 
       pt.bg=as.character(g),
       pt.cex=4,
       bty="n", 
       ncol=1,
       lty=1,
       cex = .8)

#Adding Title
  title("Network Authorities Scores  Sized - full_no_zero_fancy", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
   sprintf("Mean Authorities Scores: %.2f\n SD Authorities Scores: %.2f",
     mean(full_no_zero_auth),
     sd(full_no_zero_auth)))
```

#2.10.3 Google PageRank 
```{r}
#PageRank
V(full_no_zero)$page.rank<-page.rank(full_no_zero)$vector
full_no_zero_pr<-page.rank(full_no_zero)$vector
#stats
summary(full_no_zero_pr)
sd(full_no_zero_pr)


#Plotting based only on Authorities Scores 
plot(full_no_zero, 
     layout=layout.kamada.kawai(full_no_zero), 
     vertex.label=get.vertex.attribute(full_no_zero,"LABEL_COR"),
     vertex.size=200*page.rank(full_no_zero)$vector,
     edge.arrow.size=0.1,
     vertex.frame.color="#ffffff",
     vertex.label.color="black",
     vertex.label.cex=log(degree(full_no_zero)+1)/15,
     edge.width=log(edge.betweenness(full_no_zero, weights =E(full_no_zero)$full_no_zero))/100,
     edge.curved = TRUE)

#Solving Problems with legend rendering 
a<-V(full_no_zero)$LABEL_COR
b<-V(full_no_zero)$color
c<-table(a,b)
d<-as.data.frame(c)
e<-subset(d, d$Freq>0)
f<-t(e$a)
g<-t(e$b)

#Adding Legend
legend(x=1, y=1.25,
       legend=as.character(f),
       pch=21,
       col = "#777777", 
       pt.bg=as.character(g),
       pt.cex=4,
       bty="n", 
       ncol=1,
       lty=1,
       cex = .8)

#Adding Title
  title("Network Google Page Rank Sized - full_no_zero_fancy", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
   sprintf("Mean Page Rank Scores: %.2f\n SD Page Rank Scores: %.2f",
     mean(full_no_zero_pr),
     sd(full_no_zero_pr)))
```

#Plotting HITS by Degree
```{r}
# We will plot 6 graphs in 2 rows and 3 columns:
V(full_no_zero)$prPB<-page_rank(full_no_zero)$vector
V(full_no_zero)$hPB<-hub.score(full_no_zero)$vector
V(full_no_zero)$authPB<-authority.score(full_no_zero)$vector
V(full_no_zero)$indegPB<-degree(full_no_zero, mode="in")
V(full_no_zero)$outdegPB<-degree(full_no_zero, mode="out")

  
op <- par(mfrow = c(2, 3))
plot(prPB, hPB, xlab="PageRank", ylab="Hubs", col="blue")
plot(prPB, authPB, xlab="PageRank", ylab="Authorities", col="blue") 
plot(hPB, authPB, xlab="Hubs", ylab="Authorities", col="blue") 
plot(indegPB, outdegPB, xlab="In- degree", ylab="Out- degree", col="blue") 
plot(indegPB, authPB, xlab="In- degree", ylab="Authorities", col="blue")
plot(outdegPB, hPB, xlab="Out- degree", ylab="Hubs", col="blue")
par(op)
#Adding Title

```


#Top page rankm huFor in-degrees:
```{r}
#For in-degrees:
indegnamesPB=which(indegPB>sort(indegPB)[vcount(full_no_zero)-5]) #For out- degrees:
outdegnamesPB=which(outdegPB>sort(outdegPB)[vcount(full_no_zero)-5]) #For PageRank:
prnamesPB=which(prPB>sort(prPB)[vcount(full_no_zero)-5]) #For Hubs:
hnamesPB=which(hPB>sort(hPB)[vcount(full_no_zero)-5]) #For Authorities:
authnamesPB=which(authPB>sort(authPB)[vcount(full_no_zero)-5])
namematch=
##Create a matrix to output:
topnamesPB=cbind(indegnamesPB,outdegnamesPB,authnamesPB,prnamesPB, hnamesPB, )
#Assign column names:
colnames(topnamesPB) <- c("In- degree","Out- degree", "Authorities", "PageRank","Hubs")
row.names(topnamesPB) <- c("Assistência Hospitalar - Hospital de Pronto.Socorro", "Ambulatório de Saúde.Mental", "CAPS-AD", "Urgência.Emergência - SAMU.", "CAPS-Casa Viva")

topnamesPB
```


##2.10.3.1 Ego Network CAPSAD - Example considering its high page rank
#2.10.3.1.1 All connections 
```{r}
#Making igrpah ego object
make_ego_graph_all_full_no_zero<-make_ego_graph(full_no_zero, order=1, V(full_no_zero)[V2_LABEL_ID=="q170_CAPS...CAPS.AD"], mode ="all", mindist = 0)[[1]]

#Plotting ego based on Page Rank 
plot(make_ego_graph_all_full_no_zero, 
     layout=layout.kamada.kawai(make_ego_graph_all_full_no_zero), 
     vertex.label=get.vertex.attribute(make_ego_graph_all_full_no_zero,"LABEL_COR"),
     vertex.size=30*sqrt(page.rank(make_ego_graph_all_full_no_zero)$vector),
     edge.arrow.size=0.1,
     vertex.label.cex=log(degree(make_ego_graph_all_full_no_zero))/10,
     edge.width=log(edge.betweenness(make_ego_graph_all_full_no_zero))/100,
     edge.curved = TRUE)

#Solving Problems with legend rendering 
a<-V(make_ego_graph_all_full_no_zero)$LABEL_COR
b<-V(make_ego_graph_all_full_no_zero)$color
c<-table(a,b)
d<-as.data.frame(c)
e<-subset(d, d$Freq>0)
f<-t(e$a)
g<-t(e$b)

#Adding Legend
legend(x=1, y=1.25,
       legend=as.character(f),
       pch=21,
       col = "#777777", 
       pt.bg=as.character(g),
       pt.cex=4,
       bty="n", 
       ncol=1,
       lty=1,
       cex = .8)

#Adding Title
  title("Ego Network Page Rank Sized - All connections from CAPSAD - full_no_zero_fancy", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
   sprintf("Mean Page Rank Scores: %.2f\n SD Page Rank Scores: %.2f",
     mean(page.rank(make_ego_graph_all_full_no_zero)$vector),
     sd(page.rank(make_ego_graph_all_full_no_zero)$vector)
     ))
```

#2.10.3.1.2 Out connections 
```{r}
#Making igrpah ego object
make_ego_graph_out_full_no_zero<-make_ego_graph(full_no_zero, order=1, V(full_no_zero)[V2_LABEL_ID=="q170_CAPS...CAPS.AD"], mode ="out", mindist = 0)[[1]]

#Plotting ego based on Page Rank 
plot(make_ego_graph_out_full_no_zero, 
     layout=layout.kamada.kawai(make_ego_graph_out_full_no_zero), 
     vertex.label=get.vertex.attribute(make_ego_graph_out_full_no_zero,"LABEL_COR"),
     vertex.size=30*sqrt(page.rank(make_ego_graph_out_full_no_zero)$vector),
     edge.arrow.size=0.1,
     vertex.label.cex=log(degree(make_ego_graph_out_full_no_zero))/10,
     edge.width=log(edge.betweenness(make_ego_graph_out_full_no_zero))/100,
     edge.curved = TRUE)

#Solving Problems with legend rendering 
a<-V(make_ego_graph_out_full_no_zero)$LABEL_COR
b<-V(make_ego_graph_out_full_no_zero)$color
c<-table(a,b)
d<-as.data.frame(c)
e<-subset(d, d$Freq>0)
f<-t(e$a)
g<-t(e$b)

#Adding Legend
legend(x=1, y=1.25,
       legend=as.character(f),
       pch=21,
       col = "#777777", 
       pt.bg=as.character(g),
       pt.cex=4,
       bty="n", 
       ncol=1,
       lty=1,
       cex = .8)

#Adding Title
  title("Ego Network Page Rank Sized - Out Connections from CAPSAD - full_no_zero_fancy", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
   sprintf("Mean Page Rank Scores: %.2f\n SD Page Rank Scores: %.2f",
     mean(page.rank(make_ego_graph_out_full_no_zero)$vector),
     sd(page.rank(make_ego_graph_out_full_no_zero)$vector)
     ))
```

#2.10.3.1.3 - In connections 
```{r}
#Making igrpah ego object
make_ego_graph_in_full_no_zero<-make_ego_graph(full_no_zero, order=1, V(full_no_zero)[V2_LABEL_ID=="q170_CAPS...CAPS.AD"], mode ="in", mindist = 0)[[1]]

#Plotting ego based on Page Rank 
plot(make_ego_graph_in_full_no_zero, 
     layout=layout.kamada.kawai(make_ego_graph_in_full_no_zero), 
     vertex.label=get.vertex.attribute(make_ego_graph_in_full_no_zero,"LABEL_COR"),
     vertex.size=30*sqrt(page.rank(make_ego_graph_in_full_no_zero)$vector),
     edge.arrow.size=0.1,
     vertex.label.cex=log(degree(make_ego_graph_in_full_no_zero))/10,
     edge.width=log(edge.betweenness(make_ego_graph_in_full_no_zero))/100,
     edge.curved = TRUE)

#Solving Problems with legend rendering 
a<-V(make_ego_graph_in_full_no_zero)$LABEL_COR
b<-V(make_ego_graph_in_full_no_zero)$color
c<-table(a,b)
d<-as.data.frame(c)
e<-subset(d, d$Freq>0)
f<-t(e$a)
g<-t(e$b)

#Adding Legend
legend(x=1, y=1.25,
       legend=as.character(f),
       pch=21,
       col = "#777777", 
       pt.bg=as.character(g),
       pt.cex=4,
       bty="n", 
       ncol=1,
       lty=1,
       cex = .8)

#Adding Title
  title("Ego Network CAPS AD Page Rank Sized - In connections from CAPS AD - full_no_zero_fancy", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
   sprintf("Mean Page Rank Scores: %.2f\n SD Page Rank Scores: %.2f",
     mean(page.rank(make_ego_graph_in_full_no_zero)$vector),
     sd(page.rank(make_ego_graph_in_full_no_zero)$vector)
     ))
```

##2.10.4 Removing CAPSAD from its own ego network-  Example considering its high page rank and resilience test
```{r}
#Making igrpah ego object
make_ego_graph_all_full_no_zero_without_CAPSAD<-delete_vertices(make_ego_graph_all_full_no_zero,V(make_ego_graph_all_full_no_zero)[V2_LABEL_ID=="q170_CAPS...CAPS.AD"])

#Plotting ego based on Page Rank 
plot(make_ego_graph_all_full_no_zero_without_CAPSAD, 
     layout=layout.kamada.kawai(make_ego_graph_all_full_no_zero_without_CAPSAD), 
     vertex.label=get.vertex.attribute(make_ego_graph_all_full_no_zero_without_CAPSAD,"LABEL_COR"),
     vertex.size=30*sqrt(page.rank(make_ego_graph_all_full_no_zero_without_CAPSAD)$vector),
     edge.arrow.size=0.1,
     vertex.label.cex=log(degree(make_ego_graph_all_full_no_zero_without_CAPSAD))/10,
     edge.width=log(edge.betweenness(make_ego_graph_all_full_no_zero_without_CAPSAD))/10,
     edge.curved = TRUE)

#Solving Problems with legend rendering 
a<-V(make_ego_graph_all_full_no_zero_without_CAPSAD)$LABEL_COR
b<-V(make_ego_graph_all_full_no_zero_without_CAPSAD)$color
c<-table(a,b)
d<-as.data.frame(c)
e<-subset(d, d$Freq>0)
f<-t(e$a)
g<-t(e$b)

#Adding Legend
legend(x=1, y=1.25,
       legend=as.character(f),
       pch=21,
       col = "#777777", 
       pt.bg=as.character(g),
       pt.cex=4,
       bty="n", 
       ncol=1,
       lty=1,
       cex = .8)

#Adding Title
  title("Ego Network CAPS AD Page Rank Sized - All Connections Removing CAPS AD - full_no_zero_fancy", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
   sprintf("Mean Page Rank Scores: %.2f\n SD Page Rank Scores: %.2f",
     mean(page.rank(make_ego_graph_all_full_no_zero_without_CAPSAD)$vector),
     sd(page.rank(make_ego_graph_all_full_no_zero_without_CAPSAD)$vector)
     ))
```


##2.10.5 Removing complex services from CAPSAD ego network -  Example considering its high page rank and resilience test (!= CAPSAD, PRONTO-SOCORRO, AMBULATÓRIO, SAMU)
```{r}
#Making igrpah ego object - without (!= CAPSAD, PRONTO-SOCORRO, AMBULATÓRIO, SAMU)

#Removing high complex services

make_ego_graph_all_full_no_zero_without_complex<-
  delete_vertices(make_ego_graph_all_full_no_zero,
                  c(V(make_ego_graph_all_full_no_zero)
                  [V2_LABEL_ID=="q170_CAPS...CAPS.AD"],#Removing CAPSAD - Psychosocial Treatment Center
                  V(make_ego_graph_all_full_no_zero)
                  [V2_LABEL_ID=="q163_Assistência.Hospitalar...Hospital.de.Pronto.Socorro...HPS"], #Removing PRONTO-SOCORRO - Emergency Hospital
                  V(make_ego_graph_all_full_no_zero)
                  [V2_LABEL_ID=="q192_Ambulatório.de.Saúde.Mental...Centro.de.Atenção.à.Saúde.Mental..CASM."],#Removing AMBULATÓRIO - Outpatient Center
                  V(make_ego_graph_all_full_no_zero)
                  [V2_LABEL_ID=="q191_Urgência.Emergência...Serviço.de.Atendimento.Móvel.de.Urgência..SAMU."] #Removing SAMU - Emergency Mobile Service ? WTF
                  ))


#Plotting ego based on Page Rank 
plot(make_ego_graph_all_full_no_zero_without_complex, 
     layout=layout.kamada.kawai(make_ego_graph_all_full_no_zero_without_complex), 
     vertex.label=get.vertex.attribute(make_ego_graph_all_full_no_zero_without_complex,"LABEL_COR"),
     vertex.size=30*sqrt(page.rank(make_ego_graph_all_full_no_zero_without_complex)$vector),
     edge.arrow.size=0.1,
     vertex.label.cex=log(degree(make_ego_graph_all_full_no_zero_without_complex))/10,
     edge.width=log(edge.betweenness(make_ego_graph_all_full_no_zero_without_complex))/10,
     edge.curved = TRUE)

#Solving Problems with legend rendering 
a<-V(make_ego_graph_all_full_no_zero_without_complex)$LABEL_COR
b<-V(make_ego_graph_all_full_no_zero_without_complex)$color
c<-table(a,b)
d<-as.data.frame(c)
e<-subset(d, d$Freq>0)
f<-t(e$a)
g<-t(e$b)

#Adding Legend
legend(x=1, y=1.25,
       legend=as.character(f),
       pch=21,
       col = "#777777", 
       pt.bg=as.character(g),
       pt.cex=4,
       bty="n", 
       ncol=1,
       lty=1,
       cex = .8)

#Adding Title
  title("Ego Network CAPS AD Page Rank Sized - All Connections Removing High Complex Service - full_no_zero_fancy", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
   sprintf("Mean Page Rank Scores: %.2f\n SD Page Rank Scores: %.2f",
     mean(page.rank(make_ego_graph_all_full_no_zero_without_complex)$vector),
     sd(page.rank(make_ego_graph_all_full_no_zero_without_complex)$vector)
     ))
```

#2.11.1 Minimum spanning tree non-normalized that connects all the vertices together with the minimal total edges. - it seems that we have a problem with legends - check this out further
```{r}
#Creating Minimum spanning tree
msp_full_no_zero<-mst(full_no_zero)

#Plotting based only on degree measures 
plot(msp_full_no_zero, 
     layout=layout.kamada.kawai(msp_full_no_zero), 
     vertex.label=get.vertex.attribute(msp_full_no_zero,"LABEL_COR"),
     vertex.size=sqrt(degree(msp_full_no_zero)),
     edge.arrow.size=0.1,
     vertex.label.cex=sqrt(degree(msp_full_no_zero))/50,
     edge.width=log(edge.betweenness(msp_full_no_zero, weights =E(msp_full_no_zero)$msp_full_no_zero))/5,
     edge.curved = TRUE
     )

#Solving Problems with legend rendering 
a<-V(msp_full_no_zero)$LABEL_COR
b<-V(msp_full_no_zero)$color
c<-table(a,b)
d<-as.data.frame(c)
e<-subset(d, d$Freq>0)
f<-t(e$a)
g<-t(e$b)

#Adding Legend
legend(x=1, y=1.25,
       legend=as.character(f),
       pch=21,
       col = "#777777", 
       pt.bg=as.character(g),
       pt.cex=4,
       bty="n", 
       ncol=1,
       lty=1,
       cex = .8)

#Adding Title
  title("Minimum Spanning Tree Non-normalized - from full_no_zero_fancy", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
   sprintf("Median In Degree: %.2f\nMedian Out Degree: %.2f",
     median(degree(msp_full_no_zero, mode="in")), 
     median(degree(msp_full_no_zero, mode="out"))
   ))
```

#2.11.2 Minimum weighted spanning tree that connects all the vertices together with the minimal total weighting for its edges. - it seems that we have a problem with legends - check this out further
```{r}
#Creating Minimum weighted spanning tree
msp_full_no_zero_w<-minimum.spanning.tree(full_no_zero, weights=1/edge.betweenness(full_no_zero, weights =E(full_no_zero)$full_no_zero), algorithm='prim')

#Weight edges are considered here as barries as we are trying to find the easiest path with minimal weight - so considering the inverse of weight (1/weight) are we using "unacess" and "untrust" as barries - this would cool   

#Plotting based only on degree measures 
plot(msp_full_no_zero_w, 
     layout=layout.kamada.kawai(msp_full_no_zero_w), 
     vertex.label=get.vertex.attribute(msp_full_no_zero_w,"LABEL_COR"),
     vertex.size=sqrt(degree(msp_full_no_zero_w)),
     edge.arrow.size=0.1,
     vertex.label.cex=sqrt(degree(msp_full_no_zero_w))/20,
     edge.width=1/edge.betweenness(msp_full_no_zero_w, weights =E(msp_full_no_zero_w)$msp_full_no_zero_w),
     edge.curved = TRUE
     )

#Solving Problems with legend rendering 
a<-V(msp_full_no_zero_w)$LABEL_COR
b<-V(msp_full_no_zero_w)$color
c<-table(a,b)
d<-as.data.frame(c)
e<-subset(d, d$Freq>0)
f<-t(e$a)
g<-t(e$b)

#Adding Legend
legend(x=1, y=1.25,
       legend=as.character(f),
       pch=21,
       col = "#777777", 
       pt.bg=as.character(g),
       pt.cex=4,
       bty="n", 
       ncol=1,
       lty=1,
       cex = .8)

#Adding Title
  title("Minimum Weighted Spanning Tree (Edge Betweenness) - full_no_zero_fancy", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
   sprintf("Median In Degree: %.2f\nMedian Out Degree: %.2f",
     median(degree(msp_full_no_zero_w, mode="in")), 
     median(degree(msp_full_no_zero_w, mode="out"))
   ))
```

#3. The proportion of present edges from all possible edges in the network.
#3.1 Edge Density
```{r}
#Edge Density
edge_density_full_no_zero<-edge_density(full_no_zero) #The proportion of present edges from all possible edges in the network.

# Density - same as above
graph.density(full_no_zero)
```

#3.2.1 Reciprocity Default
```{r}
# Reciprocity Default - Proportion of mutual connections - probability that hte opposite counterpart of a directed graph is also included
reciprocity(full_no_zero, mode="default")
```

#3.2.2 Reciprocity Ratio
```{r}
# Reciprocity Ratio - Probability  of mutual connections between a vertex pair - if we know - probability that hte opposite counterpart of a directed graph is also included in the 
reciprocity(full_no_zero, mode="ratio")
```

#3.3 Connected components
```{r}
#Is connected - decides whether the graph is weakly or strongly connected.
is.connected(full_no_zero) #trick information - let's check this out further
```

#3.3.1 Strong Connected Components
```{r}
#Strong Connected
components_strong_full_no_zero<-components(full_no_zero, mode="strong")
V(full_no_zero)$components_strong_full_no_zero_membership<-components_strong_full_no_zero$membership # adding to igraph object

#Number of clusters
count_components(full_no_zero)

#Display components structure
components_strong_full_no_zero
```

#3.3.2 Plotting connected strong components using membership
```{r}
#Plotting based only on degree measures 
plot(full_no_zero, 
     layout=layout.kamada.kawai(full_no_zero), 
     vertex.label=get.vertex.attribute(full_no_zero,"LABEL_COR"),
     vertex.size=sqrt(degree(full_no_zero)),
     mark.groups = by(seq_along(components_strong_full_no_zero$membership),
                      components_strong_full_no_zero$membership, 
                      invisible),   ## Add community indicating background colors
     edge.arrow.size=0.1,
     vertex.label.cex=log(degree(full_no_zero)+1)/10,
     edge.width=edge.betweenness(full_no_zero, weights =E(full_no_zero)$full_no_zero)/500,
     edge.curved = TRUE
     )

#Solving Problems with legend rendering 
a<-V(full_no_zero)$LABEL_COR
b<-V(full_no_zero)$color
c<-table(a,b)
d<-as.data.frame(c)
e<-subset(d, d$Freq>0)
f<-t(e$a)
g<-t(e$b)

#Adding Legend
legend(x=1, y=1.25,
       legend=as.character(f),
       pch=21,
       col = "#777777", 
       pt.bg=as.character(g),
       pt.cex=4,
       bty="n", 
       ncol=1,
       lty=1,
       cex = .8)

#Adding Title
  title("Network Vertex Degree Sized - Classfied by Components Structure - full_no_zero_fancy", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
   sprintf("Median In Degree: %.2f\nMedian Out Degree: %.2f",
     median(degree(full_no_zero, mode="in")), 
     median(degree(full_no_zero, mode="out"))
   ))
```

#3.4 Dyad Census
```{r}
#Dyad Census (Number of pairs with mutual connections "mut" and number of pairs with non-mutual connections "asym")
dyad.census_full_no_zero<-dyad.census(full_no_zero)

# Calculating reciprocity - just as an example
2*dyad_census(full_no_zero)$mut/ecount(full_no_zero) # Calculating reciprocity
```

#3.4.1 Dyad Census classified by mutal non-mutual and no connection
```{r}
#3.4 Dyad Census
#Dyad - Classify dyads in a directed graphs

#The number of pairs with mutual connections.
dyad.census_full_no_zero_mut<-dyad.census_full_no_zero$mut
dyad.census_full_no_zero_mut

#The number of pairs with non-mutual connections.
dyad.census_full_no_zero_asym<-dyad.census_full_no_zero$asym
dyad.census_full_no_zero_asym

#The number of pairs with no connection between them.
dyad.census_full_no_zero_null<-dyad.census_full_no_zero$null
dyad.census_full_no_zero_null
```

#3.5 - Triad Census - Check this out in order to understand triad lables *http://www.stats.ox.ac.uk/~snijders/Trans_Triads_ha.pdf* - page 15
```{r}
#Triad Census 
tc_full_no_zero <- triad.census(full_no_zero)

#Triad Census Label 
census_labels = c('003',
                  '012',
                  '102',
                  '021D',
                  '021U',
                  '021C',
                  '111D',
                  '111U',
                  '030T',
                  '030C',
                  '201',
                  '120D',
                  '120U',
                  '120C',
                  '210',
                  '300')

#Saving in a dataframe for further studies
triad_df_full_no_zero <- data.frame(census_labels,tc_full_no_zero)
write.csv(triad_df_full_no_zero, "~/SNArRDJF/Banco Redes R/full_no_zero_complet_triads.csv)
```

#3.5.1 - Triad Census Types 
```{r}
#Describing triads
triad_df_full_no_zero
```


#3.7 Distances and paths - shortest distance between each pair of nodes in the network (in both directions for directed graphs).

#3.7.1 Average path length between any two given nodes - calculates the average path length in a graph, by calculating the shortest paths between all pairs of vertices (both ways for directed graphs). This function does not consider edge weights currently and uses a breadth-first search
```{r}
#Average path length
mean_distance(full_no_zero, directed=T, unconnected = T)
```


#3.7.2 Shortest Paths
```{r}
#Shortest Paths
full_no_zero_sp_in <- shortest.paths(full_no_zero, mode='in', weights=V(full_no_zero)$full_no_zero) # sames as bellow
full_no_zero_sp_out <- shortest.paths(full_no_zero, mode='out', weights=V(full_no_zero)$full_no_zero) #same as above
full_no_zero_sp_all <- shortest.paths(full_no_zero, mode='all', weights=V(full_no_zero)$full_no_zero) # all

#in
mean(full_no_zero_sp_in[which(full_no_zero_sp_in != Inf)])
sd(full_no_zero_sp_in[which(full_no_zero_sp_in != Inf)])

#out
mean(full_no_zero_sp_out[which(full_no_zero_sp_out != Inf)])
sd(full_no_zero_sp_out[which(full_no_zero_sp_out != Inf)])

#all
mean(full_no_zero_sp_out[which(full_no_zero_sp_out != Inf)])
sd(full_no_zero_sp_out[which(full_no_zero_sp_out != Inf)])
```


#3.7.3 Length of all shortest paths in the graph:
```{r}
#All shortest paths 
distances_sp_all_full_no_zero<-distances(full_no_zero, mode="all", weights=E(full_no_zero)$full_no_zero)
#distances_sp_all_full_no_zero

#In shortest paths 
distances_sp_in_full_no_zero<-distances(full_no_zero, mode="in",weights=E(full_no_zero)$full_no_zero)
#distances_sp_in_full_no_zero

#Out shortest paths 
distances_sp_out_full_no_zero<-distances(full_no_zero, mode="out", weights=E(full_no_zero)$full_no_zero)
#distances_sp_out_full_no_zero
```

#3.7.4 Histogram from shortest path length between each pair of vertices. 
```{r}
distance_table(full_no_zero, directed = TRUE)
```

#3.7.5 Distance from CAPSAD - just as an example for futher explortion
```{r}
#Setting distance from an vertex

full_no_zero_dist.from.CAPSAD <- distances(full_no_zero, v=V(full_no_zero)[V2_LABEL_ID=="q170_CAPS...CAPS.AD"], to=V(full_no_zero), weights=V(full_no_zero)$full_no_zero)

#Saving distance on igraph object 
V(full_no_zero)$full_no_zero_dist.from.CAPSAD<-full_no_zero_dist.from.CAPSAD

# Set colors to plot distances:
oranges <- colorRampPalette(c("dark red", "gold"))
col <- oranges(max(full_no_zero_dist.from.CAPSAD)+1)
col <- col[full_no_zero_dist.from.CAPSAD+1]

#Saving as Vertex properties
V(full_no_zero)$col<-col


#Plotting distance from CAPSAD 
plot(full_no_zero, 
     layout=layout.kamada.kawai(full_no_zero), 
     vertex.color=col, 
     vertex.label=full_no_zero_dist.from.CAPSAD,
     vertex.label.color="white",
     vertex.size=sqrt(degree(full_no_zero)),
     edge.arrow.size=0.1,
     vertex.label.cex=log(degree(full_no_zero))/10,
     edge.width=log(edge.betweenness(full_no_zero, weights =E(full_no_zero)$full_no_zero))/5,
     edge.curved = TRUE
     )

#Solving Problems with legend rendering 
a<-V(full_no_zero)$full_no_zero_dist.from.CAPSAD
b<-V(full_no_zero)$col
c<-table(a,b)
d<-as.data.frame(c)
e<-subset(d, d$Freq>0)
f<-t(e$a)
g<-t(e$b)

#Adding Legend
legend(x=1, y=1.25,
       legend=as.character(f),
       pch=21,
       col = "#777777", 
       pt.bg=as.character(g),
       pt.cex=4,
       bty="n", 
       ncol=1,
       lty=1,
       cex = .8)

#Adding Title
  title("Distance from CAPS AD - full_no_zero_fancy", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
   sprintf("Avarege path length: %.2f",
     mean_distance(full_no_zero, directed=T, unconnected = T)))
```

#3.8 Reachability Function 
```{r}
#Declaring Reachability function 
reachability <- function(g, m) {
  reach_mat = matrix(nrow = vcount(g), ncol = vcount(g))
  for (i in 1:vcount(g)) {
    reach_mat[i,] = 0
    this_node_reach <- subcomponent(g, i, mode = m) # used "i" instead of "(i - 1)"
    
    for (j in 1:(length(this_node_reach))){
      alter = this_node_reach[j]# removed "+ 1"
      reach_mat[i, alter] = 1
      }
    }
  return(reach_mat)
  }
 
#Reachability
full_no_zero_reach_in <- reachability(full_no_zero, 'in')
full_no_zero_reach_out <- reachability(full_no_zero, 'out')

#in
mean(full_no_zero_reach_in[which(full_no_zero_reach_in != Inf)])
sd(full_no_zero_reach_in[which(full_no_zero_reach_in != Inf)])

#out
mean(full_no_zero_reach_out[which(full_no_zero_reach_out != Inf)])
sd(full_no_zero_reach_out[which(full_no_zero_reach_out != Inf)])
```


#3.9 Subgroups and communities
```{r}
full_no_zero_u <- as.undirected(full_no_zero, mode= "collapse",
                         edge.attr.comb=list(weight="sum", "ignore"))
```

#3.9.1
#Find cliques (complete subgraphs of an undirected graph)

Graph clique is a subset of vertices of a graph such that every two vertices in the clique are adjacent.


```{r}
#cliques_full_no_zero_u<-cliques(full_no_zero_u) # list of cliques       
#cliques_full_no_zero_u_size<-sapply(cliques(full_no_zero_u), length) # clique sizes
largest_cliques(full_no_zero_u) # cliques with max number of nodes
```

A maximum clique is a clique that cannot be extended by including one more adjacent vertex (not included in larger one). 

A maximal clique is a clique of the largest possible size in a given graph.
And, finally, graph clique number is the size of the maximum clique.

#3.9.2 Counting cliques
```{r}
#Size of largest clique 
clique_num(full_no_zero_u)
#Number of maximal cliques
count_max_cliques(full_no_zero_u)
```

```{r}
largest = largest.cliques(g) 
op = par(mfrow = c(1,2)) labels = rep(0, vcount(g))
labels[largest[[1]]] = 2 plot(g, vertex.color = labels) labels = rep(0, vcount(g)) labels[largest[[2]]] = 2 plot(g, vertex.color = labels)
```


#3.9.3 Finding the largest cliques - important to consider connectivite 
```{r}
#Generating undirected graph
as.undirected_full_no_zero_u<-as.undirected(full_no_zero_u)

#Coloring largest clique as gold and others one as gray
vcol <- rep("grey80", vcount(full_no_zero_u))
vcol[unlist(largest_cliques(full_no_zero_u))] <- "gold"

#Saving gray and gold as igraph attribute
V(as.undirected_full_no_zero_u)$vcol<-vcol

#Saving labels to display as legend
V(as.undirected_full_no_zero_u)$vcollabel <- recode(V(as.undirected_full_no_zero_u)$vcol, "'gold' = 'Largest Clique'; else= 'Others'")

#Plotting based only on degree measures 
plot(as.undirected(full_no_zero_u), 
     layout=layout.kamada.kawai(as.undirected(full_no_zero_u)), 
     vertex.label=get.vertex.attribute(as.undirected(full_no_zero_u),"LABEL_COR"),
     vertex.color=vcol,
     vertex.size=sqrt(degree(as.undirected(full_no_zero_u))),
     edge.arrow.size=0.1,
     vertex.label.cex=log(degree(as.undirected(full_no_zero_u)))/10,
     edge.width=log(edge.betweenness(as.undirected(full_no_zero_u), weights =E(as.undirected(full_no_zero_u))$full_no_zero))/5,
     edge.curved = TRUE
     )

#Solving Problems with legend rendering 
a<-V(as.undirected_full_no_zero_u)$vcollabel
b<-V(as.undirected_full_no_zero_u)$vcol
c<-table(a,b)
d<-as.data.frame(c)
e<-subset(d, d$Freq>0)
f<-t(e$a)
g<-t(e$b)

#Adding Legend
legend(x=1, y=1.25,
       legend=as.character(f),
       pch=21,
       col = "#777777", 
       pt.bg=as.character(g),
       pt.cex=4,
       bty="n", 
       ncol=1,
       lty=1,
       cex = .8)

#Adding Title
  title("Network Vertex Degree Sized - classfied by largest clique vs. others", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
   sprintf("Size of largest clique: %.1f\nNumber of maximal cliques: %.1f",
     clique_num(full_no_zero_u), 
     count_max_cliques(full_no_zero_u)
   ))
```

k-core
k-core is a maximal subset of vertices such that each is connected to at least k others in the subset.
R has a function wich calculates the coreness for each vertex. The coreness of a vertex is k if it belongs to the k-core but not to the (k+1)-core.

Now we find maximum k-core and pick out it on graph

```{r}
 coreness <- graph.coreness(z) max_cor <- max(coreness) max_cor
```

```{r}
color_bar <- heat.colors(max_cor)
plot(z, vertex.color = color_bar[coreness])
```

Network communities
Network communities are groups of vertices such that vertices inside the group connected with many more edges than between groups.

Edge betweenness Edge betweenness is equal to the number of shortest paths σst(e) from all vertices
to all others that pass through that edge e

#3.10 Betweenness Edges Newman-Girvan Edge-Betweenness
```{r}
#Edge Betweenness
E(full_no_zero)$edge.btweenness<-edge.betweenness(full_no_zero, weights =E(full_no_zero)$full_no_zero)
full_no_zero_edge.betweenness <-edge.betweenness(full_no_zero, weights =E(full_no_zero)$full_no_zero)


```

The Newman-Girvan algorithm detects communities by progressively removing edges from the original network. The Girvan-Newman algorithm focuses on edges that are most likely “between” communities.
Algorithm:
• Step 1: the betweenness of all existing edges in the network is calculated first. • Step 2: the edge with the highest betweenness is removed.
• Step 3: the betweenness of all edges affected by the removal is recalculated.
• Step 4: steps 2 and 3 are repeated until no edges remain.


```{r}
#Edge Betweenness Descriptive
summary(full_no_zero_edge.betweenness)
sd(full_no_zero_edge.betweenness)

#Edge Betweenness order - Tenth higher
E(full_no_zero)[order(full_no_zero_edge.betweenness, decreasing=T)[1:10]]
```

The best partition is selected based on modularity. There is edge.betweenness.community function in R

#3.10.1 Community detection (using edge betweenness)
```{r}
## Community structure detection based on edge betweenness
communityEdgeBetwn_full_no_zero <- edge.betweenness.community(full_no_zero)
full_no_zero_edge.betweenness.community <- edge.betweenness.community(full_no_zero)

#Creating communities
V(full_no_zero)$edge.betweenness.community <- membership(full_no_zero_edge.betweenness.community)
full_no_zero_edge.betweenness.community <- membership(full_no_zero_edge.betweenness.community)
```

#3.10.2 Plotting community by using edge betweenness
```{r}
#Plotting based only on degree measures 
plot(full_no_zero, 
     layout=layout.kamada.kawai(full_no_zero), 
     vertex.label=get.vertex.attribute(full_no_zero,"LABEL_COR"),
     vertex.size=sqrt(degree(full_no_zero)),
     mark.groups = by(seq_along(full_no_zero_edge.betweenness.community),
                      full_no_zero_edge.betweenness.community, 
                      invisible),   ## Add community indicating background colors
     edge.arrow.size=0.1,
     vertex.label.cex=log(degree(full_no_zero))/10,
     edge.width=log(edge.betweenness(full_no_zero, weights =E(full_no_zero)$full_no_zero))/5,
     edge.curved = TRUE
     )

#Solving Problems with legend rendering 
a<-V(full_no_zero)$LABEL_COR
b<-V(full_no_zero)$color
c<-table(a,b)
d<-as.data.frame(c)
e<-subset(d, d$Freq>0)
f<-t(e$a)
g<-t(e$b)

#Adding Legend
legend(x=1, y=1.25,
       legend=as.character(f),
       pch=21,
       col = "#777777", 
       pt.bg=as.character(g),
       pt.cex=4,
       bty="n", 
       ncol=1,
       lty=1,
       cex = .8)

#Adding Title
  title("Network Vertex Degree Sized - Classfied by Communities using Edges Betweenness", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
   sprintf("Median In Degree: %.2f\nMedian Out Degree: %.2f",
     median(degree(full_no_zero, mode="in")), 
     median(degree(full_no_zero, mode="out"))
   ))
```

#3.11.Community detection (Random walk)
```{r}
#Community detection (Random walk)
full_no_zero_walktrap.community <- walktrap.community(full_no_zero)
```

#3.11.1 Network Dendograma
```{r}
dend.g.network_full_no_zero <- as.dendrogram(full_no_zero_walktrap.community, se.modularity=TRUE)
plot(dend.g.network_full_no_zero, main = "Dendrograma full_no_zero", nodePar=list(pch=c(NA, 20)))
```

#3.11.2 Saving Random walk community
```{r}
V(full_no_zero)$walktrap.community <- membership(full_no_zero_walktrap.community)
full_no_zero_walktrap.community <- membership(full_no_zero_walktrap.community)
```

#3.11.3 Plotting Random walk community 
```{r}
#Plotting based only on degree measures 
plot(full_no_zero, 
     layout=layout.kamada.kawai(full_no_zero), 
     vertex.label=get.vertex.attribute(full_no_zero,"LABEL_COR"),
     vertex.size=sqrt(degree(full_no_zero)),
     mark.groups = by(seq_along(full_no_zero_walktrap.community),
                      full_no_zero_walktrap.community, 
                      invisible),   ## Add community indicating background colors
     edge.arrow.size=0.1,
     vertex.label.cex=log(degree(full_no_zero))/10,
     edge.width=log(edge.betweenness(full_no_zero, weights =E(full_no_zero)$full_no_zero))/5,
     edge.curved = TRUE
     )

#Solving Problems with legend rendering 
a<-V(full_no_zero)$LABEL_COR
b<-V(full_no_zero)$color
c<-table(a,b)
d<-as.data.frame(c)
e<-subset(d, d$Freq>0)
f<-t(e$a)
g<-t(e$b)

#Adding Legend
legend(x=1, y=1.25,
       legend=as.character(f),
       pch=21,
       col = "#777777", 
       pt.bg=as.character(g),
       pt.cex=4,
       bty="n", 
       ncol=1,
       lty=1,
       cex = .8)

#Adding Title
  title("Network Vertex Degree Sized - Classfied by Communities using Random Walk", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
   sprintf("Median In Degree: %.2f\nMedian Out Degree: %.2f",
     median(degree(full_no_zero, mode="in")), 
     median(degree(full_no_zero, mode="out"))
   ))
```




#3.12 Decomposed Graph 
```{r}
comps_full_no_zero <- decompose.graph(full_no_zero)
sapply(comps_full_no_zero, diameter)
table(sapply(comps_full_no_zero, vcount))
```

#3.12.1 Getting Plotting Decomposed Graphs

```{r}
comps_full_no_zero1<-comps_full_no_zero[[1]]
#comps_full_no_zero2<-comps_full_no_zero[[2]]
#comps_full_no_zero3<-comps_full_no_zero[[3]]
#comps_full_no_zero4<-comps_full_no_zero[[4]]
#comps_full_no_zero5<-comps_full_no_zero[[5]]
#comps_full_no_zero6<-comps_full_no_zero[[6]]
#comps_full_no_zero7<-comps_full_no_zero[[7]]
#comps_full_no_zero8<-comps_full_no_zero[[8]]
#...
```

#3.12.2 Plotting based Decomposed Graphs
```{r}
#Plotting based Decomposed Graphs
plot(comps_full_no_zero1, 
     layout=layout.kamada.kawai(comps_full_no_zero1), 
     vertex.label=get.vertex.attribute(comps_full_no_zero1,"LABEL_COR"),
     vertex.size=sqrt(degree(comps_full_no_zero1)),
     edge.arrow.size=0.1,
     vertex.label.cex=log(degree(comps_full_no_zero1))/10,
     edge.width=log(edge.betweenness(comps_full_no_zero1, weights=E(comps_full_no_zero1)$comps_full_no_zero1))/5,
     edge.curved = TRUE
     )

#Solving Problems with legend rendering 
a<-V(comps_full_no_zero1)$LABEL_COR
b<-V(comps_full_no_zero1)$color
c<-table(a,b)
d<-as.data.frame(c)
e<-subset(d, d$Freq>0)
f<-t(e$a)
g<-t(e$b)

#Adding Legend
legend(x=1, y=1.25,
       legend=as.character(f),
       pch=21,
       col = "#777777", 
       pt.bg=as.character(g),
       pt.cex=4,
       bty="n", 
       ncol=1,
       lty=1,
       cex = .8)

#Adding Title
  title("Network Vertex Degree Sized - Classfied by Communities using Decomposed", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
   sprintf("Median In Degree: %.2f\nMedian Out Degree: %.2f",
     median(degree(comps_full_no_zero1, mode="in")), 
     median(degree(comps_full_no_zero1, mode="out"))
   ))
```

#3.13 Transitivity - clustering coefficient
```{r}
# Transitivity Global
#Net is treated as an undirected network global - ratio of triangles (direction disregarded) to connected triples.

#Non-weighted 
transitivity(full_no_zero, weights=E(full_no_zero)$full_no_zero, type="global")

#Weighted
transitivity(full_no_zero, weights=E(full_no_zero)$full_no_zero,type="weighted")
transitivity(full_no_zero, weights=E(full_no_zero)$full_no_zero,type="global")


# Transitivity Local - Ratio of triangles to connected triples each vertex is part of.

#Transitivity Local - Non-weighted
V(full_no_zero)$transitivity_local<-transitivity(full_no_zero, type="local") 
transitivity_local<-transitivity(full_no_zero, type="local") #local - ratio of triangles to connected triples each vertex is part of.

#Descriptive Statistics for Local Transitivity by Vertex 
summary(transitivity_local[which(transitivity_local != Inf)])
sd(transitivity_local[which(transitivity_local != Inf)])
 

#Transitivity Barrat Weighted
V(full_no_zero)$transitivity_local_w<-transitivity(full_no_zero, type="barrat") 
transitivity_local_w<-transitivity(full_no_zero, weights=E(full_no_zero)$full_no_zero, type="barrat")

#Descriptive Statistics for Barrat Weighted Transitivity by Vertex 
summary(transitivity_local_w[which(transitivity_local_w != Inf)])
sd(transitivity_local_w[which(transitivity_local_w != Inf)])
```

#3.14 Optimal Community - too intense
```{r}
#V(full_no_zero)$optimal_comm_full_no_zero <- #membership(optimal.community(full_no_zero))
#optimal_comm_full_no_zero <- membership(optimal.community(full_no_zero))
```

#3.14 Diameter - length of the longest geodesic.
```{r}
#Diameter Non-weigthed 
diameter(full_no_zero)
full_no_zero_dim<-get.diameter(full_no_zero)
full_no_zero_dim

#Diameter Weigthed 
diameter(full_no_zero, weights=E(full_no_zero)$full_no_zero)
full_no_zero_dim<-get.diameter(full_no_zero, weights=E(full_no_zero)$full_no_zero)
full_no_zero_dim
```

#3.14.1 Plotting Diameter
```{r}
#Preparing Nodes
g<-full_no_zero
d <- get.diameter(g)

E(g)$color <- "grey"
E(g)$width <- 1
E(g, path=d)$color <- "red"
E(g, path=d)$width <- 2
V(g)$label.color <- "blue"
V(g)$color  <- "SkyBlue2"
V(g)[ d ]$label.color <- "black"
V(g)[ d ]$color <- "red"

#Plotting
plot(g, 
     layout=layout.kamada.kawai(g), 
     vertex.label.dist=0, 
     layout=layout.kamada.kawai(g),
     vertex.label=get.vertex.attribute(g,"LABEL_COR"),
     vertex.size=sqrt(degree(g)),
     edge.arrow.size=0.1,
     vertex.label.cex=log(degree(g)+1)/10,
     edge.width=edge.betweenness(g, weights=E(g)$full_no_zero)/500,
     edge.curved = TRUE
     
     )

#Solving Problems with legend rendering 
a<-V(g)$LABEL_COR
b<-V(g)$color
c<-table(a,b)
d<-as.data.frame(c)
e<-subset(d, d$Freq>0)
f.a<-t(e$a)
f.b<-t(e$b)

#Adding Legend
legend(x=1, y=1.25,
       legend=as.character(f.a),
       pch=21,
       col = "#777777", 
       pt.bg=as.character(f.b),
       pt.cex=4,
       bty="n", 
       ncol=1,
       lty=1,
       cex = .8)

#Adding Title
  title("Diameter of full_no_zero_fancy", sub = "Source: from authors ", xlab="created by igraph 0")
#Adding Stats
  text(x = -1, y = -1.2, labels =sprintf("Diameter: %.2f",diameter(g)))
```

#3.15 Maximal Independence Vertex Sets - return a set of vertex if there no edges between any two vertices in it ***Too Intense Calculation - commented code*** 
```{r}
#Maximal Independence Vertex Sets
#independence.number(full_no_zero)
```

```{r}
# Re-generate dataframes for both nodes and edges, now containing
# calculated network attributes
node_list <- get.data.frame(full_no_zero, what = "vertices")

#Writing Node CSV 
write.csv2(node_list, "~/SNArRDJF/Banco Redes R/Networks/node_full_no_zero.csv", fileEncoding = "macintosh")

# Edge List
edge_list <- get.data.frame(full_no_zero, what = "edges") %>%
  inner_join(node_list %>% select(name, walktrap.community), by = c("from" = "name")) %>%
  inner_join(node_list %>% select(name, walktrap.community), by = c("to" = "name")) %>%
  mutate(group = ifelse(walktrap.community.x == walktrap.community.y, walktrap.community.x, NA) %>% factor())

#Writing Edge List CSV
write.csv2(edge_list, "~/SNArRDJF/Banco Redes R/Networks/edge_full_no_zero.csv", fileEncoding = "macintosh")


# Create a character vector containing every node name
all_nodes <- sort(node_list$name)

# Adjust the 'to' and 'from' factor levels so they are equal
# to this complete list of node names
plot_data <- edge_list %>% mutate(
        to = factor(to, levels = all_nodes),
        from = factor(from, levels = all_nodes))

# Create the adjacency matrix plot
ggplot(plot_data, aes(x = from, y = to, fill = group)) +
      geom_raster() +
      theme_bw() +
      # Because we need the x and y axis to display every node,
      # not just the nodes that have connections to each other,
      # make sure that ggplot does not drop unused factor levels
      scale_x_discrete(drop = FALSE) +
      scale_y_discrete(drop = FALSE) +
      theme(
        # Rotate the x-axis lables so they are legible
        axis.text.x = element_text(angle = 270, hjust = 0),
        # Force the plot into a square aspect ratio
        aspect.ratio = 1,
        # Hide the legend (optional)
        legend.position = "none")


#Create a character vector of node names sorted by their
# community membership. Here, I rearrange the node_list
# table by the "comm" variable, then extract the
# "name" vector
name_order <- (node_list %>% arrange(walktrap.community))$name

# Reorder edge_list "from" and "to" factor levels based on
# this new name_order
plot_data <- edge_list %>% mutate(
        to = factor(to, levels = name_order),
        from = factor(from, levels = name_order))


# Create the adjacency matrix plot
ggplot(plot_data, aes(x = from, y = to, fill = group)) +
      geom_raster() +
      theme_bw() +
      # Because we need the x and y axis to display every node,
      # not just the nodes that have connections to each other,
      # make sure that ggplot does not drop unused factor levels
      scale_x_discrete(drop = FALSE) +
      scale_y_discrete(drop = FALSE) +
      theme(
        # Rotate the x-axis lables so they are legible
        axis.text.x = element_text(angle = 270, hjust = 0),
        # Force the plot into a square aspect ratio
        aspect.ratio = 1,
        # Hide the legend (optional)
        legend.position = "none")

# Now run the ggplot code again
```


#Breeding Reports Like Crazy
```{r}
#It only works in Unix (Linux or OSX) - Using Stream Editor from Bash
x<-"sed -e 's/full_no_zero/var0/g' ~/SNArRDJF/full_no_zero.Rmd > ~/SNArRDJF/Scripts_Original/var0.Rmd" %>% system()

x<-"sed -e 's/full_no_zero/var1/g' ~/SNArRDJF/full_no_zero.Rmd > ~/SNArRDJF/Scripts_Original/var1.Rmd" %>% system()

x<-"sed -e 's/full_no_zero/var2/g' ~/SNArRDJF/full_no_zero.Rmd > ~/SNArRDJF/Scripts_Original/var2.Rmd" %>% system()

x<-"sed -e 's/full_no_zero/var3/g' ~/SNArRDJF/full_no_zero.Rmd > ~/SNArRDJF/Scripts_Original/var3.Rmd" %>% system()

x<-"sed -e 's/full_no_zero/var4/g' ~/SNArRDJF/full_no_zero.Rmd > ~/SNArRDJF/Scripts_Original/var4.Rmd" %>% system()

x<-"sed -e 's/full_no_zero/var5/g' ~/SNArRDJF/full_no_zero.Rmd > ~/SNArRDJF/Scripts_Original/var5.Rmd" %>% system()

x<-"sed -e 's/full_no_zero/var6/g' ~/SNArRDJF/full_no_zero.Rmd > ~/SNArRDJF/Scripts_Original/var6.Rmd" %>% system()

x<-"sed -e 's/full_no_zero/var7/g' ~/SNArRDJF/full_no_zero.Rmd > ~/SNArRDJF/Scripts_Original/var7.Rmd" %>% system()

x<-"sed -e 's/full_no_zero/var8/g' ~/SNArRDJF/full_no_zero.Rmd > ~/SNArRDJF/Scripts_Original/var8.Rmd" %>% system()

x<-"sed -e 's/full_no_zero/var9/g' ~/SNArRDJF/full_no_zero.Rmd > ~/SNArRDJF/Scripts_Original/var9.Rmd" %>% system()

x<-"sed -e 's/full_no_zero/var10/g' ~/SNArRDJF/full_no_zero.Rmd > ~/SNArRDJF/Scripts_Original/var10.Rmd" %>% system()

x<-"sed -e 's/full_no_zero/var11/g' ~/SNArRDJF/full_no_zero.Rmd > ~/SNArRDJF/Scripts_Original/var11.Rmd" %>% system()

x<-"sed -e 's/full_no_zero/var12/g' ~/SNArRDJF/full_no_zero.Rmd > ~/SNArRDJF/Scripts_Original/var12.Rmd" %>% system()

x<-"sed -e 's/full_no_zero/var13/g' ~/SNArRDJF/full_no_zero.Rmd > ~/SNArRDJF/Scripts_Original/var13.Rmd" %>% system()

x<-"sed -e 's/full_no_zero/var14/g' ~/SNArRDJF/full_no_zero.Rmd > ~/SNArRDJF/Scripts_Original/var14.Rmd" %>% system()

x<-"sed -e 's/full_no_zero/var15/g' ~/SNArRDJF/full_no_zero.Rmd > ~/SNArRDJF/Scripts_Original/var15.Rmd" %>% system()

x<-"sed -e 's/full_no_zero/var16/g' ~/SNArRDJF/full_no_zero.Rmd > ~/SNArRDJF/Scripts_Original/var16.Rmd" %>% system()

x<-"sed -e 's/full_no_zero/var17/g' ~/SNArRDJF/full_no_zero.Rmd > ~/SNArRDJF/Scripts_Original/var17.Rmd" %>% system()

x<-"sed -e 's/full_no_zero/var18/g' ~/SNArRDJF/full_no_zero.Rmd > ~/SNArRDJF/Scripts_Original/var18.Rmd" %>% system()

x<-"sed -e 's/full_no_zero/var19/g' ~/SNArRDJF/full_no_zero.Rmd > ~/SNArRDJF/Scripts_Original/var19.Rmd" %>% system()

x<-"sed -e 's/full_no_zero/var20/g' ~/SNArRDJF/full_no_zero.Rmd > ~/SNArRDJF/Scripts_Original/var20.Rmd" %>% system()

x<-"sed -e 's/full_no_zero/var21/g' ~/SNArRDJF/full_no_zero.Rmd > ~/SNArRDJF/Scripts_Original/var21.Rmd" %>% system()

x<-"sed -e 's/full_no_zero/var22/g' ~/SNArRDJF/full_no_zero.Rmd > ~/SNArRDJF/Scripts_Original/var22.Rmd" %>% system()

x<-"sed -e 's/full_no_zero/var23/g' ~/SNArRDJF/full_no_zero.Rmd > ~/SNArRDJF/Scripts_Original/var23.Rmd" %>% system()

x<-"sed -e 's/full_no_zero/var24/g' ~/SNArRDJF/full_no_zero.Rmd > ~/SNArRDJF/Scripts_Original/var24.Rmd" %>% system()

x<-"sed -e 's/full_no_zero/atividade/g' ~/SNArRDJF/full_no_zero.Rmd > ~/SNArRDJF/Scripts_Original/atividade.Rmd" %>% system()

x<-"sed -e 's/full_no_zero/acesso/g' ~/SNArRDJF/full_no_zero.Rmd > ~/SNArRDJF/Scripts_Original/acesso.Rmd" %>% system()

x<-"sed -e 's/full_no_zero/confianca/g' ~/SNArRDJF/full_no_zero.Rmd > ~/SNArRDJF/Scripts_Original/confianca.Rmd" %>% system()

x<-"sed -e 's/full_no_zero/acessoeconfianca/g' ~/SNArRDJF/full_no_zero.Rmd > ~/SNArRDJF/Scripts_Original/acessoeconfianca.Rmd" %>% system()

x<-"sed -e 's/full_no_zero/total/g' ~/SNArRDJF/full_no_zero.Rmd > ~/SNArRDJF/Scripts_Original/total.Rmd" %>% system()
```

#Breeding Fancy Names Reports Like Crazy
```{r}
x<-"sed -e 's/full_no_zero_fancy/1.CENTRALIDADE_REDE COMPLETA  - TODAS RELAÇÕES EXISTENTES/g' ~/SNArRDJF/full_no_zero.Rmd >  ~/SNArRDJF/Scripts_Finais/Centralidade/1.CENTRALIDADE_REDE_COMPLETA_TODAS_RELAÇÕES_EXISTENTES" %>% system()

x<-"sed -e 's/full_no_zero_fancy/2.CENTRALIDADE_RELACIONAMENTO FORMAL/g' ~/SNArRDJF/Scripts_Original/var0.Rmd >  ~/SNArRDJF/Scripts_Finais/Centralidade/2.CENTRALIDADE_RELACIONAMENTO_FORMAL" %>% system()

x<-"sed -e 's/full_no_zero_fancy/3.CENTRALIDADE_REFERENCIA DE ENVIO/g' ~/SNArRDJF/Scripts_Original/var1.Rmd >  ~/SNArRDJF/Scripts_Finais/Centralidade/3.CENTRALIDADE_REFERENCIA_DE_ENVIO" %>% system()

x<-"sed -e 's/full_no_zero_fancy/4.CENTRALIDADE_REFERENCIA DE RECEBIMENTO CONTRAREFERENCIA/g' ~/SNArRDJF/Scripts_Original/var2.Rmd >  ~/SNArRDJF/Scripts_Finais/Centralidade/4.CENTRALIDADE_REFERENCIA_DE_RECEBIMENTO_CONTRAREFERENCIA" %>% system()

x<-"sed -e 's/full_no_zero_fancy/5.CENTRALIDADE_COORDENAÇÃO CONJUNTA DE CASOS/g' ~/SNArRDJF/Scripts_Original/var3.Rmd >  ~/SNArRDJF/Scripts_Finais/Centralidade/5.CENTRALIDADE_COORDENAÇÃO_CONJUNTA_DE_CASOS" %>% system()

x<-"sed -e 's/full_no_zero_fancy/6.CENTRALIDADE_PROGRAMAS EM CONJUNTO/g' ~/SNArRDJF/Scripts_Original/var4.Rmd >  ~/SNArRDJF/Scripts_Finais/Centralidade/6.CENTRALIDADE_PROGRAMAS_EM_CONJUNTO" %>% system()

x<-"sed -e 's/full_no_zero_fancy/7.CENTRALIDADE_OFERTA DE TREINAMENTOS CAPACITAÇÕES/g' ~/SNArRDJF/Scripts_Original/var5.Rmd >  ~/SNArRDJF/Scripts_Finais/Centralidade/7.CENTRALIDADE_OFERTA_DE_TREINAMENTOS_CAPACITAÇÕES" %>% system()

x<-"sed -e 's/full_no_zero_fancy/8.CENTRALIDADE_RECEBER TREINAMENTOS CAPACITAÇÕES/g' ~/SNArRDJF/Scripts_Original/var6.Rmd >  ~/SNArRDJF/Scripts_Finais/Centralidade/8.CENTRALIDADE_RECEBER_TREINAMENTOS_CAPACITAÇÕES" %>% system()

x<-"sed -e 's/full_no_zero_fancy/9.CENTRALIDADE_ACESSO A) Este serviço mostra-se disponível para realização de ativades em conjunto./g' ~/SNArRDJF/Scripts_Original/var7.Rmd >  ~/SNArRDJF/Scripts_Finais/Centralidade/9.CENTRALIDADE_ACESSO_A_realização_de_ativades_em_conjunto." %>% system()

x<-"sed -e 's/full_no_zero_fancy/10.CENTRALIDADE_ACESSO B) O horário de funcionamento deste serviço possibilita a realização de ações em conjunto./g' ~/SNArRDJF/Scripts_Original/var8.Rmd >  ~/SNArRDJF/Scripts_Finais/Centralidade/10.CENTRALIDADE_ACESSO_B_realização_de_ações_em_conjunto." %>% system()

x<-"sed -e 's/full_no_zero_fancy/11.CENTRALIDADE_ACESSO C) Este serviço disponibiliza informações sobre suas ativades para o seu serviço./g' ~/SNArRDJF/Scripts_Original/var9.Rmd >  ~/SNArRDJF/Scripts_Finais/Centralidade/11.CENTRALIDADE_ACESSO_C_disponibiliza_informações." %>% system()

x<-"sed -e 's/full_no_zero_fancy/12.CENTRALIDADE_ACESSO D) O seu serviço consegue acessar facilmente informações deste serviço./g' ~/SNArRDJF/Scripts_Original/var10.Rmd >  ~/SNArRDJF/Scripts_Finais/Centralidade/12.CENTRALIDADE_ACESSO_D_acessar_facilmente_informações." %>% system()

x<-"sed -e 's/full_no_zero_fancy/13.CENTRALIDADE_ACESSO E) Os canais de comunicação com este serviço são suficientes para realização de ações em conjunto./g' ~/SNArRDJF/Scripts_Original/var11.Rmd >  ~/SNArRDJF/Scripts_Finais/Centralidade/13.CENTRALIDADE_ACESSO_E_canais_de_comunicação_suficientes." %>% system()

x<-"sed -e 's/full_no_zero_fancy/14.CENTRALIDADE_ACESSO F) Seu serviço tem facilade de entrar em contato com este serviço para realizar ativades em conjunto./g' ~/SNArRDJF/Scripts_Original/var12.Rmd >  ~/SNArRDJF/Scripts_Finais/Centralidade/14.CENTRALIDADE_ACESSO_F_entrar_em_contato_ara_realizar_ativades" %>% system()

x<-"sed -e 's/full_no_zero_fancy/15.CENTRALIDADE_ACESSO G) O acesso geográfico a este serviço impede a realização de ativades em conjunto. /g' ~/SNArRDJF/Scripts_Original/var13.Rmd >  ~/SNArRDJF/Scripts_Finais/Centralidade/15.CENTRALIDADE_ACESSO_G_O_acesso_geográfico_impede" %>% system()

x<-"sed -e 's/full_no_zero_fancy/16.CENTRALIDADE_ACESSO H) O custo para acessar este serviço impede a realização de ativades em conjunto./g' ~/SNArRDJF/Scripts_Original/var14.Rmd >  ~/SNArRDJF/Scripts_Finais/Centralidade/16.CENTRALIDADE_ACESSO_H_O_custo_para_acessar" %>% system()

x<-"sed -e 's/full_no_zero_fancy/17.CENTRALIDADE_ACESSO I) O tempo gasto para acessar este serviço impede a realização ativades em conjunto./g' ~/SNArRDJF/Scripts_Original/var15.Rmd >  ~/SNArRDJF/Scripts_Finais/Centralidade/17.CENTRALIDADE_ACESSO_I_O_tempo_gasto_para_acessar_este_serviço_" %>% system()

x<-"sed -e 's/full_no_zero_fancy/18.CENTRALIDADE_CONFIANÇA A) Confia na qualificação profissional da equipe deste serviço /g' ~/SNArRDJF/Scripts_Original/var16.Rmd >  ~/SNArRDJF/Scripts_Finais/Centralidade/18.CENTRALIDADE_CONFIANÇA_A_Confia_na_qualificação_profissional_" %>% system()

x<-"sed -e 's/full_no_zero_fancy/19.CENTRALIDADE_CONFIANÇA B) Confia na capacade técnica da equipe deste serviço./g' ~/SNArRDJF/Scripts_Original/var17.Rmd >  ~/SNArRDJF/Scripts_Finais/Centralidade/19.CENTRALIDADE_CONFIANÇA_B_Confia_na_capacade_técnica_" %>% system()

x<-"sed -e 's/full_no_zero_fancy/20.CENTRALIDADE_CONFIANÇA C) Confia nas ações realizadas por este serviço./g' ~/SNArRDJF/Scripts_Original/var18.Rmd >  ~/SNArRDJF/Scripts_Finais/Centralidade/20.CENTRALIDADE_CONFIANÇA_C_Confia_nas_ações_realizadas_" %>% system()

x<-"sed -e 's/full_no_zero_fancy/21.CENTRALIDADE_CONFIANÇA D) Confia em compartilhar informações internas com este serviço. /g' ~/SNArRDJF/Scripts_Original/var19.Rmd >  ~/SNArRDJF/Scripts_Finais/Centralidade/21.CENTRALIDADE_CONFIANÇA_D_Confia_em_compartilhar_informações_" %>% system()

x<-"sed -e 's/full_no_zero_fancy/22.CENTRALIDADE_CONFIANÇA E) Confia nas informações recebas deste serviço./g' ~/SNArRDJF/Scripts_Original/var20.Rmd >  ~/SNArRDJF/Scripts_Finais/Centralidade/22.CENTRALIDADE_CONFIANÇA_E_Confia_nas_informações_recebidas." %>% system()

x<-"sed -e 's/full_no_zero_fancy/23.CENTRALIDADE_CONFIANÇA F) Confia em realizar ações conjuntas com este serviço./g' ~/SNArRDJF/Scripts_Original/var21.Rmd >  ~/SNArRDJF/Scripts_Finais/Centralidade/23.CENTRALIDADE_CONFIANÇA_F_Confia_em_realizar_ações_conjuntas" %>% system()

x<-"sed -e 's/full_no_zero_fancy/24.CENTRALIDADE_CONFIANÇA G) Está satisfeito com as ações desempenhadas por este serviço./g' ~/SNArRDJF/Scripts_Original/var22.Rmd >  ~/SNArRDJF/Scripts_Finais/Centralidade/24.CENTRALIDADE_CONFIANÇA_G_Está_satisfeito_com_as_ações_desempenhadas" %>% system()

x<-"sed -e 's/full_no_zero_fancy/25.CENTRALIDADE_CONFIANÇA H) Espera que o relacionamento com este serviço continue nos próximos anos./g' ~/SNArRDJF/Scripts_Original/var23.Rmd >  ~/SNArRDJF/Scripts_Finais/Centralidade/25.CENTRALIDADE_CONFIANÇA_H_Relacionamento_continue_nos_próximos_anos." %>% system()

x<-"sed -e 's/full_no_zero_fancy/26.CENTRALIDADE_CONFIANÇA I) Espera que o relacionamento com este serviço se fortaleça nos próximos anos./g' ~/SNArRDJF/Scripts_Original/var24.Rmd >  ~/SNArRDJF/Scripts_Finais/Centralidade/26.CENTRALIDADE_CONFIANÇA_I_Relacionamento_se_fortaleça_nos_próximos_anos." %>% system()

x<-"sed -e 's/full_no_zero_fancy/27.CENTRALIDADE_ATIVIDADE/g' ~/SNArRDJF/Scripts_Original/atividade.Rmd >  ~/SNArRDJF/Scripts_Finais/Centralidade/27.CENTRALIDADE_ATIVIDADE" %>% system()

x<-"sed -e 's/full_no_zero_fancy/28.CENTRALIDADE_ACESSO/g' ~/SNArRDJF/Scripts_Original/acesso.Rmd >  ~/SNArRDJF/Scripts_Finais/Centralidade/28.CENTRALIDADE_ACESSO" %>% system()

x<-"sed -e 's/full_no_zero_fancy/29.CENTRALIDADE_CONFIANÇA/g' ~/SNArRDJF/Scripts_Original/confianca.Rmd >  ~/SNArRDJF/Scripts_Finais/Centralidade/29.CENTRALIDADE_CONFIANÇA" %>% system()

x<-"sed -e 's/full_no_zero_fancy/30.CENTRALIDADE_ACESSO E CONFIANÇA/g' ~/SNArRDJF/Scripts_Original/acessoeconfianca.Rmd >  ~/SNArRDJF/Scripts_Finais/Centralidade/30.CENTRALIDADE_ACESSO_E_CONFIANÇA" %>% system()

x<-"sed -e 's/full_no_zero_fancy/31.CENTRALIDADE_TOTAL/g' ~/SNArRDJF/Scripts_Original/total.Rmd >  ~/SNArRDJF/Scripts_Finais/Centralidade/31.CENTRALIDADE_TOTAL" %>% system()
```


#4. Saving objects with new variables and changes
```{r}
save.image("~/SNArRDJF/2.SNA.RData") 
load("~/SNArRDJF/2.SNA.RData")# just in case 
```



