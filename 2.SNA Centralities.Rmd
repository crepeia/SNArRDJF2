---
title: "SNA Centralities full_no_zero*"
...
topnamesPB
#I need to double check this result as we would expect CAPS AD top #1
```
...
plot(make_ego_graph_out_full_no_zero, 
    ...
    vertex.frame.color="#ffffff",
     vertex.label.color="black",
     vertex.label.cex=log(degree(make_ego_graph_out_full_no_zero)+1)/10,
...
plot(make_ego_graph_all_full_no_zero_without_CAPSAD, 
     ...
     vertex.size=100*page.rank(make_ego_graph_all_full_no_zero_without_CAPSAD)$vector,
     edge.arrow.size=0.1,
     vertex.frame.color="#ffffff",
      vertex.label.color="black",
     vertex.label.cex=log(page.rank(make_ego_graph_all_full_no_zero_without_CAPSAD)$vector+1)*10,
...
plot(make_ego_graph_all_full_no_zero_without_complex, 
     ...
     edge.arrow.size=0.1,
     vertex.label.cex=log(page.rank(make_ego_graph_all_full_no_zero_without_complex)$vector+1)/10,
...
#Plotting based only on degree measures 
plot(msp_full_no_zero, 
     layout=layout.auto(msp_full_no_zero), 
     vertex.label=get.vertex.attribute(msp_full_no_zero,"LABEL_COR"),
     vertex.size=sqrt(degree(msp_full_no_zero)),
     edge.arrow.size=0.1,
     vertex.label.cex=sqrt(degree(msp_full_no_zero))/50,
...
#Adding Title
  title("Alternative Visualization -Minimum Spanning Tree Non-normalized - from full_no_zero_fancy", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
   sprintf("Median In Degree: %.2f\nMedian Out Degree: %.2f",
     median(degree(msp_full_no_zero, mode="in")), 
     median(degree(msp_full_no_zero, mode="out"))
...
plot(msp_full_no_zero_w, 
     ...
     edge.width=1/edge.betweenness(msp_full_no_zero_w, weights =E(msp_full_no_zero_w)$msp_full_no_zero_w)*2,
     edge.curved = TRUE
     )
...
#3.9.2.1 Plotting Largest Cliques - It needs to be improved and I'm not sure if it can handle nice information - check the bellow graph
```{r}
#Find largest clique
largest = largest.cliques(full_no_zero_u)

#Create Label for first one
labels1 = rep(0, vcount(full_no_zero_u))
labels1[largest[[1]]] = 2 

#Create Label for second one
labels2 = rep(0,vcount(full_no_zero_u)) 
labels2[largest[[2]]] = 2 

op = par(mfrow = c(1,2)) 
#Plotting based only on degree measures 
plot(full_no_zero_u, 
     layout=layout.kamada.kawai(full_no_zero_u), 
     vertex.label=get.vertex.attribute(full_no_zero,"LABEL_COR"),
     vertex.color = labels1,
     vertex.size=degree(full_no_zero_u)/2,
     vertex.frame.color="#ffffff",
     edge.arrow.size=degree(full_no_zero_u)/10000,
     vertex.label.cex=degree(full_no_zero_u)/600,
     edge.width=(edge.betweenness(full_no_zero_u, weights =E(full_no_zero)$full_no_zero))/500,
     edge.curved = TRUE
     )
  #Plotting based only on degree measures 
plot(full_no_zero_u, 
     layout=layout.kamada.kawai(full_no_zero_u), 
     vertex.label=get.vertex.attribute(full_no_zero,"LABEL_COR"),
     vertex.color = labels2,
     vertex.size=degree(full_no_zero_u)/2,
     vertex.frame.color="#ffffff",
     edge.arrow.size=degree(full_no_zero_u)/10000,
     vertex.label.cex=degree(full_no_zero_u)/600,
     edge.width=(edge.betweenness(full_no_zero_u, weights =E(full_no_zero)$full_no_zero))/5000,
     edge.curved = TRUE
     )
par(op)
```
...
plot(full_no_zero, vertex.color = color_bar[coreness])
```
