...
#1. Descriptive Analysis for var6* - Complete Graph - All networks were generated using from this one
...
hist(degree(var6, mode = "in", normalized = F), ylab="Frequency", xlab="Degree",  breaks=vcount(var6)/10, main="Histogram of Indegree Nodes - var6*")
hist(degree(var6, mode = "out", normalized = F), ylab="Frequency", xlab="Degree",  breaks=vcount(var6)/10, main="Histogram of Outdegree Nodes - var6*")
hist(degree(var6, mode = "all", normalized = F), ylab="Frequency", xlab="Degree",  breaks=vcount(var6)/10, main="Histogram of All Degree Nodes - var6*")
```  
...
```{r}
V(var6)$strength_var6<- strength(var6, weights=E(var6)$var6)
strength_var6<- strength(var6, weights=E(var6)$var6)

##Stats
summary(strength_var6)
sd(strength_var6)
```
...
#Histogram var6 degree
hist(degree(var6, mode = "in", normalized = F), ylab="Frequency", xlab="Normalized Degree",  breaks=vcount(var6)/10, main="Histogram of Normalized Indegree Nodes - var6*")
hist(degree(var6, mode = "out", normalized = F), ylab="Frequency", xlab="Normalized Degree",  breaks=vcount(var6)/10, main="Histogram of Normalized Indegree Nodes - var6*")
hist(degree(var6, mode = "all", normalized = F), ylab="Frequency", xlab="Normalized Degree",  breaks=vcount(var6)/10, main="Histogram of Normalized All Degree Nodes - var6*")
```

#2.2.1 Centralization Degree
```{r}
V(var6)$centr_degree_var6 <- centralization.degree(var6)$res
centr_degree_var6 <- centralization.degree(var6)

#Centralization
centr_degree_var6$centralization
#Theoretical Max
centr_degree_var6$theoretical_max
```
...
hist(degree.distribution(var6), breaks=vcount(var6)/10, ylab="Frequency", xlab="Degree Distribuition", main="Histogram of Degree Distribuition - var6*")

#Degree Probability Distribution
dd <- degree.distribution(var6, cumulative=T, mode="all")
plot(dd, pch=19, cex=1, col="orange", xlab="Degree", ylab="Cumulative Frequency", main= "Cumulative Frequency of var6* ")
```
...
plot(d[ind], 
     ...
     main="Log-Log Degree Distribution For var6*"
     )
```

#2.5.1 Average Neighbor Degree versus Vertex Degree (log-log scale for var6*)
```{r}
#Average Neighbor Degree versus vertex degree (log-log scale for var6)
a.nn.deg.var6 <- graph.knn(var6,V(var6))$knn

#Saving to igraph object
V(var6)$a.nn.deg.var6 <- a.nn.deg.var6 <- graph.knn(var6,V(var6))$knn
...
plot(degree(var6), 
     ...
     main="Average Neighbor Degree vs Vertex Degree - Log-Log Scale for var6*"
     )
```

#2.5.2 Average Weighted Neighbor Degree versus vertex degree (log-log scale for weighted var6*)
```{r}
#Average Weighted Neighbor Degree versus vertex degree (log-log scale for var6)
a.nn.deg.var6_w <- graph.knn(var6,V(var6), weights=E(var6)$var6)$knn

#Saving to igraph object
V(var6)$a.nn.deg.var6_w <-a.nn.deg.var6 <- graph.knn(var6,V(var6), weights=E(var6)$var6)$knn

#Average Weighted Neighbor Descriptive
summary(a.nn.deg.var6_w)
sd(a.nn.deg.var6_w)
...
plot(degree(var6), 
     ...
     xlab=c("Log Vertex Degree"),
     ylab=c("Log Average Neighbor Degree"),
     main="Average Weighted Neighbor Degree vs Vertex Degree - Log-Log Scale For Weighted var6*"
     )
```
...
#Adding Title
  title("Network Vertex Degree Sized - var6*", sub = "Source: from authors ")
...
```{r}
#Get Variable
V(var6)$var6_color_degree<-V(var6)$totaldegree
...
#Adding Title
  title("Network Vertex Degree Sized and Colored - var6*", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
   sprintf("Median In Degree: %.2f\nMedian Out Degree: %.2f",
     median(degree(var6, mode="in")), 
     median(degree(var6, mode="out"))
...
```{r}
#Get Variable
V(var6)$var6_color_degree<-V(var6)$centr_degree_var6
...
plot(small_var6, 
     ...
     vertex.frame.color="#ffffff",
     edge.arrow.size=0.1,
     vertex.label.cex=log(degree(small_var6))/10,
...
#Adding Title
  title("Network Higher Than Median Degree - var6*", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
   sprintf("Median In Degree: %.2f\nMedian Out Degree: %.2f",
...
plot(small_var6, 
     ...
     vertex.label.cex=log(degree(small_var6)+2)/2,
...
#Adding Title
  title("Network Lower than Median Degree - var6*", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
   sprintf("Median In Degree: %.2f\nMedian Out Degree: %.2f",
     median(degree(small_var6, mode="in")), 
     median(degree(small_var6, mode="out"))
...
```{r}
#Plotting based only on degree measures  #a.nn.deg.var6_w 
plot(var6, 
     layout=layout.kamada.kawai(var6), 
     vertex.label=get.vertex.attribute(var6,"LABEL_COR"),
     ...
     vertex.frame.color="#ffffff",
     edge.arrow.size=0.1,
     vertex.label.cex=sqrt(graph.knn(var6,V(var6))$knn)/20,
     edge.width=(edge.betweenness(var6, weights =E(var6)$var6))/500,
     edge.curved = TRUE
     )
...
#Adding Title
  title("Network Average Neighbor Degree Sized - var6*", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
...
plot(var6, 
     layout=layout.kamada.kawai(var6), 
     vertex.label=get.vertex.attribute(var6,"LABEL_COR"),
     vertex.size=sqrt(graph.knn(var6,V(var6), weights= E(var6)$var6)$knn),
     vertex.frame.color="#ffffff",
     edge.arrow.size=0.1,
     vertex.label.cex=log(graph.knn(var6,V(var6), weights= E(var6)$var6)$knn)/10,
     edge.width=edge.betweenness(var6, weights =E(var6)$var6)/500,
     edge.curved = TRUE
     )
...
#Adding Title
  title("Network Average Weighted Neighbor Degree Sized - var6*", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
   sprintf("Median Average Weighte Neighbor Degree: %.2f",
     median(graph.knn(var6,V(var6), weights= E(var6)$var6)$knn)
   ))
```
...
#Saving to Igraph object
V(var6)$incloseness_w <- closeness(var6, mode = "in", normalized = T, weights=E(var6)$var6) #%>% round(6)
V(var6)$outcloseness_w <- closeness(var6, mode = "out", normalized = T,weights=E(var6)$var6) #%>% round(6)
V(var6)$totalcloseness_w <- closeness(var6, mode = "total", normalized = T, weights=E(var6)$var6) #%>% round(6)

#Saving to Environment
var6_incloseness_w<- closeness(var6, mode = "in", normalized = T, weights=E(var6)$var6) #%>% round(6)
var6_outcloseness_w<- closeness(var6, mode = "out", normalized = T, weights=E(var6)$var6) #%>% round(6)
var6_totalcloseness_w<- closeness(var6, mode = "total", normalized = T, weights=E(var6)$var6) #%>% round(6)
...
```{r}
V(var6)$closeness_stats_var6_w <- closeness(var6, vids = V(var6), mode = "all", weights =V(var6)$var6, normalized = FALSE)

closeness_stats_var6_w <- closeness(var6, vids = V(var6), mode = "all", weights =V(var6)$var6, normalized = FALSE)
```

#2.7.5 Centralization Closseness
```{r}
V(var6)$centr_closeness_var6 <- centralization.closeness(var6)$res
centr_closeness_var6 <- centralization.closeness(var6)

#Centralization
centr_closeness_var6$centralization
#Theoretical Max
centr_closeness_var6$theoretical_max
```
...
#Adding Title
  title("Network Closeness Degree Sized and Colored - var6*", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
...
```{r}
#Get Variable
V(var6)$var6_color_degree<-round(V(var6)$centr_closeness_var6,3)
...
#Adding Title
  title("Network Centralization Closeness - var6*", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
   sprintf("Median In Closennes: %.4f\nMedian Out Closennes: %.4f",
     median(closeness(var6, mode="in")), 
     median(closeness(var6, mode="out"))
...
```{r}
V(var6)$betweenness_var6_w <- betweenness(var6, weights=E(var6)$var6) %>% round(1)

betweenness_var6_w <- betweenness(var6, weights=E(var6)$var6) %>% round(1)

##all
summary(betweenness_var6_w)
sd(betweenness_var6_w)
```

#2.8.3 Centralization Betweenness
```{r}
V(var6)$centr_betweenness_var6 <- centralization.betweenness(var6)$res
centr_betweenness_var6 <- centralization.betweenness(var6)

#Centralization
centr_betweenness_var6$centralization
#Theoretical Max
centr_betweenness_var6$theoretical_max
```
...
plot(var6, 
     ...
     edge.width=log(edge.betweenness(var6, weights =E(var6)$var6))/50,
     edge.curved = TRUE
     )
...
#Adding Title
  title("Network Vertex Betweenness Sized - var6*", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
...
```{r}
#Get Variable
V(var6)$var6_color_degree<-round(V(var6)$centr_betweenness_var6,3)
...
#Adding Title
  title("Network Centralization Betweenness - var6*", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
...
```{r}
#Eigen_Centrality 
V(var6)$eigenvector <- eigen_centrality(var6, directed = TRUE, weights = V(var6)$var6) %$% vector %>% round(3)

var6_eigenvector <- eigen_centrality(var6, directed = TRUE, weights = V(var6)$var6) %$% vector %>% round(3)

#Eigenvalue Calculated Eigenvector
var6_eigenvector_value <- eigen_centrality(var6, directed = TRUE, weights = V(var6)$var6) %$% value %>% round(3) # The eigenvalue corresponding to the calculated eigenvector, i.e. the centrality scores.
```
...
```{r}
V(var6)$centr_eigen_var6 <- centr_eigen(var6, directed = T)$vector
centr_eigen_var6 <-centr_eigen(var6, directed = T)$vector
#Centralization
...
plot(var6, 
     layout=layout.kamada.kawai(var6), 
     vertex.label=get.vertex.attribute(var6,"LABEL_COR"),
     vertex.size=(eigen_centrality(var6)$vector)*20,
     vertex.frame.color="#ffffff",
     edge.arrow.size=0.1,
     vertex.label.cex=sqrt(eigen_centrality(var6)$vector)/2,
     edge.width=edge.betweenness(var6, weights =E(var6)$var6)/500,
     edge.curved = TRUE
     )
...
#Adding Title
  title("Eigenvector Centrality Measure - var6*", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
...
plot(var6, 
     ...
     vertex.frame.color="#ffffff",
     edge.arrow.size=0.1,
     vertex.label.cex=(centr_eigen(var6, directed = T)$vector + 1)/10,
     edge.width=(edge.betweenness(var6, weights =E(var6)$var6))/500,
     edge.curved = TRUE
     )
...
power_centrality_stats_var6 <- power_centrality(var6, nodes = V(var6), loops = FALSE, exponent = 1, rescale = FALSE, tol = 1e-07, sparse = TRUE)

#Saving power as an igraph attributte
V(var6)$power_centrality_stats_var6 <-power_centrality(var6, nodes = V(var6), loops = FALSE, exponent = 1, rescale = FALSE, tol = 1e-07, sparse = TRUE) 

#Or are you more powerful if you're connected to weak, dependent people? (negative attenuation factor)
i=sort(power_centrality_stats_var6) %>% round(3)
table_i<-cbind(V(var6)$V5_LABEL_LABEL,i)
...
d=sort(power_centrality_stats_var6, decreasing = T) %>% round(3)
table_d<-cbind(V(var6)$V5_LABEL_LABEL,d)
...
V(var6)$power_centrality_stats_var6_r<-round(V(var6)$power_centrality_stats_var6,0)

# Creating brewer pallette
vertex_col_spower <- colorRampPalette(brewer.pal(length(unique(V(var6)$power_centrality_stats_var6_r)), "Blues"))(length(unique(V(var6)$power_centrality_stats_var6_r))) 

#Saving as Vertex properties                                   
V(var6)$color_spower <- vertex_col_spower[factor(V(var6)$power_centrality_stats_var6_r)]  
```
...
#Solving Problems with legend rendering 
a<-V(var6)$power_centrality_stats_var6_r
...
#Adding Title
  title("Bonacich Power Centrality Scores of Network Positions - var6*", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
   sprintf("Median Power Degree: %.2f\n SD Power Degree:%.2f",
     median(power_centrality_stats_var6),sd(power_centrality_stats_var6)
     ))
```
...
```{r, results="asis"}
#Saving power as an object
alpha_centrality_stats_var6 <- alpha_centrality(var6, loops = FALSE, alpha = 1, exo = 1, weights = V(var6)$var6, tol = 1e-07, sparse = TRUE)

#Saving power as an igraph attributte
V(var6)$alpha_centrality_stats_var6<-alpha_centrality(var6, loops = FALSE, alpha = 1, exo = 1, weights = V(var6)$var6, tol = 1e-07, sparse = TRUE)

#Descriptive 
summary(alpha_centrality_stats_var6)
sd(alpha_centrality_stats_var6)

#Or are you more powerful if you're connected to weak, dependent people? (negative attenuation factor)
i=sort(alpha_centrality_stats_var6) %>% round(3)
table_i<-cbind(V(var6)$V5_LABEL_LABEL,i)
...
#Are you more powerful if you're connected to powerful people? (positive attenuation factor)
d=sort(alpha_centrality_stats_var6, decreasing = T) %>% round(3)
table_d<-cbind(V(var6)$V5_LABEL_LABEL,d)
colnames(table_d) <- c("ID", "Power")
table_d10<-table_d[1:10,]
...
```{r}
# Set colors to plot distances:
V(var6)$alpha_centrality_stats_var6_r<-round(V(var6)$alpha_centrality_stats_var6,0)

# Creating brewer pallette
vertex_col_s <- colorRampPalette(brewer.pal(length(unique(V(var6)$alpha_centrality_stats_var6_r)), "YlOrRd"))(length(unique(V(var6)$alpha_centrality_stats_var6_r))) 

#Saving as Vertex properties                                                                  
V(var6)$color_s <- vertex_col_s[factor(V(var6)$alpha_centrality_stats_var6_r)]  
```
...
plot(var6, 
     ...
     vertex.label.color="black",
     vertex.frame.color="#ffffff",
     vertex.size=sqrt((alpha_centrality(var6, weights = V(var6)$var6)^2)),
     edge.arrow.size=0.1,
...
#Solving Problems with legend rendering 
a<-V(var6)$alpha_centrality_stats_var6_r
b<-V(var6)$color_s
c<-table(a,b)
d<-as.data.frame(c)
e<-subset(d, d$Freq>0)
...
#Adding Title
  title("Bonacich's Alpha Centrality Scores of Network Positions - var6*", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
   sprintf("Median Alpha Degree: %.2f\n SD Alpha Degree:%.2f",
     median(alpha_centrality_stats_var6),sd(alpha_centrality_stats_var6)
     ))
```
...
var6_hub<- hub.score(var6)$vector
...
plot(var6, 
     layout=layout.kamada.kawai(var6), 
     vertex.label=get.vertex.attribute(var6,"LABEL_COR"),
     vertex.size=10*sqrt(hub.score(var6)$vector),
     edge.arrow.size=0.1,
     vertex.label.cex=log(degree(var6))/10,
     edge.width=log(edge.betweenness(var6, weights =E(var6)$var6))/5,
     edge.curved = TRUE)
...
#Adding Title
  title("Network Hub Sized - var6*", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
...
var6_auth <- authority.score(var6)$vector
#stats
summary(var6_auth)
sd(var6_auth)
...
plot(var6, 
     layout=layout.kamada.kawai(var6), 
     vertex.label=get.vertex.attribute(var6,"LABEL_COR"),
     vertex.size=10*sqrt(authority.score(var6)$vector),
     edge.arrow.size=0.1,
     vertex.label.cex=log(degree(var6))/10,
     edge.width=log(edge.betweenness(var6, weights =E(var6)$var6))/5,
     edge.curved = TRUE)
...
#Adding Title
  title("Network Authorities Scores  Sized - var6*", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
...
plot(var6, 
     layout=layout.kamada.kawai(var6), 
     vertex.label=get.vertex.attribute(var6,"LABEL_COR"),
     vertex.size=30*sqrt(page.rank(var6)$vecto),
     edge.arrow.size=0.1,
     vertex.label.cex=log(degree(var6))/10,
     edge.width=log(edge.betweenness(var6, weights =E(var6)$var6))/100,
     edge.curved = TRUE)
...
#Adding Title
  title("Network Google Page Rank Sized - var6*", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
...
```{r}
#For in-degrees:
indegnamesPB=which(indegPB>sort(indegPB)[vcount(var6)-5]) #For out- degrees:
outdegnamesPB=which(outdegPB>sort(outdegPB)[vcount(var6)-5]) #For PageRank:
prnamesPB=which(prPB>sort(prPB)[vcount(var6)-5]) #For Hubs:
hnamesPB=which(hPB>sort(hPB)[vcount(var6)-5]) #For Authorities:
authnamesPB=which(authPB>sort(authPB)[vcount(var6)-5])
...
plot(make_ego_graph_all_var6, 
     ...
     edge.arrow.size=0.1,
     vertex.label.cex=log(degree(make_ego_graph_all_var6))/10,
...
#Adding Title
  title("Ego Network Page Rank Sized - All connections from CAPSAD - var6*", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
   sprintf("Mean Page Rank Scores: %.2f\n SD Page Rank Scores: %.2f",
...
plot(make_ego_graph_out_var6, 
     ...
     vertex.size=30*sqrt(page.rank(make_ego_graph_out_var6)$vector),
     edge.arrow.size=0.1,
     vertex.label.cex=log(degree(make_ego_graph_out_var6))/10,
...
#Adding Title
  title("Ego Network Page Rank Sized - Out Connections from CAPSAD - var6*", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
   sprintf("Mean Page Rank Scores: %.2f\n SD Page Rank Scores: %.2f",
...
plot(make_ego_graph_in_var6, 
     ...
     vertex.size=30*sqrt(page.rank(make_ego_graph_in_var6)$vector),
     edge.arrow.size=0.1,
...
#Adding Title
  title("Ego Network CAPS AD Page Rank Sized - In connections from CAPS AD - var6*", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
   sprintf("Mean Page Rank Scores: %.2f\n SD Page Rank Scores: %.2f",
...
plot(make_ego_graph_all_var6_without_CAPSAD, 
     ...
     vertex.size=30*sqrt(page.rank(make_ego_graph_all_var6_without_CAPSAD)$vector),
     edge.arrow.size=0.1,
     vertex.label.cex=log(degree(make_ego_graph_all_var6_without_CAPSAD))/10,
     edge.width=log(edge.betweenness(make_ego_graph_all_var6_without_CAPSAD))/10,
     edge.curved = TRUE)
...
#Adding Title
  title("Ego Network CAPS AD Page Rank Sized - All Connections Removing CAPS AD - var6*", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
   sprintf("Mean Page Rank Scores: %.2f\n SD Page Rank Scores: %.2f",
...
plot(make_ego_graph_all_var6_without_complex, 
     ...
     edge.arrow.size=0.1,
     vertex.label.cex=log(degree(make_ego_graph_all_var6_without_complex))/10,
     edge.width=log(edge.betweenness(make_ego_graph_all_var6_without_complex))/10,
     edge.curved = TRUE)
...
#Adding Title
  title("Ego Network CAPS AD Page Rank Sized - All Connections Removing High Complex Service - var6*", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
   sprintf("Mean Page Rank Scores: %.2f\n SD Page Rank Scores: %.2f",
...
#Adding Title
  title("Minimum Spanning Tree Non-normalized - from var6*", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
   sprintf("Median In Degree: %.2f\nMedian Out Degree: %.2f",
...
msp_var6_w<-minimum.spanning.tree(var6, weights=1/edge.betweenness(var6, weights =E(var6)$var6), algorithm='prim')
...
#Plotting based only on degree measures 
plot(msp_var6_w, 
     layout=layout.kamada.kawai(msp_var6_w), 
     ...
     edge.width=1/edge.betweenness(msp_var6_w, weights =E(msp_var6_w)$msp_var6_w),
     edge.curved = TRUE
     )
...
#Adding Title
  title("Minimum Weighted Spanning Tree (Edge Betweenness) - var6*", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
   sprintf("Median In Degree: %.2f\nMedian Out Degree: %.2f",
...
is.connected(var6) #trick information - let's check this out further
```

#3.3.1 Strong Connected Components
```{r}
#Strong Connected
components_strong_var6<-components(var6, mode="strong")
V(var6)$components_strong_var6_membership<-components_strong_var6$membership # adding to igraph object

#Number of clusters
count_components(var6)

#Display components structure
components_strong_var6
```
...
plot(var6, 
     ...
     mark.groups = by(seq_along(components_strong_var6$membership),
                      components_strong_var6$membership, 
...
#Adding Title
  title("Network Vertex Degree Sized - Classfied by Components Structure - var6*", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
   sprintf("Median In Degree: %.2f\nMedian Out Degree: %.2f",
     median(degree(var6, mode="in")), 
     median(degree(var6, mode="out"))
...
```{r}
#Shortest Paths
var6_sp_in <- shortest.paths(var6, mode='in', weights=V(var6)$var6) # sames as bellow
var6_sp_out <- shortest.paths(var6, mode='out', weights=V(var6)$var6) #same as above
var6_sp_all <- shortest.paths(var6, mode='all', weights=V(var6)$var6) # all
...
```{r}
#All shortest paths 
distances_sp_all_var6<-distances(var6, mode="all", weights=E(var6)$var6)
#distances_sp_all_var6

#In shortest paths 
distances_sp_in_var6<-distances(var6, mode="in",weights=E(var6)$var6)
#distances_sp_in_var6

#Out shortest paths 
distances_sp_out_var6<-distances(var6, mode="out", weights=E(var6)$var6)
#distances_sp_out_var6
```

#3.7.4 Histogram from shortest path length between each pair of vertices. 
```{r}
distance_table(var6, directed = TRUE)
```
...
var6_dist.from.CAPSAD <- distances(var6, v=V(var6)[V2_LABEL_ID=="q170_CAPS...CAPS.AD"], to=V(var6), weights=V(var6)$var6)
...
#Adding Title
  title("Distance from CAPS AD - var6*", sub = "Source: from authors ")
  text(x = -1, y = -1.2, labels = 
...
cliques_var6_u_size<-sapply(cliques(var6_u), length) # clique sizes
largest_cliques(var6_u) # cliques with max number of nodes
```
...
V(as.undirected_var6_u)$vcollabel <- recode(V(as.undirected_var6_u)$vcol, "'gold' = 'Largest Clique'; else= 'Others'")
...
plot(as.undirected(var6_u), 
     ...
     edge.arrow.size=0.1,
     vertex.label.cex=log(degree(as.undirected(var6_u)))/10,
     edge.width=log(edge.betweenness(as.undirected(var6_u), weights =E(as.undirected(var6_u))$var6))/5,
     edge.curved = TRUE
     )
...
```{r}
#Edge Betweenness
E(var6)$edge.btweenness<-edge.betweenness(var6, weights =E(var6)$var6)
...
E(var6)[order(var6_edge.betweenness, decreasing=T)[1:10]]
```

#3.10.1 Community detection (using edge betweenness)
```{r}
## Community structure detection based on edge betweenness
communityEdgeBetwn_var6 <- edge.betweenness.community(var6)
...
plot(comps_var61, 
     ...
     edge.arrow.size=0.1,
     vertex.label.cex=log(degree(comps_var61))/10,
...
#Non-weighted 
transitivity(var6, weights=E(var6)$var6, type="global")

#Weighted
transitivity(var6, weights=E(var6)$var6,type="weighted")
transitivity(var6, weights=E(var6)$var6,type="global")
...
transitivity_local_w<-transitivity(var6, weights=E(var6)$var6, type="barrat")
...
#Diameter Weigthed 
diameter(var6, weights=E(var6)$var6)
var6_dim<-get.diameter(var6, weights=E(var6)$var6)
var6_dim
```
...
#Adding Title
  title("Diameter of var6*", sub = "Source: from authors ", xlab="created by igraph 0")
#Adding Stats
...
node_list <- get.data.frame(var6, what = "vertices")

#Writing Node CSV 
write.csv2(node_list, "~/SNArRDJF/Banco Redes R/Networks/node_var6.csv", fileEncoding = "macintosh")

# Edge List
edge_list <- get.data.frame(var6, what = "edges") %>%
...
write.csv2(edge_list, "~/SNArRDJF/Banco Redes R/Networks/edge_var6.csv", fileEncoding = "macintosh")
