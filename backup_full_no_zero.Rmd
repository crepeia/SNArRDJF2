---
title: "SNA Centralities full_no_zero_fancy"
...
***USE THIS ONLY TO RUN THE FIRST OBJECT 'FULL.NO.ZERO'*** Keep this line commented in order to prevent damage for all further analyses that are made not complex graph objects
```{r}
full_no_zero<-simplify(full_no_zero)
```
...
#Histogram full_no_zero degree
hist(degree(full_no_zero, mode = "in", normalized = T), ylab="Frequency", xlab="Normalized Degree",  breaks=vcount(full_no_zero)/10, main="Histogram of Normalized Indegree Nodes - full_no_zero_fancy")
hist(degree(full_no_zero, mode = "out", normalized = T), ylab="Frequency", xlab="Normalized Degree",  breaks=vcount(full_no_zero)/10, main="Histogram of Normalized Indegree Nodes - full_no_zero_fancy")
hist(degree(full_no_zero, mode = "all", normalized = T), ylab="Frequency", xlab="Normalized Degree",  breaks=vcount(full_no_zero)/10, main="Histogram of Normalized All Degree Nodes - full_no_zero_fancy")
```
...
full_no_zero_a.nn.deg <- graph.knn(full_no_zero,V(full_no_zero))$knn

#Saving to igraph object
V(full_no_zero)$full_no_zero_a.nn.deg <- graph.knn(full_no_zero,V(full_no_zero))$knn
...
full_no_zero_a.nn.deg_w <- graph.knn(full_no_zero,V(full_no_zero), weights=E(full_no_zero)$weight)$knn
...
#Adding Title
  title("Network Vertex Degree Sized - full_no_zero_fancy", sub = "Source: from authors ", cex = .5)
...
plot(full_no_zero, 
     layout=layout.kamada.kawai(full_no_zero), 
     vertex.label=get.vertex.attribute(full_no_zero,"LABEL_COR"),
     vertex.size=sqrt(graph.knn(full_no_zero,V(full_no_zero), weights= E(full_no_zero)$weight)$knn),
     vertex.label.color="black",
     vertex.frame.color="#ffffff",
     vertex.label.color="black",
     edge.arrow.size=0.1,
     vertex.label.cex=log(graph.knn(full_no_zero,V(full_no_zero), weights= E(full_no_zero)$weight)$knn)/10,
     edge.width=edge.betweenness(full_no_zero, weights =E(full_no_zero)$weight)/500,
     edge.curved = TRUE
     )
...
#Adding Title
  ...
  text(x = -1, y = -1.2, labels = 
   sprintf("Median Average Weighte Neighbor Degree: %.2f",
     median(graph.knn(full_no_zero,V(full_no_zero), weights= E(full_no_zero)$weight)$knn)
   ))
```
...
```{r}
V(full_no_zero)$full_no_zero_closeness_stats_w <- closeness(full_no_zero, vids = V(full_no_zero), mode = "all", weights =E(full_no_zero)$weight, normalized = FALSE)

full_no_zero_closeness_stats_w <- closeness(full_no_zero, vids = V(full_no_zero), mode = "all", weights =E(full_no_zero)$weight, normalized = FALSE)
```
...
```{r}
#2.5.3 Closeness Measures Dinamic Table
...
```{r}
#2.5.3 Betweenness Vertex Centrality Measures Dinamic Table
#Getting  Measures
full_no_zero_betweenness <- betweenness(full_no_zero) %>% round(1)
full_no_zero_betweenness_w <- betweenness(full_no_zero, weights=E(full_no_zero)$weight) %>% round(1)
...
```{r}
#All shortest paths 
distances_sp_all_full_no_zero<-distances(full_no_zero, mode="all", weights=E(full_no_zero)$weight)
#distances_sp_all_full_no_zero

#In shortest paths 
distances_sp_in_full_no_zero<-distances(full_no_zero, mode="in",weights=E(full_no_zero)$weight)
#distances_sp_in_full_no_zero

#Out shortest paths 
distances_sp_out_full_no_zero<-distances(full_no_zero, mode="out", weights=E(full_no_zero)$weight)
...
#Non-weighted 
transitivity(full_no_zero, weights=E(full_no_zero)$weight, type="global")

#Weighted
transitivity(full_no_zero, weights=E(full_no_zero)$weight,type="weighted")
transitivity(full_no_zero, weights=E(full_no_zero)$weight,type="global")
...
transitivity_local_w<-transitivity(full_no_zero, weights=E(full_no_zero)$weight, type="barrat")
