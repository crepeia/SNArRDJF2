...
write.csv2(full_data_frame_id, "~/SNArRDJF/Banco Redes R/Banco CSV/full_data_frame_id.csv", fileEncoding = "macintosh")

#Changing ID to number on Excel (ps:I need to improve this code using R)
#full_data_frame_id=read.table("~/Banco Redes R/Banco CSV/full_data_frame_id.csv", sep=";", #stringsAsFactors=FALSE, header = T)
...
```{r}
vertex_col <- brewer.pal(length(unique(V(full_no_zero)$V1_LABEL_TIPO)), "Set3")
V(full_no_zero)$color <- vertex_col[factor(V(full_no_zero)$V1_LABEL_TIPO)]
```
...
"A) Este serviço mostra-se disponível para realização de ativades em conjunto.",
"B) O horário de funcionamento deste serviço possibilita a realização de ações em conjunto.",
"C) Este serviço disponibiliza informações sobre suas ativades para o seu serviço.",
"D) O seu serviço consegue acessar facilmente informações deste serviço.",
"E) Os canais de comunicação com este serviço são suficientes para realização de ações em conjunto.",
"F) Seu serviço tem facilade de entrar em contato com este serviço para realizar ativades em conjunto.",
"G) O acesso geográfico a este serviço impede a realização de ativades em conjunto.  ",
"H) O custo para acessar este serviço impede a realização de ativades em conjunto.",
"I) O tempo gasto para acessar este serviço impede a realização ativades em conjunto.",
"A) Seu serviço confia  na qualificação profissional da equipe deste serviço ",
"B) Seu serviço confia  na capacade técnica da equipe deste serviço.",
"C) Seu serviço confia  nas ações realizadas por este serviço.",
"D) Seu serviço confia  em compartilhar informações internas com este serviço. ",
"E) Seu serviço confia nas informações recebas deste serviço.",
"F) Seu serviço confia  em realizar ações conjuntas com este serviço.",
"G) Seu serviço está satisfeito  com as ações desempenhadas por este serviço.",
"H) Seu serviço espera que o relacionamento com este serviço continue nos próximos anos.",
"I) Seu serviço espera que o relacionamento com este serviço se fortaleça nos próximos anos.",
"atividade",
"acesso",
"confianca",
"total")

reachability2 <- function(g, m) {
reach_mat = matrix(nrow = vcount(g), ncol = vcount(g))
for (i in 1:vcount(g)) 
{reach_mat[i,] = 0}
this_node_reach = subcomponent(g, (i-1), mode = "in")
...
V(temp)$shortest.paths.in <- shortest.paths(temp, mode='in')
V(temp)$shortest.paths.out <- shortest.paths(temp, mode='out')
shortest.paths.in <- shortest.paths(var1, mode='in')
...
```{r}
#saveAsGEXF(full_no_zero, "~/SNArRDJF/Banco Redes R/Bancos GEFX/full_no_zero.gexf")
#saveAsGEXF(full_with_zero, "~/SNArRDJF/Banco Redes R/Bancos GEFX/full_with_zero.gexf")
```
