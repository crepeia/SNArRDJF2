...
hier.mask <- rep(1,16)
hier.mask[c(6:8,10:11)] Â <- 0

#compute triad count and triad proportion for a given weighting vector
mask.stat <- function(my.graph, my.mask){
  Â  Â  n.nodes <- vcount(my.graph)
  Â  Â  n.edges <- ecount(my.graph)
  Â  Â  #set probability of edge formation in random graph to proportion of possible edges present in original
  Â  Â  p.edge <- n.edges/(n.nodes*(n.nodes +1)/2)
  Â  Â  r.graph <- as.network.numeric(n.nodes, density = p.edge)
  Â  Â  r.igraph <- as.igraph(r.graph)
  Â  Â  tc.graph <- triad.census(r.igraph)
  Â  Â  clust <- sum(tc.graph*my.mask)
  Â  Â  clust.norm <- clust/sum(tc.graph)
  Â  Â  return(c(clust,clust.norm))
}
...
emp.distro <- function(this.graph){
  Â  clust <- matrix(rep(0,200), nrow=2)Â 
  Â  hier <- matrix(rep(0,200),nrow=2)
  Â  for(i in c(1:100)){
    Â  Â  Â clust[,i] <- mask.stat(this.graph, clust.mask)
    Â  Â  Â hier[,i] <- mask.stat(this.graph, hier.mask)
    Â  }
  Â  my.mat <- rbind(clust, hier)
  Â  rownames(my.mat) <- c("clust.ct", "clust.norm", "hier.ct", "hier.ct.norm")
  Â  return(my.mat)
}
...
#generate Â 95% empirical confidence intervals for triad counts
